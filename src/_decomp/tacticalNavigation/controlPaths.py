#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\tacticalNavigation\controlPaths.py
import math
...
class AreaIndication:
    ...
    def Start(self):
        self._started = True
        sm.GetService('sceneManager').RegisterPersistentUIObject((self, self._resPath), self.model)
        self.Update()
    ...
    def Abort(self):
        if self._started:
            sm.GetService('sceneManager').RemovePersistentUIObject((self, self._resPath))
...
class SinglePointPath:
    ...
    def Start(self, anchorCurve = None, areaIndication = None):
        ...
        if anchorCurve is not None:
            ...
            labelCurve = AverageVectorFunction([destCurve, trinity.Tr2TranslationAdapter()]).GetBlueFunction()
    ...
    def _UpdateDistanceText(self):
        if self.label is not None:
            ...
            self.label.SetText(str(max(0, int(distance / 1000.0 + 0.5))))
