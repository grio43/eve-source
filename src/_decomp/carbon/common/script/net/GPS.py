...
import log
import uthread
from carbon.common.script.net.GPSExceptions import GPSTransportClosed, GPSRemoteTransportClosed
...
from eve.common.script.util.writebuffer import WriteBuffer
...
class GPSAddressable(object):
    ...
    def __GetPrefixedAddress(self, prefixWhat, natOffset = False):
        ...
        if prefix:
        ...
        elif prefs.clusterMode not in ('LIVE', 'TEST'):
            ...
            if ipaddress is None:
                raise RuntimeError('No sane interfaces found from %s', addresses)
        else:
...
class GPSTransport(GPSAddressable):
    ...
    def HandleClientAuthentication(self, loggedOnUserCount, transportID, counter, sessionID, eveInVIP):
        ...
        try:
            ...
            packet = self.ApplyWithTimeout(self.UnEncryptedRead, (), message=message, timeout=handshakeTimeout)
            if len(packet) == 0:
                log.general.Log('Handshake Failed - Empty packet from %s' % self.GetAddress(), log.LGERR)
                self.Close(localization.GetByLabel('/Carbon/GPS/IncompatibleProtocol'), 'HANDSHAKE_INCOMPATIBLEPROTOCOL')
                return 'ERR'
            response = macho.Loads(packet)
            if '@' in response[5]:
            ...
            from carbon.server.script.net import handShake
            ...
            if not request['user_name'] or not (request['user_password'] or request['user_password_hash']) or logonQueuePosition > 1:
                return 'OK'
            if vipKey and vipKey != Crypto.CryptoHash(CaseFold(request['user_name'] or '')):
            ...
            if funcResult.get('errors', ''):
                log.general.Log('Errors in verifying handshake function %s' % funcResult['errors'], log.LGERR)
    ...
    def Authenticate(self, username, password, token):
        ...
        try:
            ...
            if username:
                self.UnEncryptedWrite(macho.Dumps((None, 'VK', Crypto.CryptoHash(CaseFold(username)))))
            if username is None:
