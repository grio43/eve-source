...
from carbon.common.script.net import objectCaching
...
from cluster import MACHONETMSG_TYPE_AUTHENTICATION_REQ
...
class ObjectCall():
    ...
    def CallUp(self, packet):
        if packet.command == MACHONETMSG_TYPE_AUTHENTICATION_REQ:
            ...
            if isinstance(object, basesession.ObjectConnection):
                objclass = object.GetObjectConnectionLogClass()
            else:
                objclass = macho.AssignLogName(object)
            IngressTracer.start_trace(packet, objclass, method)
            callTimer = basesession.CallTimer('%s::%s (ObjectCall\\Server)' % (objclass, method))
            ...
            try:
                ...
                if not isinstance(object, basesession.ObjectConnection):
        ...
        if packet.payload[0] == 1:
            ...
            if method == 'ClientHasReleasedTheseObjects':
                ...
                if isinstance(object, basesession.ObjectConnection):
                    ...
                    objclass = macho.AssignLogName(object)
                callTimer = basesession.CallTimer('%s::%s (ObjectNotify\\Server)' % (objclass, method))
                try:
                    func = getattr(object, method)
                    if method in object.__dict__.get('__restrictedcalls__', {}) or method.startswith('_'):
                        raise RuntimeError('This call can only be the result of a hack attack', method)
                    if not isinstance(object, basesession.ObjectConnection) and len(tup) == 4 and 'machoVersion' in tup[3]:
                        if len(tup[3]) == 1:
                            tup = (tup[0], tup[1], tup[2])
                        else:
                            del tup[3]['machoVersion']
    ...
    def GetObjectID(self, object):
        if type(object) == types.InstanceType:
            ...
            elif isinstance(object, Moniker):
                object.QuickResolve()
    ...
    def ObjectCallWithoutTheStars(self, oob, persistantObjectID, blocking, sess, destination, objectID, method, args, keywords):
        ...
        if blocking == 1:
            ...
            service_name = self.get_brokering_service_name(persistantObjectID)
            with EgressTracer(packet, service_name, method) as trace:
                bind_params = self.get_object_bind_params(persistantObjectID)
                trace.set_bind_parameters(bind_params)
                if oob:
                    packet.oob = oob
                ...
                if self.added_objects:
                    if 'OID+' in packet.oob:
                        ...
                        packet.oob['OID+'] = self.added_objects
                try:
                    callTimer = basesession.CallTimer('Remote Object Call::%s (ObjectCall\\Client)' % (method,))
                    try:
                        response = self.ForwardCallDown(packet)
                    finally:
                        callTimer.Done()

                    retval = self.FromPickle(response.payload, packet.destination)
    ...
    def get_brokering_service_name(self, persistantObjectID):
        if isinstance(persistantObjectID, tuple) and len(persistantObjectID) > 1:
            return persistantObjectID[0]
        return 'unknown object call type'

    def get_object_bind_params(self, persistantObjectID):
        if isinstance(persistantObjectID, tuple) and len(persistantObjectID) > 1:
            return persistantObjectID[1]
