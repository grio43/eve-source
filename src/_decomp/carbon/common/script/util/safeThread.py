#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\carbon\common\script\util\safeThread.py
import sys
import traceback
import blue
import uthread
from carbon.common.script.util import debug, timerstuff
    ...
    def __MainLoop(self):
        ...
        try:
            while self.__killLoop == False:
                ...
                if self.__debugging:
                    try:
                        if timerstuff.ClockThis(self.uniqueIDstring + '::SafeThreadLoop', self.SafeThreadLoop, now) == self.KILL_ME:
                            self.__killLoop = True
                    except SystemExit:
                        raise
                    except TaskletExit:
