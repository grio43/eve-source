#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\carbon\common\script\util\datatable.py
import datetime
        ...
        for r in row:
            ...
            if typeName == 'boolean':
                ...
                if isinstance(r, (int, long, float)):
                    temp.append('{{v:{}}}'.format(r))
                else:
                    raise DataTableException("Row doesn't contain number '%s'. Line(column): %i(%i). Value: %s" % (row,
                     cidx,
                     rownum,
                     r))
