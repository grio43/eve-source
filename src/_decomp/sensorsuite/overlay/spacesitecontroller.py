#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\sensorsuite\overlay\spacesitecontroller.py
from sensorsuite.error import InvalidClientStateError, BookmarkInvalidPositionError
import carbonui.const as uiconst
from sensorsuite.overlay.sitestore import SiteMapStore, DuplicateSiteIdError
...
class SpaceSiteController:
    ...
    def ProcessSiteDataUpdate(self, addedSites, removedSites, siteType, addSiteDataMethod):
        ...
        for siteID, siteInfoObject in addedSites.iteritems():
            try:
                siteData = addSiteDataMethod(siteID, siteInfoObject)
            except BookmarkInvalidPositionError:
                continue

            try:
                self.siteMaps.AddSiteToMap(siteType, siteID, siteData)
            except DuplicateSiteIdError:
                continue
