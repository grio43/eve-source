#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\eveclientqatools\blueobjectviewer.py
...
class TreeListWnd:
    ...
    def Show(self):
        winID = 'MyTreeListWnd'
        Window.CloseIfOpen(windowID=winID)
        wnd = Window.Open(windowID=winID)
        wnd.height = 400
        wnd.SetMinSize([400, 300])
        wnd.width = 510
        wnd.SetCaption('Blue Object Viewer')
        main = wnd.GetMainArea()
        self.scroll = eveScroll.Scroll(parent=main, align=uiconst.TOALL, padding=(5, 5, 5, 5))
        self.scroll.sr.id = 'somerandomid'
        self.DoSomeContent()

    def GetAContainerNode(self, sublevel, map_node):
        return GetFromClass(ListGroup, {'label': map_node.displayName,
         'sublevel': sublevel,
         'id': (123, map_node),
    ...
    def GetAnEntryNode(self, sublevel, map_node):
        return GetFromClass(Generic, {'label': '%s<t>%s' % (map_node.displayName, str(map_node.value)),
         'sublevel': sublevel,
         'id': (123, map_node)})
