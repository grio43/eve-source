#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\eveclientqatools\tablereports.py
import blue
import trinity
import uthread
...
def CreateTableWindow(name, caption, contentFunction, minWidth = 340, updateFreq = 1000):
    ...
    def _closure():
        ...
        scroll = eveScroll.Scroll(parent=main)
        scroll.sr.id = 'insider' + name + 'Scroll'

        def _updateList():
            while Window.IsOpen(windowID=winID):
    ...
    def SpaceLODOverviewCont():
        ...
        for each in scene.objects:
            ...
            for turretSet in getattr(each, 'turretSets', []):
                lod = trinity.Tr2LodResourceType.LOD_COUNT
                if trinity.EveTurretSetLOD.LOD_HIGHEST == turretSet.lodLevel:
                    lod = trinity.Tr2LodResourceType.LOD_MEDIUM
