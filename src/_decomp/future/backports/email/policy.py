#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\future\backports\email\policy.py
...
class EmailPolicy(Policy):
    ...
    def header_fetch_parse(self, name, value):
        if hasattr(value, u'name'):
            return value
        return self.header_factory(name, u''.join(value.splitlines()))
    ...
    def _fold(self, name, value, refold_binary = False):
        ...
        if refold or refold_binary and _has_surrogates(value):
            return self.header_factory(name, u''.join(lines)).fold(policy=self)
        return name + u': ' + self.linesep.join(lines) + self.linesep
