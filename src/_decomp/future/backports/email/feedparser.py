#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\future\backports\email\feedparser.py
...
class FeedParser(object):
    ...
    def _parsegen(self):
        ...
        if self._headersonly:
            ...
            return
        if self._cur.get_content_type() == u'message/delivery-status':
            while True:
            ...
            return
        if self._cur.get_content_maintype() == u'message':
            for retval in self._parsegen():
                if retval is NeedMoreData:
                    yield NeedMoreData
                    continue
            ...
            self._pop_message()
            return
        if self._cur.get_content_maintype() == u'multipart':
