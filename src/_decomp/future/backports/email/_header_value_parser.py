#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\future\backports\email\_header_value_parser.py
...
class TokenList(list):
    ...
    def pop_leading_fws(self):
        if self[0].token_type == u'fws':
            return self.pop(0)
        return self[0].pop_leading_fws()

    def pop_trailing_ws(self):
        if self[-1].token_type == u'cfws':
            return self.pop(-1)
        ...
        for part in self.parts:
            ...
            if folded.append_if_fits(part, tstr):
                if has_ew and not part.comments:
                    last_ew = len(folded.current) - 1
                elif part.comments or part.token_type == u'quoted-string':
                    last_ew = None
                continue
            part._fold(folded)
...
class Comment(WhiteSpaceTokenList):
    ...
    def quote(self, value):
        if value.token_type == u'comment':
            return str(value)
        return str(value).replace(u'\\', u'\\\\').replace(u'(', u'\\(').replace(u')', u'\\)')
...
class Address(TokenList):
    ...
    def mailboxes(self):
        if self[0].token_type == u'mailbox':
            return [self[0]]
        if self[0].token_type == u'invalid-mailbox':
            return []
...
class GroupList(TokenList):
    ...
    @property
    def mailboxes(self):
        if not self or self[0].token_type != u'mailbox-list':
            return []
        return self[0].mailboxes
...
class Group(TokenList):
    ...
    @property
    def mailboxes(self):
        if self[2].token_type != u'group-list':
            return []
...
class Mailbox(TokenList):
    ...
    @property
    def display_name(self):
        if self[0].token_type == u'name-addr':
            return self[0].display_name
...
class Value(TokenList):
    ...
    def stripped_value(self):
        ...
        if token.token_type == u'cfws':
            token = self[1]
        if token.token_type.endswith((u'quoted-string', u'attribute', u'extended-attribute')):
            return token.stripped_value
        return self.value
...
class ParameterizedHeaderValue(TokenList):
    ...
    @property
    def parts(self):
        if self and self[-1].token_type == u'mime-parameters':
