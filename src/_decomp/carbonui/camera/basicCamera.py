#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\carbonui\camera\basicCamera.py
import blue
import math
import geo2
import trinity
...
class BasicCamera(object):
    ...
    def GetRay(self, x, y):
        ...
        if trinity.IsRightHanded():
            ...
            startPoint = trinity.TriVector(self.frontClip * dx, self.frontClip * dy, self.frontClip)
            endPoint = trinity.TriVector(self.backClip * dx, self.backClip * dy, self.backClip)
        invViewMatrix = mathCommon.ConvertTupleToTriMatrix(self.viewMatrix.transform)
