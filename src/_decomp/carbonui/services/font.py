#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\carbonui\services\font.py
import blue
import telemetry
import types
import trinity
...
class Tr2GlyphString(list):
    ...
    def Insert(self, params, text, idx, align = 0):
        ...
        dpi_fontsize = int(check_convert_font_size(params.fontsize) * uicore.desktop.dpiScaling + 0.5)
...
import re
tag = re.compile('<color=0x.*?>|</color>|<right>|<center>|<left>')
...
class FontHandler(object):
    ...
    @telemetry.ZONE_METHOD
    def GetTextHeight(self, strng, width = 0, font = None, fontsize = None, lineSpacing = 0.0, letterspace = 0, shadow = [(1, -1, -1090519040)], uppercase = 0, specialIndent = 0, getTextObj = 0, tabs = [], fontPath = None, fontStyle = None, fontFamily = None, linespace = None, maxLines = None, bold = False, **kwds):
        if strng is None or strng == '':
            return 0
        fontsize = check_convert_font_size(fontsize or fontconst.DEFAULT_FONTSIZE)
    ...
    @telemetry.ZONE_METHOD
    def GetTextWidth(self, strng, fontsize = 12, letterspace = 0, uppercase = 0, font = None, fontPath = None, fontStyle = None, fontFamily = None, bold = False):
        if not strng:
            return 0
        font = font
        fontsize = check_convert_font_size(fontsize or fontconst.DEFAULT_FONTSIZE)
        ...
        if '<br>' in strng:
            val = max([ self.GetTextWidth(line, fontsize, letterspace, uppercase, font, fontPath, fontStyle, fontFamily, bold) for line in strng.split('<br>') ])
        else:
            if self.defaultLabelClass is None:
                from carbonui.control.label import LabelCore
                self.defaultLabelClass = LabelCore
            textmeasurer = self.defaultLabelClass(text=StripTags(strng, ignoredTags=['b']), parent=None, align=uiconst.TOPLEFT, fontsize=fontsize, letterspace=letterspace, uppercase=uppercase, measuringText=True, fontPath=fontPath, fontStyle=fontStyle, fontFamily=fontFamily, color=None, bold=bold)
    ...
    def MeasureTabstops(self, stringsData, margin = None, minwidth = 32):
        ...
        for stringData in stringsData:
            ...
            for columnText in columnTexts:
                width = max(0, shift - margin) + margin + self.GetTextWidth(columnText, fontsize, letterSpace, uppercase) + margin
                if width > tabstops[i]:
                    tabstops[i] = width
                i += 1
                shift = 0
    ...
    def GetFontFamilyBasedOnClientLanguageID(self):
        ...
        if languageID == 'ZH':
            ...
            return self.GetFontFamilyBasedOnWindowsLanguageID(languageConst.LANG_KOREAN)
        else:
    ...
    def GetFontFamilyBasedOnWindowsLanguageID(self, windowsLanguageID):
        if fontconst.FONT_FAMILY_OVERRIDE:
            return fontconst.FONT_FAMILY_OVERRIDE
        if windowsLanguageID in fontconst.LANGUAGES_USING_EVE_FONT:
