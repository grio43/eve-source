#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\carbonui\primitives\layoutGrid.py
import weakref
from carbonui import uiconst
from carbonui.primitives.childrenlist import PyChildrenList as ChildrenList
...
class LayoutGridCell(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        if cellObject:
            cellObjectRef = weakref.ref(cellObject)
            if cellObject.align != uiconst.TOALL:
                cellObject._originalUpdateAlignment = cellObject.UpdateAlignment
                cellObject.UpdateAlignment = lambda *args, **kwds: self.UpdateCellObjectAlignment(cellObjectRef(), *args, **kwds)
            cellObject._originalClose = cellObject.Close
            cellObject.Close = lambda *args: self.CloseCellObject(cellObjectRef(), *args)
    ...
    def GetCellSize(self):
        ...
        if self._content.children:
            ...
            if cellObjectAlign in (uiconst.CENTERLEFT, uiconst.CENTERRIGHT):
                neededCellWidth = cellObject.left + cellObject.width
                neededCellHeight = cellObject.top * 2 + cellObject.height
            elif cellObjectAlign in (uiconst.CENTERTOP, uiconst.CENTERBOTTOM):
                neededCellWidth = cellObject.left * 2 + cellObject.width
                neededCellHeight = cellObject.top + cellObject.height
        ...
        if self.minWidth:
            neededCellWidth = max(self.minWidth, neededCellHeight)
        if self.minHeight:
            neededCellHeight = max(self.minHeight, neededCellHeight)
...
class LayoutGrid(Container):
    ...
    def AddCell(self, cellObject = None, colSpan = 1, rowSpan = 1, cellPadding = None, bgColor = None, cellClipChildren = False, cellClass = None, minWidth = None, minHeight = None, **keywords):
        if self.destroyed:
            return
        ...
        cell = cellClass(parent=self, align=uiconst.NOALIGN, colSpan=colSpan, rowSpan=rowSpan, cellObject=cellObject, cellPadding=cellPadding, gridCellPadding=self.cellPadding, clipChildren=cellClipChildren, bgColor=bgColor, minWidth=minWidth, minHeight=minHeight)
    ...
    def AddRow(self, rowObjects = None, rowClass = None, **keywords):
        colSpan = keywords.get('colSpan', self.columns)
        keywords['columns'] = colSpan
