#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\carbonui\primitives\gradientSprite.py
import math
import trinity
...
class GradientSprite(Sprite):
    ...
    def _CheckColorBlindReplace(self):
        if not self.ignoreColorBlindMode:
            self.colorData = [ (x, colorblind.CheckReplaceColor(color, maintainBrightness=True)) for x, color in self.colorData ]
