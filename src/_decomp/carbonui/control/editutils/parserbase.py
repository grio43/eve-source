#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\carbonui\control\editutils\parserbase.py
import _weakref
import sys
import types
import urllib2
...
class ParserBaseCore(object):
    ...
    def _Reset(self):
        ...
        attrEntry['color'] = getattr(self, 'defaultFontColor', TextColor.NORMAL.rgba)
    ...
    def BreakLines(self, textbuffer, batchHeight = 0, abspos = 0, recurse = 1):
        ...
        for rpush, lpush, startpos, s, others in self.GetLine(textbuffer, abspos, recurse):
            ...
            for obj, pos in inlines:
                ...
                obj.inlineWidth = obj.control.width
                obj.inlineHeight = obj.control.height
                valign = getattr(obj, 'valign', parser.ALIGNBASELINE)
                if valign in (parser.ALIGNTOP, parser.ALIGNSUB, parser.ALIGNSUPER):
                    baseHeight = max(baseHeight, objectHeight)
                elif valign == parser.ALIGNMIDDLE:
                    baseLine = max(baseLine, max(baseHeight, objectHeight) / 2 - (baseHeight - baseLine) / 2)
                    baseHeight = max(baseHeight, objectHeight)
                elif valign == parser.ALIGNBOTTOM:
                    diffBefore = baseHeight - baseLine
                    baseHeight = max(baseHeight, objectHeight)
                    baseLine = baseHeight - diffBefore
                elif valign == parser.ALIGNBASELINE:
    ...
    def GetScrollEntry(self, attrs):
        decoClass = get_inline_deco_class(attrs.type)
    ...
    def CheckURL(self, URL, getAnchor = 0):
        ...
        URL, anchor = browserutil.ParseURL(URL, currentURL)
        if URL.startswith('./'):
            if currentURL.endswith('/'):
                base = currentURL
            else:
                base = browserutil.DirUp(currentURL)
            URL = base + URL[2:]
        elif URL.startswith('../'):
            base = browserutil.DirUp(browserutil.DirUp(currentURL))
            URL = base + URL[3:]
        if getAnchor:
            return (URL, anchor)
        return URL
    ...
    def OnStart_li(self, attrs):
        ...
        if tag == 'ul':
            ...
            if orderType in 'aA':
                ...
                text = eveformat.number_roman(idx).lower()
            else:
    ...
    def OnStart_link(self, attrs):
        ...
        if rel.lower() == 'stylesheet':
            ...
            if href.startswith('res:') or href.startswith('script:'):
                ...
                return
            fullPath = browserutil.ParseURL(url, currentURL)[0]
            try:
                buff = browserutil.GetStringFromURL(*(browserutil.ParseURL(fullPath)[:1] + (None,))).read()
    ...
    def OnStart_form(self, attrs):
        ...
        if self.sr.Get('browser', None):
            self.sr.form = browserutil.NewBrowserForm(attrs, _weakref.proxy(self.sr.browser))
        else:
            self.sr.form = browserutil.NewBrowserForm(attrs, _weakref.proxy(self))
    ...
    def OnStart_input(self, attrs):
        ...
        if form is None:
            form = self.sr.form = browserutil.NewBrowserForm(attrs, _weakref.proxy(self))
        s = {'width': None,
         'height': None}
    ...
    def OnStart_tr(self, attrs):
        ...
        if attrs.valign is not None:
            s['vertical-align'] = getattr(parser, 'ALIGN%s' % unicode(attrs.valign).upper(), 1)
        else:
            s['vertical-align'] = parser.ALIGNTOP
        s['background-image'] = None
        s['background-color'] = None
        if attrs.bgcolor is not None:
            s['background-color'] = ParseHTMLColor(attrs.bgcolor, 1)
