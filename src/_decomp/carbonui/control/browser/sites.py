#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\carbonui\control\browser\sites.py
import cPickle
import hashlib
import urlparse
import urllib
...
class SitesSvc(Service):
    ...
    def MakeBookmarkEntry(self, name, url):
        return utillib.KeyVal(name=name, url=url)
    ...
    def GetSiteKey(self):
        return utillib.KeyVal(trusted=0, temporary=0, auto=0, community=0, protocol='', hostname='', path='')
    ...
    def __PopulateSystemLists(self):
        ...
        self.systemLists = utillib.KeyVal(blacklist=[], bannedInChat=[], whitelist=['about:blank'], community=[], autotrusted=[])
