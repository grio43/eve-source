#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\projectdiscovery\client\projects\subcellular\categoryselector.py
import blue
import localization
import trinity
...
class CategoryGroup(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.starting_scale = attributes.get('starting_scale')
        self.sub_categories = []
        self.label = eveLabel.Label(name='categorygrouplabel', text=category['name'].upper(), parent=self, color=(1, 1, 1, 0.5), top=4, left=self.HEX_WIDTH, opacity=0, fontSize=20)
...
class CategoryGroupHexagon(Hexagon):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.groupLabel = eveLabel.Label(name='groupLabel', text=str(attributes.get('number') + 1), parent=self, fontsize=self.originalFontSize, align=uiconst.CENTER, color=(1, 1, 1, 0.25))
        self.groupHexImage = Sprite(parent=self, align=uiconst.TOALL, texturePath='res:/UI/Texture/classes/ProjectDiscovery/hexMask.png', state=uiconst.UI_DISABLED, color=(0.2, 0.2, 0.2, 1))
        self.resize_to_scale(self.starting_scale)
...
class CategoryHexagon(Hexagon):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.consensus_percentage = eveLabel.Label(parent=self, align=uiconst.CENTER, text='0', fontsize=16, state=uiconst.UI_HIDDEN)
        ...
        self.selected_texture = Sprite(name='selected_texture', parent=self, texturePath='res:/UI/Texture/classes/ProjectDiscovery/categorySelected.png', align=uiconst.TOALL, opacity=0, idx=0, state=uiconst.UI_DISABLED, padding=(-8, -8, -8, -8))
        self.correct_texture = Sprite(parent=self, texturePath='res:/UI/Texture/classes/ProjectDiscovery/categoryMatch.png', opacity=0, idx=0, state=uiconst.UI_DISABLED, align=uiconst.TOALL, padding=(-8, -8, -8, -8))
        self.unmatched_texture = Sprite(parent=self, texturePath='res:/UI/Texture/classes/ProjectDiscovery/categoryWrong.png', align=uiconst.TOALL, padding=(-8, -8, -8, -8), opacity=0, idx=0, state=uiconst.UI_DISABLED)
        self.missed_texture = Sprite(parent=self, texturePath='res:/UI/Texture/classes/ProjectDiscovery/categoryMissed.png', align=uiconst.TOALL, padding=(-8, -8, -8, -8), opacity=0, idx=0, state=uiconst.UI_DISABLED)
        self.color_overlay = Sprite(parent=self, align=uiconst.TOALL, texturePath='res:/UI/Texture/classes/ProjectDiscovery/hexMask.png', textureSecondaryPath='res:/UI/Texture/classes/ProjectDiscovery/hexMask.png', spriteEffect=trinity.TR2_SFX_MASK, state=uiconst.UI_DISABLED, opacity=0, padding=(5, 5, 5, 5))
        self.image = Sprite(name='image', parent=self, align=uiconst.TOALL, texturePath='res:/UI/Texture/classes/ProjectDiscovery/subcellular/' + self.category['url'][:-4] + '_NA.png', textureSecondaryPath='res:/UI/Texture/classes/ProjectDiscovery/hexMask.png', spriteEffect=trinity.TR2_SFX_MASK, state=uiconst.UI_DISABLED)
...
class CategoryTooltipWrapper(TooltipBaseWrapper):
    ...
    def add_example_images(self):
        ...
        for imagePath in image_paths:
            TooltipExampleSprite(name='TooltipExampleSprite', parent=Container(parent=image_icon_container, width=self.IMAGE_SIZE / len(image_paths), align=uiconst.TOLEFT), align=uiconst.TOALL, padding=(5, 5, 5, 5), texturePath=imagePath, originalPath=self._imagePath)

        self.tooltipPanel.AddCell(image_icon_container, cellPadding=(0, 5, 0, 5))

    def add_main_image_container(self):
        image_container = Container(width=self.IMAGE_SIZE, height=self.IMAGE_SIZE, align=uiconst.TOPLEFT)
        themeColored.FrameThemeColored(bgParent=image_container, colorType=uiconst.COLORTYPE_UIHILIGHT)
        self.imageOverlaySprite = Sprite(parent=image_container, width=self.IMAGE_SIZE - 2, height=self.IMAGE_SIZE - 2, texturePath='res:/UI/Texture/classes/ProjectDiscovery/subcellular/' + self.category['url'][:-4] + '_A.png', align=uiconst.CENTER, opacity=0)
        self.imageSprite = Sprite(parent=image_container, width=self.IMAGE_SIZE - 2, height=self.IMAGE_SIZE - 2, texturePath=self._imagePath, align=uiconst.CENTER)
...
class TooltipExampleSprite(Sprite):
