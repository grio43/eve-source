#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\projectdiscovery\client\projects\subcellular\subcellularwindow.py
import logging
import math
import blue
import localization
...
from projectdiscovery.client.const import Events, Sounds, rank_paths
...
class ProjectDiscoveryWindow(Window):
    ...
    def on_window_closed(self, wndID, wndCaption, wndGUID):
        sm.GetService('audio').SendUIEvent(Sounds.MainImageLoopStop)
        sm.GetService('audio').SendUIEvent(Sounds.MainImageLoadStop)
        sm.GetService('audio').SendUIEvent(Sounds.RewardsWindowLoopStop)
    ...
    def on_window_maximized(self, wnd, wasMinimized):
        sm.GetService('audio').SendUIEvent(Sounds.MainImageLoopPlay)
    ...
    def resize_background(self, scale):
        try:
            ...
            logger.warning('ProjectDiscovery window - resize_background Attribute Error', exc_info=1)
    ...
    def ApplyAttributes(self, attributes):
        ...
        try:
            self.player_statistics = self.projectdiscoverySvc.get_player_statistics(get_history=True)
    ...
    def setup_layout(self):
        ...
        self.bottom_container = Container(name='bottom_container', parent=self.sr.main, align=uiconst.TOBOTTOM, height=50)
        self.project_container = Container(name='ProjectContainer', parent=self.main, align=uiconst.TOPLEFT, height=self.default_minSize[1], width=self.default_minSize[0], top=-20)
        ...
        self.header_element = WindowHeader(parent=self.main.parent, align=uiconst.CENTERTOP, height=53, width=355, idx=0, top=2, bgTexturePath='res:/UI/Texture/classes/ProjectDiscovery/headerBG.png', playerState=self.playerState, playerStatistics=self.player_statistics)
        self.help_button = ButtonIcon(name='helpButton', parent=self.bottom_container, align=uiconst.BOTTOMLEFT, iconSize=22, width=22, height=22, texturePath='res:/UI/Texture/WindowIcons/question.png', func=lambda : self.start_tutorial())
    ...
    def setup_side_panels(self):
        Sprite(parent=self.main, align=uiconst.CENTERRIGHT, width=14, height=416, top=-20, texturePath='res:/UI/Texture/classes/ProjectDiscovery/sideElement.png')
        Sprite(parent=Transform(parent=self.main, align=uiconst.CENTERLEFT, width=14, height=416, top=-20, rotation=math.pi), align=uiconst.TOLEFT_NOPUSH, width=14, height=416, texturePath='res:/UI/Texture/classes/ProjectDiscovery/sideElement.png')

    @on_event(Events.SubmitSolution)
    def hide_help_button(self):
        self.help_button.SetState(uiconst.UI_HIDDEN)

    @on_event(Events.RewardViewFadeOut)
    def show_help_button(self):
        self.help_button.SetState(uiconst.UI_NORMAL)

    @on_event(Events.NoConnectionToAPI)
    ...
    def get_tutorial_tooltip_steps(self):
        return [{'owner': self.header_element.rankIcon,
          'header': localization.GetByLabel('UI/ProjectDiscovery/Subcellular/Tutorial/RankIconHeader'),
          'text': localization.GetByLabel('UI/ProjectDiscovery/Subcellular/Tutorial/RankIconText')}, {'owner': self.header_element.accuracyRatingContainer,
    ...
    def show_background_grid(self):
        self.gridContainer = Container(name='gridContainer', parent=self.main, align=uiconst.TOALL, clipChildren=True, padBottom=-49)
        self.background_shade = Sprite(name='background_shade', parent=self.gridContainer, align=uiconst.TOALL, texturePath='res:/UI/Texture/classes/ProjectDiscovery/backgroundShade.png', padTop=-20, padLeft=2, padRight=2, padBottom=0, opacity=0.5)
        self.gridBackground = Sprite(name='gridBackground', parent=self.gridContainer, align=uiconst.TOPLEFT, texturePath='res:/UI/Texture/classes/ProjectDiscovery/hexBGTile.png', tileX=True, tileY=True, scale=(2, 2))
    ...
    def on_analysis_kredits_update(self):
        ...
        self.header_element.update_analysis_kredits(player_state)
...
class WindowHeader(Container):
    ...
    def setup_layout(self):
        ...
        self.rankIcon = Sprite(name='rankIcon', parent=self.rankInfoContainer, texturePath=self.get_rank_icon_path(), height=36, width=36, align=uiconst.TOLEFT, left=5)
        ...
        self.rankLabel = eveLabel.Label(parent=self.rankInfoContainer, fontsize=16, text=self.rank, align=uiconst.CENTERLEFT, height=20, left=40)
        self.accuracyRatingContainer = Container(name='accuracyRatingContainer', parent=self.headerContainer, align=uiconst.TORIGHT, width=75, left=5, top=3)
        self.accuracyRatingIconContainer = Container(name='accuracyRatingIconContainer', parent=self.accuracyRatingContainer, height=32, width=32, align=uiconst.CENTER, left=20, bgTexturePath='res:/UI/Texture/classes/ProjectDiscovery/accuracyMeterBack.png')
        ...
        self.accuracyRatingLabel = eveLabel.Label(name='AccuracyRating', parent=self.accuracyRatingContainer, fontsize=16, text='00,0%', align=uiconst.CENTERLEFT, autoFitToText=True, height=20)
        self.state = uiconst.UI_NORMAL
    ...
    def get_player_statistics(self, get_history):
        ...
        try:
            ...
            sm.ScatterEvent(Events.NoConnectionToAPI)
    ...
    def get_rank_icon_path(self):
        return rank_paths[calculate_rank_band(self.rank)]
