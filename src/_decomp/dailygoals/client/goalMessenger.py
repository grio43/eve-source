#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\dailygoals\client\goalMessenger.py
import httplib
import logging
import uuid
import blue
...
from eveProto.generated.eve_public.dailygoal.api.requests_pb2 import GetRequest, GetResponse, GetAllCurrentRequest, GetAllCurrentResponse, GetAllWithRewardsRequest, GetAllWithRewardsResponse, RedeemRequest, RedeemResponse, PayForCompletionRequest, PayForCompletionResponse
...
class DailyGoalMessenger(object):
    _instance = None
    public_gateway = None
    ...
    def get_instance(cls, public_gateway):
        if not cls._instance:
            cls._instance = DailyGoalMessenger(public_gateway)
    ...
    def iter_format_goals(self, goals):
        for goal in goals:
            yield format_goal_from_payload(goal_id=uuid.UUID(bytes=goal.id.uuid), goal_attributes=goal.goal, goal_progress=goal.current_progress, earnings=goal.earnings, paid_completion=goal.paid_completion)
    ...
    def get_current_goals(self):
        ...
        if payload.goals is None:
            return []
        data = [ goal for goal in self.iter_format_goals(payload.goals) if goal is not None ]
        return data

    def get_goal(self, goal_id):
        logger.info('Daily Goals: sending GetRequest for goal %s' % goal_id)
        request = GetRequest()
    ...
    def pay_for_completion(self, goal_id):
        logger.info('Daily Goals: sending PayForCompletionRequest for goal %s' % goal_id)
        request = PayForCompletionRequest()
        request.goal.uuid = goal_id.bytes
        try:
            self._send_request(request, PayForCompletionResponse)
        except GoalMessengerError as gme:
            logger.error('Daily Goals: error while paying to complete goal %s' % goal_id)
            raise gme
    ...
    def redeem_reward(self, goal_id, do_redeem_current_location):
        logger.info('Daily Goals: sending RedeemRequest for goal %s, to current location: %s' % (goal_id, do_redeem_current_location))
    ...
    def on_goal_redeemed(self, payload):
        logger.info('Daily Goals: goal %s Redeemed Notice' % payload.goal)
        goal_id = uuid.UUID(bytes=payload.goal.uuid)
