#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\chat\client\member_list_entry.py
import sys
...
class ChatMemberListEntryUnderlay(ListEntryUnderlay):
    default_padBottom = 0


class ChatMemberListEntry(SE_BaseClassCore):
    __guid__ = 'listentry.ChatMemberListEntry'
    __notifyevents__ = ['OnContactLoggedOn',
     ...
     'OnDisapprovalUpdate']
    default_highlightClass = ChatMemberListEntryUnderlay
    isDragObject = True
    charid = None
    selected = 0
    id = None
    ...
    def Startup(self, scroll, *args, **kwargs):
        self._CreatePicture()
        self._CreateName()
        self.flagCont = None
    ...
    def GetMenu(self):
        if self.destroyed:
            ...
            if len(selected) < 2:
                m = GetMenuService().GetMenuFromItemIDTypeID(self.id, self.sr.node.invtype, channelID=self.channelID)
                if self.charid != session.charid:
                    m += [None, (MenuLabel('UI/Chat/ReportIskSpammer'), self._ReportISKSpammer)]
                return m
            charIDs = []
    ...
    def _ReportISKSpammer(self, *args):
    ...
    def OnDblClick(self, *args):
        ...
        if onDblClick == 0:
            ...
            sm.GetService('chat').invite_to_private_chat(self.charid)
    ...
    def UpdateRelationship(self, characterID):
        if self.destroyed:
            return
        charID = self.GetCharIdFromNode()
        if charID is None or charID != characterID:
            return
...
class ChatMemberCompactListEntry(ChatMemberListEntry):
    __guid__ = 'listentry.ChatMemberCompactListEntry'
    ENTRYHEIGHT = 16

    def Startup(self, *args):
        ChatMemberListEntry.Startup(self, *args)
    ...
    def Load(self, node):
        ChatMemberListEntry.Load(self, node)
