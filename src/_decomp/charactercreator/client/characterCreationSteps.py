#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\charactercreator\client\characterCreationSteps.py
import charactercreator.const as ccConst
STEPS_BY_MODE = {ccConst.MODE_FULL_BLOODLINECHANGE: [ccConst.EMPIRESTEP,
 ...
 ccConst.MODE_FULLINITIAL_CUSTOMIZATION: [ccConst.EMPIRESTEP,
                                          ...
                                          ccConst.CUSTOMIZATIONSTEP,
                                          ccConst.NAMINGSTEP]}
STEP_ORDER_LIST = [ccConst.EMPIRESTEP,
 ccConst.TECHNOLOGYSTEP,
 ccConst.BLOODLINESTEP,
 ccConst.CUSTOMIZATIONSTEP,
 ccConst.PORTRAITSTEP,
...
def GetStepOrder(step):
    try:
        return STEP_ORDER_LIST.index(step)
    except ValueError:
        return -1
...
def GetStepsUpToAndIncluding(lastStep):
    s = set()
    if lastStep not in STEP_ORDER_LIST:
        return s
    for step in STEP_ORDER_LIST:
        s.add(step)
        if step == lastStep:
            return s
