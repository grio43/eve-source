#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\charactercreator\client\characterDollUtilities.py
import charactercreator.const as ccConst
from eve.common.script.paperDoll.paperDollDefinitions import BODY_CATEGORIES, GENDER, HAIR_CATEGORIES, SEPERATOR_CHAR
    ...
    def ApplyTypeToDoll(self, itemType, weight = 1.0, doUpdate = True, rawColorVariation = None, metadata = None, factory = None, doll = None):
        ...
        if type(itemType) is not tuple:
            ...
            if itemTypeData is None:
                ...
                if BODY_CATEGORIES.TOPINNER in itemTypeLower:
                    self.ApplyItemToDoll(BODY_CATEGORIES.TOPINNER, None, removeFirst=True, doUpdate=False, doll=doll, metadata=metadata, factory=factory)
                elif BODY_CATEGORIES.BOTTOMINNER in itemTypeLower:
                    self.ApplyItemToDoll(BODY_CATEGORIES.BOTTOMINNER, None, removeFirst=True, doUpdate=False, doll=doll, metadata=metadata, factory=factory)
                else:
        ...
        for otherCategory, otherResourceID in metadata.types.iteritems():
            ...
            if otherResource.typeID is None:
                continue
            if otherResource.clothingRemovesCategory == modifierLocationKey:
        ...
        myTypeID = itemType[2]
        if myTypeID:
            removesCategory = cfg.paperdollResources.Get(itemType[0]).clothingRemovesCategory
            if removesCategory:
