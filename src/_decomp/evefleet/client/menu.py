#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\evefleet\client\menu.py
from __future__ import absolute_import
from collections import defaultdict
from menu import MenuList
from evefleet.const import fleetRoleMember, fleetRoleSquadCmdr, fleetRoleWingCmdr, fleetRoleLeader, fleetJobCreator, MAX_MEMBERS_IN_SQUAD
RANK_LABELS = {fleetRoleLeader: 'UI/Fleet/Ranks/FleetCommander',
 fleetRoleWingCmdr: 'UI/Fleet/Ranks/WingCommander',
 fleetRoleSquadCmdr: 'UI/Fleet/Ranks/SquadCommander'}

def GetRankLabel(role):
    return RANK_LABELS.get(role, 'UI/Fleet/Ranks/SquadMember')
...
class MemberMenu(object):
    ...
    def Get(self):
        ...
        if self.IsInviteToFleet():
            self.fleetMenu.append([self.MenuLabel(GetRankLabel(fleetRoleMember)), self.callback, (self.targetCharID,
              None,
              None,
              None)])
    ...
    def GetMenuEntry(self, role, wingID, squadID):
        return [self.MenuLabel(GetRankLabel(role)), self.callback, (self.targetCharID,
          wingID,
          squadID,
          role)]
    ...
    def IsSquadFull(self, squadID, squadSize):
        if squadSize >= MAX_MEMBERS_IN_SQUAD and (not self.targetMember or not self.IsTargetMemberSquad(squadID)):
            return True
        else:
            return False
