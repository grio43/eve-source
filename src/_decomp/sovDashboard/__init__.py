#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\sovDashboard\__init__.py
...
def CalculateStructureStatusFromStructureInfo(structureInfo):
    ...
    return CalculateStructureStatusCampaignStateAndVulnerability(campaignState, vulnerabilityState)


def CalculateStructureStatusCampaignStateAndVulnerability(campaignState, vulnerabilityState):
    if campaignState:
...
def GetSovStructureInfoByTypeID(solarsystemStructureInfo):
    structureInfosByTypeID = OrderedDict([(typeTerritorialClaimUnit, KeyVal(typeID=typeTerritorialClaimUnit)), (typeInfrastructureHub, KeyVal(typeID=typeInfrastructureHub))])
