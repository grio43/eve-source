#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\sovDashboard\dashboardEntry.py
...
class DashboardEntry(MouseInsideScrollEntry):
    ...
    def GetMenu(self):
        m = GetMenuService().GetMenuFromItemIDTypeID(itemID=self.node.structureInfo['itemID'], typeID=self.node.structureInfo['typeID'])
        return m
    ...
    def GetSolarSystemText(node):
        ...
        text = '%s <color=%s>%s</color>' % (solarsystemLink, Color.RGBtoHex(color[0], color[1], color[2]), securityStatus)
        return text
    ...
    def GetJumpText(node):
        if IsUnreachableJumpCount(node.jumpCount):
            return '-'
        else:
