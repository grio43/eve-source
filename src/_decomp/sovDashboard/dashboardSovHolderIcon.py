#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\sovDashboard\dashboardSovHolderIcon.py
...
class SovHolderIcon(Container):
    ...
    def GetTexturePaths(structureTypeID, large = False):
        if structureTypeID == const.typeInfrastructureHub:
            texturePath = eveicon.sov_hub
    ...
    def LoadTooltipPanelNoOwner(self, tooltipPanel, *args):
        ...
        structureName = evetypes.GetName(self.structureInfo['typeID'])
    ...
    def GetMenu(self):
        m = GetMenuService().GetMenuFromItemIDTypeID(itemID=self.structureInfo.get('itemID', None), typeID=self.structureInfo['typeID'])
        return m
