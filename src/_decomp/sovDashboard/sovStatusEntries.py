#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\sovDashboard\sovStatusEntries.py
...
class SovSystemStatusEntry(MouseInsideScrollEntry):
    ...
    def ApplyAttributes(self, attributes):
        ...
        devIndex = self.node.get('devIndex', None)
        ...
        if currentIndex is not None:
            ...
            if devIndex is not None:
                self.AddDeltaSprite(indexID, devIndex)
    ...
    def AddDeltaSprite(self, indexID, devIndex):
        indexInfo = sm.GetService('sov').GetLevelInfoForIndex(indexID, devIndex=devIndex)
        changeSprite = Sprite(parent=self, align=uiconst.CENTERRIGHT, pos=(0, 0, 16, 16))
...
class SovStructureStatusEntry(MouseInsideScrollEntry):
    ...
    def GetMenu(self):
        m = GetMenuService().GetMenuFromItemIDTypeID(itemID=self.node.structureInfo.get('itemID', None), typeID=self.node.structureInfo.get('typeID', None))
        return m
