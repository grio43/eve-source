#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\fsdlite\cache.py
import os
import time
import sqlite3
import threading
...
class Cache(DictMixin):
    ...
    def __del__(self):
        if Cache and hasattr(Cache, '_connections'):
            connection = Cache._connections.get(self.path, None)
            if connection:
                connection.commit()
                connection.close()
                del Cache._connections[self.path]
