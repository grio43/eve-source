#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\eveAssets\assetSearchUtil.py
import re
...
class AssetKeywordSearch(object):
    ...
    def __init__(self, nameHelper, uiSvc, mapSvc, godma, structureDirectorySvc, localization, cfg, *args):
        self.structureDirectorySvc = structureDirectorySvc
        self.uiSvc = uiSvc
        self.mapSvc = mapSvc
        self.godma = godma
    ...
    def MatchTypeExact(self, conditions, value):
        typeName = value
        ...
        def CheckType(item):
            try:
                ...
                tName = self.nameHelper.GetTypeName(item.typeID).strip()
                tName = self.localization.CleanImportantMarkup(tName)
                matched = tName == typeName
                self.matchResultsTypeNamesByTypeIDs[item.typeID] = matched
                return matched
    ...
    def GetSearchKeywords(self):
        keywords = [KeywordOption(self.getByLabelFunc('UI/Inventory/AssetSearch/KeywordType'), self.getByLabelFunc('UI/Inventory/AssetSearch/DescriptionType'), None, self.MatchType),
         KeywordOption(self.getByLabelFunc('UI/Inventory/AssetSearch/KeywordTypeExact'), self.getByLabelFunc('UI/Inventory/AssetSearch/DescriptionTypeExact'), None, self.MatchTypeExact),
         ...
         KeywordOption(self.getByLabelFunc('UI/Inventory/AssetSearch/KeywordMetagroup'), self.getByLabelFunc('UI/Inventory/AssetSearch/DescriptionMetagroup'), self.localizationSortFunc([ metaGroups.get_name(groupID).lower() for groupID in const.metaGroupsUsed ]), self.MatchMetagroup),
    ...
    def GetMetaLevel(self, typeID):
        try:
            ...
            metaLevel = int(evetypes.GetMetaLevel(typeID) or 0)
    ...
    def GetMetaGroupName(self, metaGroupID):
        try:
            ...
            lower = metaGroups.get_name(metaGroupID).lower()
    ...
    def GetMetaGroupID(self, typeID):
        try:
            ...
            metaGroupID = int(evetypes.GetMetaGroupID(typeID) or 0)
    ...
    def GetTechLevel(self, typeID):
        try:
            ...
            techLevel = int(evetypes.GetTechLevel(typeID) or 1)
    ...
    def _GetSolarSystemID(self, stationID):
        try:
            ...
            stationData = self.uiSvc.GetStation(stationID)
            if stationData:
                solarSystemID = stationData.solarSystemID
            else:
                structureData = self.structureDirectorySvc.GetStructureInfo(stationID)
                solarSystemID = structureData.solarSystemID
