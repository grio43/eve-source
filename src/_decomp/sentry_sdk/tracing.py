#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\sentry_sdk\tracing.py
import re
import uuid
_traceparent_header_format_re = re.compile('^[ \t]*([0-9a-f]{2})-([0-9a-f]{32})-([0-9a-f]{16})-([0-9a-f]{2})(-.*)?[ \t]*$')
...
class SpanContext(object):
    ...
    def __repr__(self):
        return '%s(trace_id=%r, span_id=%r, recorded=%r)' % (self.__class__.__name__,
         self.trace_id,
         self.span_id,
         self.recorded)
    ...
    def start_trace(cls, recorded = False):
        return cls(trace_id=uuid.uuid4().hex, span_id=uuid.uuid4().hex[16:], recorded=recorded)

    def new_span(self):
        if self.trace_id is None:
            return SpanContext.start_trace()
        return SpanContext(trace_id=self.trace_id, span_id=uuid.uuid4().hex[16:], parent=self, recorded=self.recorded)
