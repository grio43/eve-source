#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\sentry_sdk\integrations\aws_lambda.py
import sys
from sentry_sdk.hub import Hub, _should_send_default_pii
from sentry_sdk._compat import reraise
from sentry_sdk.utils import AnnotatedValue, capture_internal_exceptions, event_from_exception, logger
    ...
    def sentry_handler(event, context, *args, **kwargs):
        ...
        with hub.push_scope() as scope:
            ...
            try:
                ...
                event, hint = event_from_exception(exc_info, client_options=hub.client.options, mechanism={'type': 'aws_lambda',
                 'handled': False})
...
class AwsLambdaIntegration(Integration):
    ...
    def setup_once():
        ...
        if not hasattr(lambda_bootstrap, 'handle_event_request'):
            logger.warning('Not running in AWS Lambda environment, AwsLambdaIntegration disabled')
            return
        if pre_37:
...
def _make_request_event_processor(aws_event, aws_context):
    ...
    def event_processor(event, hint):
        ...
        extra['lambda'] = {'remaining_time_in_millis': aws_context.get_remaining_time_in_millis(),
        ...
        if aws_event.get('body', None):
            request['data'] = AnnotatedValue('', {'rem': [['!raw',
                      'x',
                      0,
                      0]]})
