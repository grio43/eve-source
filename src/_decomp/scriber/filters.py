#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\scriber\filters.py
import datetime
import json
...
from scriber.esp_filters import *
...
def itemurl(item_model):
    try:
        return htmlutils.esp_item_link(item_model)
    except Exception:
        item_id = getattr(item_model, 'item_id', None)
        if item_id:
            return '/gm/inventory.py?action=Item&itemID=%d' % item_id
...
def floatformatm(value, precision = 2):
    str_value = floatformat(value)
    if precision and str_value:
        idx = precision + 1
        return '%s<span class="muted">%s</span>' % (str_value[:-idx], str_value[-idx:])
    return str_value
...
def qtable(data, dom_id = None, dom_classes = '+', dom_style = None, has_header = None, **kwargs):
    ...
    if isinstance(dom_classes, (list, tuple)):
        ...
        if dom_classes.startswith('+'):
            class_list.extend(widgets.QuickTable.DEFAULT_DOM_CLASSES)
            dom_classes = dom_classes[1:]
        if len(dom_classes) > 0:
            class_list.extend(dom_classes.strip().split(' '))
...
def joinlist(list_or_dict, format_str = '%s<br>', use_key_as_value = False):
    if list_or_dict:
        buff = []
        if isinstance(list_or_dict, dict):
            if use_key_as_value:
                list_or_dict = list_or_dict.keys()
            else:
                list_or_dict = list_or_dict.values()
        try:
            for i in list_or_dict:
                buff.append(format_str % i)
...
def ul(list_or_dict, format_str = '%s', use_key_as_value = False, **kwargs):
    items = joinlist(list_or_dict, '<li>%s</li>' % format_str, use_key_as_value)
    if items:
        if kwargs:
            attribs = ' %s' % ' '.join([ '%s="%s"' % (k, v) for k, v in kwargs.iteritems() ])
        else:
            attribs = ''
        return '<ul%s>\n\t%s\n</ul>\n' % (attribs, items)
    return ''
...
def ol(list_or_dict, format_str = '%s', use_key_as_value = False, **kwargs):
    items = joinlist(list_or_dict, '<li>%s</li>' % format_str, use_key_as_value)
    if items:
        if kwargs:
            attribs = ' %s' % ' '.join([ '%s="%s"' % (k, v) for k, v in kwargs.iteritems() ])
...
def parse_markdown(value):
    if value:
        return htmlutils.parse_markdown_link(value)
    return ''


def sanitize(value):
    if value:
        return htmlutils.sanitize(value, keep_amp=True)
    return ''
