#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\scriber\nom\__init__.py
import uuid
import json
...
class NoteOMatic(object):
    ...
    def render_zd(self):
        ...
        def markdown_hyperlinks(text):
            if isinstance(text, basestring) and re.match(scriber.NOTE_LINK_MARKDOWN_PATTERN, text):
                if ('http://' or 'https://') not in text:
                    return re.sub(scriber.NOTE_LINK_MARKDOWN_PATTERN, '[\\1](https://eveesp.ccpgames.com\\2)', text)
            return text
        ...
        context = {'ticket_id': None,
         ...
         'ticket_link': None,
         'reason_name': reason_name,
         'reason_value': markdown_hyperlinks(self._reason),
         'reason': 'Reason: %s (#%s)' % (reason_name, markdown_hyperlinks(self._reason)) if self._reason else None,
         'note': markdown_hyperlinks(self._note),
         'reference': self._reference,
         'user_id': self.user_id,
         'nl': '\n'}
        if self._params and isinstance(self._params, dict):
            for key, val in self._params.iteritems():
                context.update({key: markdown_hyperlinks(val)})
    ...
    def save(self, db2_service, gm_id, append_to_note_id = None):
        try:
            if self._copy_to_zendesk and self._ticket_id:
                self.cache = sm.GetService('cache')
                proxyMapStr = self.cache.Setting('charon', 'proxyMap')
                proxyMap = None
                if proxyMapStr and isinstance(proxyMapStr, (str, unicode)):
                    proxyMapStr = proxyMapStr.strip()
                    if proxyMapStr.startswith('{') and proxyMapStr.endswith('}'):
                        proxyMap = json.loads(proxyMapStr)
                try:
                    self.zendeskApi = zendeskapi.ZendeskAPI(self.cache.Setting('charon', 'zendeskApiRoot'), self.cache.Setting('charon', 'zendeskApiUsername'), self.cache.Setting('charon', 'zendeskApiKey'), proxyMap)
                    self.zendeskApi.create_comment(self._ticket_id, self.render_zd())
                except Exception as ex:
                    log.exception('Error while sending note to Zendesk: %r' % ex)
