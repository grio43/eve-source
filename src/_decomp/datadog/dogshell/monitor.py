#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\datadog\dogshell\monitor.py
...
class MonitorClient(object):
    ...
    def setup_parser(cls, subparsers):
        ...
        parser.add_argument('--string_ids', action='store_true', dest='string_ids', help='Represent monitor IDs as strings instead of ints in JSON')
        ...
        post_parser.add_argument('query', help='query to notify on with syntax varying depending on what type of monitor you are creating')
        post_parser.add_argument('--name', help='name of the alert', default=None)
        post_parser.add_argument('--message', help='message to include with notifications for this monitor', default=None)
        ...
        update_parser.add_argument('type', help="type of the monitor, e.g. 'metric alert' 'service check'")
        update_parser.add_argument('query', help='query to notify on with syntax varying depending on what type of monitor you are creating')
        update_parser.add_argument('--name', help='name of the alert', default=None)
        update_parser.add_argument('--message', help='message to include with notifications for this monitor', default=None)
        ...
        mute_parser.add_argument('--end', help='POSIX timestamp for when the mute should end (optional)', default=None)
    ...
    def _post(cls, args):
        ...
        res = api.Monitor.create(type=args.type, query=args.query, name=args.name, message=args.message, options=options)
        report_warnings(res)
        report_errors(res)
        if format == 'pretty':
