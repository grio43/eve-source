#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\datadog\dogshell\screenboard.py
import argparse
import sys
import platform
import webbrowser
...
class ScreenboardClient(object):
    ...
    def setup_parser(cls, subparsers):
        ...
        parser.add_argument('--string_ids', action='store_true', dest='string_ids', help='Represent screenboard IDs as strings instead of ints in JSON')
        ...
        push_parser = verb_parsers.add_parser('push', help='Push updates to screenboards from local files to the server')
        push_parser.add_argument('--append_auto_text', action='store_true', dest='append_auto_text', help='When pushing to the server, appends filename and timestamp to the end of the screenboard description')
        push_parser.add_argument('file', help='screenboard files to push to the server', nargs='+', type=argparse.FileType('r'))
        push_parser.set_defaults(func=cls._push)
        new_file_parser = verb_parsers.add_parser('new_file', help='Create a new screenboard and put its contents in a file')
    ...
    def _push(cls, args):
        ...
        for f in args.file:
            ...
            if args.append_auto_text:
                ...
                auto_text = '<br/>\nUpdated at {0} from {1} ({2}) on {3}'.format(datetime_str, f.name, screen_obj['id'], platform.node())
    ...
    def _post(cls, args):
        ...
        res = api.Screenboard.create(title=args.title, description=args.description, graphs=[graphs], template_variables=args.template_variables, width=args.width, height=args.height)
        report_warnings(res)
        report_errors(res)
        if format == 'pretty':
            print pretty_json(res)
    ...
    def _update(cls, args):
        ...
        res = api.Screenboard.update(args.screenboard_id, title=args.title, description=args.description, graphs=graphs, template_variables=args.template_variables, width=args.width, height=args.height)
        report_warnings(res)
        report_errors(res)
        if format == 'pretty':
            print pretty_json(res)
    ...
    def _new_file(cls, args):
        ...
        res = api.Screenboard.create(title=args.filename, description='Description for {0}'.format(args.filename), graphs=[graphs])
        report_warnings(res)
        report_errors(res)
