#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\datadog\dogshell\timeboard.py
import os.path
import platform
import sys
import webbrowser
...
class TimeboardClient(object):
    ...
    def setup_parser(cls, subparsers):
        ...
        parser.add_argument('--string_ids', action='store_true', dest='string_ids', help='Represent timeboard IDs as strings instead of ints in JSON')
        ...
        post_parser.add_argument('--template_variables', type=_template_variables, default=[], help="a json list of template variable dicts, e.g. [{'name': 'host', 'prefix': 'host', 'default': 'host:my-host'}]'")
        ...
        update_parser.add_argument('--template_variables', type=_template_variables, default=[], help="a json list of template variable dicts, e.g. [{'name': 'host', 'prefix': 'host', 'default': 'host:my-host'}]'")
        ...
        push_parser = verb_parsers.add_parser('push', help='Push updates to timeboards from local files to the server')
        push_parser.add_argument('--append_auto_text', action='store_true', dest='append_auto_text', help='When pushing to the server, appends filename and timestamp to the end of the timeboard description')
        push_parser.add_argument('file', help='timeboard files to push to the server', nargs='+', type=argparse.FileType('r'))
        push_parser.set_defaults(func=cls._push)
        new_file_parser = verb_parsers.add_parser('new_file', help='Create a new timeboard and put its contents in a file')
        new_file_parser.add_argument('filename', help='name of file to create with empty timeboard')
        new_file_parser.add_argument('graphs', help='graph definitions as a JSON string. if unset, reads from stdin.', nargs='?')
    ...
    def _pull_all(cls, args):
        ...
        for dash_summary in res['dashes']:
            ...
            cls._write_dash_to_file(dash_summary['id'], os.path.join(args.pull_dir, filename + '.json'), args.timeout, format, args.string_ids)
    ...
    def _new_file(cls, args):
        ...
        res = api.Timeboard.create(title=args.filename, description='Description for {0}'.format(args.filename), graphs=[graphs])
        report_warnings(res)
        report_errors(res)
    ...
    def _push(cls, args):
        ...
        for f in args.file:
            ...
            if args.append_auto_text:
                ...
                auto_text = '<br/>\nUpdated at {0} from {1} ({2}) on {3}'.format(datetime_str, f.name, dash_obj['id'], platform.node())
                dash_obj['description'] += auto_text
            tpl_vars = dash_obj.get('template_variables', [])
            if 'id' in dash_obj:
                dash_obj['id'] = int(dash_obj['id'])
                res = api.Timeboard.update(dash_obj['id'], title=dash_obj['title'], description=dash_obj['description'], graphs=dash_obj['graphs'], template_variables=tpl_vars)
            else:
                res = api.Timeboard.create(title=dash_obj['title'], description=dash_obj['description'], graphs=dash_obj['graphs'], template_variables=tpl_vars)
            if 'errors' in res:
    ...
    def _post(cls, args):
        ...
        res = api.Timeboard.create(title=args.title, description=args.description, graphs=[graphs], template_variables=args.template_variables)
        report_warnings(res)
        report_errors(res)
        if format == 'pretty':
            print pretty_json(res)
    ...
    def _update(cls, args):
        ...
        res = api.Timeboard.update(args.timeboard_id, title=args.title, description=args.description, graphs=graphs, template_variables=args.template_variables)
        report_warnings(res)
        report_errors(res)
        if format == 'pretty':
            print pretty_json(res)
