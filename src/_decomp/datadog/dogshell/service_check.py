#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\datadog\dogshell\service_check.py
...
class ServiceCheckClient(object):
    ...
    def setup_parser(cls, subparsers):
        ...
        check_parser.add_argument('status', help="integer for the status of the check. i.e: '0': OK, '1': WARNING, '2': CRITICAL, '3': UNKNOWN")
    ...
    def _check(cls, args):
        ...
        format = args.format
        res = api.ServiceCheck.check(check=args.check, host_name=args.host_name, status=int(args.status), timestamp=args.timestamp, message=args.message, tags=args.tags)
        report_warnings(res)
        report_errors(res)
        if format == 'pretty':
