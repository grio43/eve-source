#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\datadog\dogshell\__init__.py
import os
import argparse
...
def main():
    parser = argparse.ArgumentParser(description='Interact with the Datadog API', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--config', help='location of your dogrc file (default ~/.dogrc)', default=os.path.expanduser('~/.dogrc'))
    parser.add_argument('--api-key', help='your API key, from https://app.datadoghq.com/account/settings#api', dest='api_key', default=None)
    parser.add_argument('--application-key', help='your Application key, from https://app.datadoghq.com/account/settings#api', dest='app_key', default=None)
    parser.add_argument('--pretty', help='pretty-print output (suitable for human consumption, less useful for scripting)', dest='format', action='store_const', const='pretty')
    parser.add_argument('--raw', help='raw JSON as returned by the HTTP service', dest='format', action='store_const', const='raw')
    parser.add_argument('--timeout', help='time to wait in seconds before timing out an API call (default 10)', default=10, type=int)
    parser.add_argument('-v', '--version', help='Dog API version', action='version', version='%(prog)s {0}'.format(get_version()))
