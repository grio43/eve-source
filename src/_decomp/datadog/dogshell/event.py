#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\datadog\dogshell\event.py
import datetime
import time
import re
import sys
...
time_pat = re.compile('(?P<delta>[0-9]*\\.?[0-9]+)(?P<unit>[mhd])')
...
class EventClient(object):
    ...
    def setup_parser(cls, subparsers):
        ...
        post_parser.add_argument('--no_host', help='no host is associated with the event (overrides --host))', action='store_true')
        ...
        stream_parser = verb_parsers.add_parser('stream', help='Retrieve events from the Event Stream', description="Stream start and end times can be specified as either a POSIX timestamp (e.g. the output of `date +%s`) or as a period of time in the past (e.g. '5m', '6h', '3d').")
    ...
    def _post(cls, args):
        ...
        res = api.Event.create(title=args.title, text=message, handle=args.handle, priority=args.priority, related_event_id=args.related_event_id, tags=tags, host=host, device=args.device, aggregation_key=args.aggregation_key, source_type_name=args.type, alert_type=args.alert_type)
        report_warnings(res)
        report_errors(res)
        if format == 'pretty':
