#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\datadog\dogshell\downtime.py
...
class DowntimeClient(object):
    ...
    def setup_parser(cls, subparsers):
        ...
        parser.add_argument('--string_ids', action='store_true', dest='string_ids', help='Represent downtime IDs as strings instead of ints in JSON')
        ...
        post_parser.add_argument('--message', help='message to include with notifications for this downtime', default=None)
        ...
        update_parser.add_argument('--message', help='message to include with notifications for this downtime', default=None)
        ...
        show_all_parser.add_argument('--current_only', help='only return downtimes that are active when the request is made', default=None)
    ...
    def _update_downtime(cls, args):
        api._timeout = args.timeout
        format = args.format
        res = api.Downtime.update(args.downtime_id, scope=args.scope, start=args.start, end=args.end, message=args.message)
        report_warnings(res)
        report_errors(res)
        if format == 'pretty':
            print pretty_json(res)
