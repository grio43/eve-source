#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\brennivin\threadutils.py
...
class expiring_memoize(object):
    ...
    def __init__(self, expiry = 0, gettime = None, jitter = None):
        if jitter is not None:
            if jitter >= expiry:
                raise ValueError('Jitter must be < expiry')
        if expiry <= 0:
    ...
    def _get_next_expiry(self):
        from_timestamp = self.gettime()
        if self.jitter is None:
            return from_timestamp + self.expiry
        jitter_time = self.random.randint(-self.jitter, self.jitter)
        jittered_expiry = self.expiry + jitter_time
        return from_timestamp + jittered_expiry
    ...
    def __call__(self, func):
        ...
        def wrapped(*args):
            ...
            try:
                result, expiry_timestamp = cache[args]
                if self.gettime() < expiry_timestamp:
                    return result
            except KeyError:
                pass

            result = func(*args)
            cache[args] = (result, self._get_next_expiry())
            return result
