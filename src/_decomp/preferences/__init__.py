#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\preferences\__init__.py
import cPickle
import json
import logging
...
def RemoveValue(region, variable):
    if __ginstance__ is not None:
        __ginstance__.RemoveValue(region, variable)


def GetVariablesForRegion(region):
    if __ginstance__ is None:
        return []
    return __ginstance__.prefs.get(region, [])
