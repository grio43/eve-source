#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\jobboard\client\features\agent_missions\list_entry.py
...
class AgentMissionListEntry(JobListEntry):
    AGENT_PORTRAIT_SIZE = 28
    ...
    def _construct_left_content(self, parent):
        super(AgentMissionListEntry, self)._construct_left_content(parent)
        portrait_container = eveui.ContainerAutoSize(parent=parent, align=carbonui.Align.TORIGHT, width=self.AGENT_CONTAINER_SIZE, padRight=8)
        self._construct_state_container(portrait_container)
        self._construct_agent_portrait(portrait_container)

    def _construct_attention_icons(self, parent):
        super(AgentMissionListEntry, self)._construct_attention_icons(parent)
    ...
    def _construct_state_container(self, container_parent):
        self._state_container = eveui.Container(parent=container_parent, align=carbonui.Align.CENTER, width=self.AGENT_CONTAINER_SIZE, height=self.AGENT_CONTAINER_SIZE)
        icon_container = eveui.ContainerAutoSize(parent=self._state_container, state=eveui.State.normal, align=carbonui.Align.BOTTOMRIGHT)
        self._state_icon = eveui.Sprite(name='state_icon', parent=icon_container, state=eveui.State.normal, align=carbonui.Align.CENTER, width=self.STATUS_ICON_SIZE, height=self.STATUS_ICON_SIZE, opacity=0.8)

    def _construct_agent_portrait(self, container_parent):
        self.agent_portrait = eveui.CharacterPortrait(parent=container_parent, align=carbonui.Align.CENTER, size=self.AGENT_PORTRAIT_SIZE, character_id=self.job.agent_id, textureSecondaryPath='res:/UI/Texture/circle_full.png', spriteEffect=trinity.TR2_SFX_MODULATE)
        eveui.Sprite(parent=container_parent, align=carbonui.Align.CENTER, width=self.AGENT_CONTAINER_SIZE, height=self.AGENT_CONTAINER_SIZE, texturePath='res:/UI/Texture/circle_full.png', color=eveColor.BLACK, opacity=0.2)
    ...
    def _construct_time_remaining(self, parent):
        if not self.job.has_expiration_time():
            return
        time_remaining_icon = TimeRemainingIcon(parent=parent, job=self.job, padLeft=4, get_text=self.job.get_expiration_text, idx=0)
