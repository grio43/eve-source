#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\common\script\dogma\baseDogmaLocation.py
import math
import sys
import weakref
from collections import defaultdict
from dogma.const import dgmOperatorIDtoIndex
...
import uthread
import utillib
from carbon.common.script.sys.basesession import FindSessionClient
...
from dogma.items.fighterDogmaItem import FighterDogmaItem
from dogma.exceptions import EmbarkOnlineError, EffectFailedButShouldBeStopped
from dogma.attributes import BrainLiteralAttribute
from eve.common.script.dogma.effect import ApplyBrainEffect, RemoveBrainEffect
from eve.common.script.util.inventoryUtil import GetComputedBasePrice
from eve.common.script.util.industryCommon import CHARACTER_MODIFIERS
from eveexceptions import ExceptionEater, UserError
from inventorycommon.const import categoryShip, categoryStructure, serviceSlotFlags, DEPRECATED_POS_MODULE_GROUPS
from inventorycommon.util import IsShipFittingFlag, IsFittingModule, IsSubsystemFlag
...
opDecoder = {None: '..',
 ...
 const.dgmAssSpecialSkillRelationship: '.!',
 const.dgmAssPostPercentUnNerfed: '.%'}
CATEGORY_SHIPS_AND_STRUCTURES = {categoryShip, categoryStructure}
PROBE_GROUPS = (const.groupSurveyProbe,
 const.groupWarpDisruptionProbe,
 const.groupBurstEffectProbe,
...
class BaseDogmaLocation(object):
    ...
    def __init__(self, broker):
        ...
        self.activeDungeonEffectsByItem = defaultdict(lambda : defaultdict(set))
        self.instanceFlagQuantityCache = {}
        self.unloadingItems = set()
        self.extraTracebacks = True
        self.pendingPowerCpuRequirementChecks = set()
        ...
        self.RegisterCallback(const.attributeMaxGroupOnline, self.OnMaxGroupOnlineChanged)
        self.msgMgr = MessageMgr()
        self.brainLiterals = keydefaultdict(BrainLiteralAttribute)
        self.scarecrows = {}
        self.brainUpdate = BrainUpdate(self.CheckPowerCpuRequirementForOnlineModules)

    def LoadItem(self, itemKey, instanceRow = None, wait = False, invItem = None, newInstance = False, timerName = None):
    ...
    def ApplyBrainEffects(self, shipID, charID, timerName = 'unknown'):
        ...
        self._ApplyPilotSecurityStatusToShip(shipID, charID)
        endTime = blue.os.GetWallclockTimeNow()
        self.broker.LogInfo('%s - ApplyBrainEffects took %s ms' % (self.locationID, blue.os.TimeDiffInMs(startTime, endTime)))
    ...
    def RemoveBrainEffects(self, shipID, charID, timerName = None):
        ...
        if shipID is not None:
            ...
            self._ApplyPilotSecurityStatusToShip(shipID, None)
    ...
    def OnCharacterEmbarkation(self, charID, shipID, switching = False, raiseOnOnlineFailure = False):
        ...
        with ExceptionEater('OnCharacterEmbarkation %s %s' % (charID, shipID)):
            shipItem = self.GetItem(shipID)
            if shipItem.ownerID != charID and shipItem.categoryID != categoryStructure:
        ...
        for effectID in self.dogmaStaticMgr.GetPassiveFilteredEffectsByType(itemTypeID):
            if effectID not in aeff:
                ...
                if otherID is None and self.effectCompiler.IsEffectDomainOther(effectID):
                    continue
                self.LogInfo('InstallPilotForItem.effect', itemKey, effectID, dogma_data.get_effect_name(effectID))
                try:
    ...
    def FitItemToLocation(self, locationID, itemID, flagID):
        ...
        if not locationDogmaItem.CanFitItem(dogmaItem, flagID):
            if locationDogmaItem.categoryID == categoryShip:
        ...
        if startedEffects is not None:
            ...
            if IsSubsystemFlag(dogmaItem.flagID):
        ...
        if locationCategoryID in CATEGORY_SHIPS_AND_STRUCTURES and dogmaItem.attributes.get(const.attributeIsOnline, False) and self.dogmaStaticMgr.TypeHasEffect(dogmaItem.typeID, const.effectOnline):
    ...
    def _StartModuleOnlineEffect(self, itemID, pilotID, locationID, context = None, raiseUserError = False):
        environment = self.GetEnvironment(self.dogmaItems[itemID].GetEnvironmentInfo(), const.effectOnline)
        ...
        for effectID in self.dogmaStaticMgr.TypeGetOrderedEffectIDs(itemTypeID):
            ...
            if effect.effectCategory in [const.dgmEffPassive, const.dgmEffSystem]:
                try:
                    ...
                    for _effectID in effectsCompleted:
                        try:
                            info = dogmaItem.GetEnvironmentInfo()
                            env = self.GetEnvironment(info, _effectID)
                            self.StartEffect(_effectID, itemID, env)
                        except UserError as e:
                            log.LogException(channel='svc.dogmaIM')
                            sys.exc_clear()
    ...
    def StartEffect(self, effectID, itemKey, environment, repeat = 0, byUser = 0, checksOnly = None):
        ...
        if effectID not in self.dogmaStaticMgr.effectsByType.get(dogmaItem.typeID, []):
            if effectID == const.effectOnline:
                ...
                if evetypes.GetGroupID(dogmaItem.typeID) in DEPRECATED_POS_MODULE_GROUPS:
                    raise TypeError('DeprecatedPosModule')
            return
    ...
    def _StartEffect(self, effect, dogmaItem, environment, duration, repeat, byUser, checksOnly):
        ...
        if effectID == const.effectOnline:
            for onlineEffectID in self.dogmaStaticMgr.effectsByType[dogmaItem.typeID]:
                if self.dogmaStaticMgr.effects[onlineEffectID].effectCategory == const.dgmEffOnline:
                    self.effectCompiler.PreStartEffectChecks(onlineEffectID, environment)
        ...
        if checksOnly is None:
            ...
            if flag is not None:
                checksOnly = not flag
        self.PossiblyStartOverloadingEffects(effect.effectCategory, effectID, environment.shipID, itemKey, dogmaItem.typeID, environment)
        if checksOnly:
    ...
    def PossiblyStartOverloadingEffects(self, *args):
        pass
    ...
    def UnregisterEffect(self, effect, dogmaItem, environment):
        ...
        try:
            if shipID in self.activeEffectsByShip:
                if effectID in self.activeEffectsByShip[shipID]:
    ...
    def GetMissingSkills(self, typeID, currentSkills):
        ret = {}
        for skillID, skillLevel in self.dogmaStaticMgr.GetRequiredSkills(typeID).iteritems():
            if skillID not in currentSkills or currentSkills[skillID].effectiveSkillLevel < skillLevel:
    ...
    def GetClassForItem(self, item):
        if item.categoryID == categoryShip:
            return ShipDogmaItem
        elif item.categoryID == categoryStructure:
        ...
        elif item.groupID in PROBE_GROUPS:
    ...
    def IsFighterOwnerModifiablesAtLocation(self, locationID):
        return True
    ...
    def OnFittingChanged(self, itemID, loadAttributeID, outputAttributeID):
        ...
        if output - load < 1e-07:
            self.CheckPowerCpuRequirementForOnlineModules(itemID)

    def ScheduleCheckPowerCpuRequirementForOnlineModules(self, shipID):
        if shipID not in self.pendingPowerCpuRequirementChecks:
            self.pendingPowerCpuRequirementChecks.add(shipID)
            uthread.pool('DogmaLocation::CheckPowerCpuRequirementForOnlineModules', self.CheckPowerCpuRequirementForOnlineModules, shipID)

    def CheckPowerCpuRequirementForOnlineModules(self, shipID):
        if self.brainUpdate.IsEventHappening(shipID):
            return
        self.pendingPowerCpuRequirementChecks.discard(shipID)
        ...
        if not self.IsShipLoaded(shipID):
            return
        if self.IsItemUnloading(shipID):
            return
        dogmaItem = self.dogmaItems[shipID]
        if getattr(dogmaItem, 'isBeingDisembarked', False):
            return
        if dogmaItem.categoryID not in CATEGORY_SHIPS_AND_STRUCTURES:
            return
        ...
        if cpuUsage <= 1.0 and powerUsage <= 1.0:
            return
        resourceAttributeID = const.attributePower if powerUsage > cpuUsage else const.attributeCpu
        self._OfflineModuleUsingMostResource(shipID, dogmaItem, resourceAttributeID)

    def _OfflineModuleUsingMostResource(self, shipID, dogmaItem, resourceAttributeID):
        if resourceAttributeID not in (const.attributePower, const.attributeCpu):
            raise ValueError('_OfflineModuleUsingMostResource called with invalid resourceAttributeID')
        if not self.IsShipLoaded(shipID):
            return
        candidates = []
        for moduleID, moduleDogmaItem in dogmaItem.GetFittedItems().iteritems():
            ...
            if not moduleDogmaItem.IsOnline():
                continue
            resourceUsed = self.GetAttributeValue(moduleID, resourceAttributeID)
            if resourceUsed:
                if moduleDogmaItem.flagID in serviceSlotFlags:
                    sortKey = (0, resourceUsed)
                else:
                    sortKey = (1, resourceUsed)
                candidates.append((sortKey, moduleID))

        if len(candidates) == 0:
            self.broker.LogError('The ship', shipID, 'has something modifying resource attribute', resourceAttributeID, "that isn't an online effect")
            return
        itemKey = max(candidates)[1]
        if (itemKey, const.effectOnline) not in self.deactivatingEffects:
            self.OfflineModule(itemKey)

    def OnMaxGroupOnlineChanged(self, attributeID, itemID, newValue, oldValue = None):
        pass
    ...
    def GetIndustryCharacterModifiers(self, characterID):
        if not self.IsItemLoaded(characterID):
            raise RuntimeError('Characters must be loaded before you can query about their industry attributes.  Try again later.')
        return {attributeID:self.GetAttributeValue(characterID, attributeID) for attributeID in (m[1] for m in CHARACTER_MODIFIERS)}
    ...
    def ItemHasAttribute(self, itemID, attributeID):
        return itemID in self.dogmaItems and attributeID in self.dogmaItems[itemID].attributes
    ...
    def IsShipLoaded(self, shipID):
        ...
        if dogmaItem is None:
            ...
            return dogmaItem.categoryID in CATEGORY_SHIPS_AND_STRUCTURES
    ...
    def _RemoveAndCleanup(self, theDict, index, toRemove, debugStr = '<none>'):
        try:
            ...
            try:
                ...
                LogWarn('_RemoveAndCleanup: item `toRemove` not found in `theDict` at the given `index`')

            if not theDict[index]:
                del theDict[index]
        except KeyError:
            LogWarn('_RemoveAndCleanup: no innerContainer found at the given `index`')
    ...
    def FullyDescribeAttribute(self, itemID, attributeID, reason, force = 0):
        ...
        rep = ['Full description requested for attribute %s on item %s, reason: %s' % (attribute.name, self.GetLocName(itemID), reason)]
    ...
    def _PickShipOrStructureEffects(self, shipEffects, structureEffects, categoryId):
        if categoryId == categoryShip:
            return shipEffects
        return structureEffects

    def _GetValidAttributes(self, shipTypeId):
        validAttributes = {x.attributeID for x in dogma_data.get_type_attributes(shipTypeId)}
    ...
    def IsItemIdStructure(self, itemId):
        ...
        return shipDogmaItem and shipDogmaItem.categoryID == categoryStructure

    def UpdateStructureServices(self, structureID):
        pass

    def UninstallPilotFromShipAndItems(self, oldShipID):
        dogmaItem = self.dogmaItems.get(oldShipID)
        if dogmaItem is None:
            return
        fittedItems = dogmaItem.GetFittedItems()
        for itemKey in fittedItems:
            self.UninstallPilotForItem(itemKey)

    def UninstallPilotForItem(self, itemKey):
        dogmaItem = self.dogmaItems[itemKey]
        for effectID in dogmaItem.activeEffects.keys():
            effect = self.dogmaStaticMgr.effects[effectID]
            if effect.effectCategory == const.dgmEffPassive and self.effectCompiler.IsEffectCharacterModifier(effectID):
                self.LogInfo('UninstallPilotForItem.effect', itemKey, effectID, dogma_data.get_effect_name(effectID))
                self.StopEffect(effectID, itemKey, False)

        if dogmaItem.categoryID == const.categoryDrone and dogmaItem.IsOwnerModifiable():
            dogmaItem.RemoveDroneOwnerModifiers()

    def GetCharacterSecurityStatus(self, charID):
        return 0.0

    def _ApplyPilotSecurityStatusToShip(self, shipID, pilotID):
        if pilotID:
            secStatus = self.GetCharacterSecurityStatus(pilotID)
        else:
            secStatus = 0.0
        self.SetAttributeValue(shipID, const.attributePilotSecurityStatus, secStatus)

    def FindNumUnitsToRepairModule(self, _charid, dmg, hp, itemTypeID, itemID):
        consumptionTypeID = const.consumptionTypeIDForModuleRepair
        basePrice = GetComputedBasePrice(itemTypeID, 1)
        if basePrice == 0.0:
            self.SetAttributeValue(itemID, const.attributeDamage, 0)
            return None
        unitPrice = basePrice / hp
        costToRepair = dmg * unitPrice * self.GetAttributeValue(_charid, const.attributeShipBrokenModuleRepairCostMultiplier)
        unitsConsumed = math.ceil(costToRepair / evetypes.GetBasePrice(consumptionTypeID))
    ...
    def IsAttributeInIcomingModifiers(self, itemID, toAttributeID, attributeToCheck, operation):
        dogmaItem = self.GetItem(itemID)
        if not dogmaItem:
            return False
        toAttrib = dogmaItem.attributes.get(toAttributeID, None)
        if not toAttrib:
            return False
        attributesForOp = toAttrib.incomingModifiers[dgmOperatorIDtoIndex[operation]]
        return attributeToCheck in attributesForOp
