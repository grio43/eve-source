#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\common\script\paperDoll\paperDollDataManagement.py
import os
import uthread
import copy
import types
...
MAP_CACHE_PATH = u'cache:/Avatars/cachedMaps'
...
def GetCacheFilePath(cachePath, hashKey, textureWidth, mapType):
    ...
    return u'{0}/{1}/{2}{3}.dds'.format(cachePath, textureWidth, mapType, hashKey)


def FindCachedMap(hashKey, textureWidth, mapType):
    resPath = GetCacheFilePath(MAP_CACHE_PATH, hashKey, textureWidth, mapType)
    if blue.paths.exists(resPath):
        rotPath = blue.paths.ResolvePath(resPath)
        os.utime(rotPath, None)
        cachedTexture = blue.resMan.GetResource(resPath)
...
def SaveMapsToDisk(hashKey, maps):
    for i, each in enumerate(maps):
        if each is not None:
            ...
            filePath = blue.paths.ResolvePathForWriting(GetCacheFilePath(MAP_CACHE_PATH, hashKey, textureWidth, i))
            if filePath:
