#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\common\script\planet\baseColony.py
import math
import types
import weakref
import blue
...
from fsdBuiltData.common.planet import schematic_exists
...
class BaseColony(object):
    ...
    def RegisterTemporaryID(self, oldID, newID):
        ...
        self.colonyData.RegisterTemporaryID(oldID, newID)
    ...
    def ClearTemporaryIDs(self):
        self.temporaryIDMap = {}
        self.colonyData.ClearTemporaryIDs()

    def ReplaceTemporaryIDsInPath(self, path):
        newPath = []
        for waypoint in path:
            realID = self.GetRealIDFromTemporaryID(waypoint)
            newPath.append(realID if realID else waypoint)
    ...
    def GetPin(self, pinID):
        if self.colonyData is None:
            return
        pinData = self.colonyData.GetPin(pinID)
        if pinData is None:
            possiblePinID = self.GetRealIDFromTemporaryID(pinID)
            pinData = self.colonyData.GetPin(possiblePinID)
        return pinData
            ...
            with bluepy.Timer('BaseColony::RunSimulation'):
                while len(self.simQueue) > 0:
                    ...
                    if simPin is None:
                        ...
                        continue
                    if simPin.IsExtractor() and (simPin.GetCycleTime() is None or simPin.GetCycleTime() <= 0):
                        break
                    with bluepy.TimerPush('EvaluatePin::' + simPin.__class__.__name__):
    ...
    def EvaluatePin(self, pin):
        ...
        with bluepy.TimerPush('Run'):
            commodities = self.GetCommoditiesProducedByPin(pin)
        if pin.IsConsumer():
        ...
        with bluepy.TimerPush('RouteCommodityOutput'):
            self.RouteCommodityOutput(pin, commodities)
        self.LogInfo('EvaluatePin ::', pin.id, 'Done')

    def GetCommoditiesProducedByPin(self, pin):
        return pin.Run(self.currentSimTime)
    ...
    def ValidateInstallSchematic(self, charID, pinID, schematicID):
        ...
        if not schematic_exists(schematicID):
    ...
    def ValidateAddExtractorHead(self, pinID, headID, latitude, longitude):
        ...
        if pin.IsActive():
            raise RuntimeError('You cannot add an extractor head while program is active')
        ...
        if angleBetween > areaOfInfluence:
            raise UserError('CannotPlaceHeadTooFarAway', {'maxDist': FmtDist(areaOfInfluence * self.planet.radius)})
    ...
    def ValidateRemoveExtractorHead(self, pinID, headID):
        ...
        if pin.IsActive():
            raise RuntimeError('You cannot remove an extractor head while program is active:pinID:{}/headID:{}'.format(pinID, headID))
    ...
    def ValidateMoveExtractorHead(self, pinID, headID, latitude, longitude):
        ...
        areaOfInfluence = pin.GetAreaOfInfluence()
        if angleBetween > areaOfInfluence:
            raise UserError('CannotPlaceHeadTooFarAway', {'maxDist': FmtDist(angleBetween * self.planet.radius)})
        ...
        if self.colonyData is not None:
            for pin in self.colonyData.pins.itervalues():
                if not pin.IsCommandCenter() and pin.IsSpaceport():
                    endpoints.append(utillib.KeyVal(id=pin.id, capacity=pin.GetCapacity(), capacityUsed=pin.capacityUsed))
    ...
    def CreateProgram(self, harmonic, ecuPinID, resourceTypeID, points = None, headRadius = None):
        ...
        with bluepy.TimerPush('OverlapOthersHeads'):
            otherHeadsInfo = []
            for pin in self.colonyData.GetECUPins():
                if pin.id == ecuPinID:
                    continue
            ...
            for index, surfacePoint1 in heads:
                ...
                for otherHeadRadius, otherHeads in otherHeadsInfo:
                    ...
                    for index2, longitude2, latitude2 in otherHeads:
                        ...
                        d = 0.0001 if d == 0.0 else d
                        R = RADIUS_DRILLAREAMAX if R == 0.0 else R
                        r = RADIUS_DRILLAREAMAX if r == 0.0 else r
                        if d > R + r:
                            continue
