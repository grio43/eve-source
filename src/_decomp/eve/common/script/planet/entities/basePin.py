#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\common\script\planet\entities\basePin.py
import math
import weakref
import blue
import evetypes
...
class BasePin(object):
    ...
    def CanAccept(self, typeID, quantity):
        ...
        if self.GetCapacity() is not None:
            ...
            capacityRemaining = self.GetCapacityRemaining()
    ...
    def GetTotalAmountProduced(self):
        return sum(self.GetProductMaxOutput().values())

    def Serialize(self, full = False):
        data = utillib.KeyVal(id=self.id, latitude=self.latitude, longitude=self.longitude, ownerID=self.ownerID, lastRunTime=self.lastRunTime, typeID=self.typeID, contents=self.contents.copy(), state=self.activityState)
        return data
