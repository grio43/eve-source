#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\spacecomponents\activatecontroller.py
...
class ActivateCounterController(BaseCounterController):
    ...
    def UpdateTimerState(self, instance, slimItem):
        if slimItem.component_activate is not None:
            ...
            elif instance.activeTimestamp is None:
                self.RemoveTimer()
            elif self.timer is None:
                logger.debug('%s Adding timer for %s', self.__class__.__name__, slimItem.itemID)
                seconds = instance.GetDurationSeconds()
                self.AddTimer(instance.activeTimestamp, seconds)
