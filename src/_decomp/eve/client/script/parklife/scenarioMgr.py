#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\parklife\scenarioMgr.py
import random
import sys
import math
import blue
...
from carbon.common.script.sys.serviceConst import ROLE_CONTENT, ROLE_SERVICE
from carbonui import uiconst
from carbonui.uicore import uicore
from eve.client.script.parklife import dungeonEditorTools
from eve.client.script.parklife import dungeonHelper
...
class ScenarioMgr(Service):
    __guid__ = 'svc.scenario'
    __exportedcalls__ = {'BindClientToLevelEditor': [ROLE_SERVICE],
     'ChoosePathStep': [ROLE_SERVICE]}
    ...
    def __init__(self):
        super(ScenarioMgr, self).__init__()
        self.selection = []
        self.selectionObjs = []
    ...
    def Run(self, memStream = None):
        super(ScenarioMgr, self).Run(memStream)
        self.ed = None

    def GetLevelEditor(self):
        if self.ed is None and session.role & ROLE_CONTENT == ROLE_CONTENT:
    ...
    def Stop(self, stream):
        self.HideCursor()
        super(ScenarioMgr, self).Stop(stream)
    ...
    def ReplaceObjectBall(self, ballID, slimItem):
        ...
        if targetModel and targetModel.rotationCurve and hasattr(targetModel.rotationCurve, 'value'):
            ...
            tf.rotationCurve = trinity.Tr2RotationAdapter()
    ...
    def AddBoundingCube(self, ball, model, scale = 1.0, rotation = geo2.Vector(0, 0, 0, 1)):
        ...
        transform.rotationCurve = trinity.Tr2RotationAdapter()
    ...
    def DoBallsRemove(self, pythonBalls, isRelease):
        ...
        for ball, slimItem, terminal in pythonBalls:
            if ball and ball.id in self.selection:
