#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\parklife\fightersSvc.py
...
class FightersSvc(Service):
    ...
    def ToggleSelectionForFightersForTube(self, tubeFlagID):
        fighters = self.shipFighterState.GetFightersForTubeID(tubeFlagID)
        if fighters:
            movementFunctions.ToggleSelectForNavigation(fighters.itemID)

    def LaunchSelectedFighters(self):
        tubeIDsToLaunch = movementFunctions.GetSelectedFightersByTubeIDs().keys()
        if tubeIDsToLaunch:
            self.LaunchFightersFromTubes(tubeIDsToLaunch)

    def RecallSelectedFightersToTubes(self):
        fightersToRecall = movementFunctions.GetSelectedFightersByTubeIDs().values()
        self.RecallFightersToTubes(fightersToRecall)

    def SelectAllFighters(self):
        fightersToSelect = self.shipFighterState.GetAllFightersIDs()
        for eachFighterID in fightersToSelect:
            movementFunctions.SelectForNavigation(eachFighterID)

    def DeselectAllFighters(self):
        fightersToDeselect = self.shipFighterState.GetAllFightersIDs()
        for eachFighterID in fightersToDeselect:
            movementFunctions.DeselectForNavigation(eachFighterID)
