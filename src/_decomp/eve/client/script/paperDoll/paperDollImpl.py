#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\paperDoll\paperDollImpl.py
import datetime
import os
import sys
...
from eve.client.script.paperDoll import renderDrivers
from eve.client.script.paperDoll import paperDollStatistics as statistics
...
class UVCalculator(object):
    ...
    def GetMapSize(modifier):
        ...
        if path:
            ...
            timeout_t = starttime + datetime.timedelta(seconds=10)
            while not (param.resource.isPrepared and param.resource.isGood) and datetime.datetime.now() < timeout_t:
                pdCf.Yield()
    ...
    def ApplyUVs(buildDataManager, renderDriver):
        ...
        for modifier in modifierGenerator:
            ...
            for mesh in iter(meshes):
                ...
                for fx in iter(fxs):
                    if fx.effectResource:
                        starttime = datetime.datetime.now()
                        timeout_t = starttime + datetime.timedelta(seconds=60)
                        while not (fx.effectResource.isPrepared and fx.effectResource.isGood) and datetime.datetime.now() < timeout_t:
                            pdCf.Yield()
...
class UpdateRuleBundle(object):
    ...
    def __init__(self):
        ...
        self.textureLoadingFailure = False
...
class Factory(object, CompressionSettings, pdDm.ModifierLoader):
    ...
    def FactoryReady(self):
        if self.shaderPreload is None:
            return False
        if self.texturePreload is None:
            return False
        if not all([ hasattr(effect.effectResource, 'isGood') for effect in self.shaderPreload ]):
            return False
        if not all([ hasattr(tex.resource, 'isGood') for tex in self.texturePreload ]):
            return False
        return self.IsLoaded and self.HeadsLoaded and all([ effect.effectResource.isGood for effect in self.shaderPreload ]) and all([ tex.resource.isGood for tex in self.texturePreload ])
    ...
    def ProcessBlendshapesForCloth(clothMeshes, morphTargets, clothMorphValues):
        ...
        pdCf.WaitForAll(clothMeshes, lambda x: not x.morphRes.isPrepared)
        for mesh in clothMeshes:
            if not mesh.clothingRes.isGood:
                raise ValueError('Paperdoll::Clothing mesh failed to load.')
    ...
    @telemetry.ZONE_METHOD
    def CreateGrannyAnimation(self, avatar, grannyFile):
        import trinity
        remotefilecache.prefetch_folder(os.path.dirname(grannyFile))
        animation = trinity.Tr2GrannyAnimation()
        animation.resPath = grannyFile
        trinity.WaitForResourceLoads()
        animation.animationEnabled = True
        clip = animation.grannyRes.GetAnimationName(0)
        animation.PlayAnimationEx(clip, 0, 0, 1)
    ...
    def CompositeCombinedTexture(self, mapType, gender, buildDataManager, mapBundle, textureCompositor, compressionSettings = None, partsToComposite = None, copyToExistingTexture = False, lod = None):
        ...
        try:
            ...
            if copyToExistingTexture and mapBundle[mapType] and mapBundle[mapType].isGood:
                tex = textureCompositor.Finalize(textureToCopyTo=mapBundle[mapType], compressionSettings=compressionSettings, mapType=mapType)
            else:
                tex = textureCompositor.Finalize(compressionSettings=compressionSettings, mapType=mapType)
            textureCompositor.renderTarget = None
            return tex
        except (trinity.ALOutOfMemoryError, tc.TextureLoadingError):
            raise
        except TaskletExit:
    ...
    def GetDNAFromYamlFile(self, filename):
        if blue.paths.exists(filename):
            fileStream = blue.paths.GetFileContentsWithYield(filename)
            yamlStr = fileStream.Read()
            if len(yamlStr) == 0:
...
class TextureLoadingFailureException(Exception):
    pass
...
class Doll(object):
    ...
    def OnCreate(self, dev):
        if self.currentLOD == pdDef.LOD_SCATTER_SKIN:
    ...
    def MatchDNA(self, factory, dna):
        ...
        for modifier in self.buildDataManager.GetSortedModifiers():
            parent_modifiers = self.buildDataManager.GetParentModifiers(modifier)
            for par_mod in parent_modifiers:
                self.buildDataManager.RemoveModifier(par_mod)
    ...
    def AddItemType(self, factory, itemType, weight = 1.0, rawColorVariation = None):
        ...
        if type(itemType) is tuple:
            return self.AddResource(itemType[0].lower(), weight, factory, variation=itemType[1], colorVariation=itemType[2], rawColorVariation=rawColorVariation)
    ...
    def IsReady(self):
        return not self.IsBusyUpdating() and self.hasUpdatedOnce
    ...
    def Update_t(self, factory, avatar, visualModel, channel = None):
        ...
        try:
            while self.busyLoadingDNA or not factory.FactoryReady():
                pdCf.Yield()
            ...
            if not (updateRuleBundle.blendShapesOnly or updateRuleBundle.decalsOnly):
                self._ProcessRules(factory, buildDataManager, addedAndChangedModifiers, updateRuleBundle)
            ...
            if visualModel and (self.currentLOD <= self.previousLOD or not self.mapBundle.AllMapsGood()):
                ...
                if updateRuleBundle.textureLoadingFailure:
                    raise TextureLoadingFailureException('Texture failed to load!')
            ...
            self.useDXT5N = compSet and compSet.compressTextures and compSet.AllowCompress(pdDef.NORMAL_MAP)
            meshes = buildDataManager.GetMeshes()
            if updateRuleBundle.meshesChanged:
                self._HandleBlendShapes(removedModifiers)
            self.WaitForChildTaskletsToFinish()
            if updateRuleBundle.meshesChanged:
                ...
                if visualModel:
                    ...
                    visualModel.ResetAnimationBindings()
            Factory.BindMapsToMeshes(meshes, self.mapBundle)
            ...
            self.lastUpdateRedundant = True
            raise
        except (OutOfVideoMemoryException, TextureLoadingFailureException) as err:
            self.lastUpdateRedundant = True
            newSize = map(lambda x: x / 2, self.mapBundle.baseResolution)
            if trinity.device.scene and trinity.device.scene.shadowSize > newSize[0]:
                trinity.device.scene.shadowSize /= 2
            self.hasUpdatedOnce = False
            ...
            if not updateRuleBundle.videoMemoryFailure and not updateRuleBundle.textureLoadingFailure:
                uthread.new(self.NotifyUpdateDoneListeners_t)
                uthread.new(self.NotifyUpdateDoneCallbacks_t)
            self._currentUpdateTasklet = None
            if updateRuleBundle.videoMemoryFailure or updateRuleBundle.textureLoadingFailure:
    ...
    def NotifyUpdateDoneCallbacks_t(self):
        for callback in self.onUpdateDoneCallbacks[:]:
            uthread.new(callback, self)
            self.onUpdateDoneCallbacks.remove(callback)

    def AddUpdateDoneCallback(self, callBack):
        self.onUpdateDoneCallbacks.append(callBack)
            ...
            for p in f.parameters:
                if p.name == 'CutMaskInfluence':
                    if remove:
                        ...
                        p.value = 0.0
                    elif abs(p.value - 0.85) > 0.001 and abs(p.value - 0.999) > 0.001:
                        p.value = 1.01
    ...
    def CompositeTextures(self, factory, hashKey, updateRuleBundle):
        if not updateRuleBundle.blendShapesOnly and factory.allowTextureCache:
            ...
            pdCf.WaitForAll(textureCache.values(), lambda x: x is not None and not x.isPrepared)
            for i in xrange(mapCount):
                if textureCache.get(i):
                    if textureCache[i].isGood:
                        self.mapBundle.SetMapByTypeIndex(i, textureCache[i], hashKey)
                    else:
                        updateRuleBundle.textureLoadingFailure = True
                        log.LogError('Paperdoll::Texture failed to load.  Index %d' % i)
                        raise TextureLoadingFailureException('Paperdoll::Texture failed to load.  Index %d' % i)
        ...
        def CompositeTasklet_t(mapTypeIndex, copyToExistingTexture):
            ...
            try:
                ...
                info = sys.exc_info()
                if info[0] == trinity.ALOutOfMemoryError:
    ...
    def LoadClothData(self, modifierList):
        ...
        pdCf.WaitForAll(clothMeshes, lambda x: not x.clothingRes.isPrepared)
        for mesh in clothMeshes:
            if not mesh.clothingRes.isGood:
                raise ValueError('Paperdoll::Clothing mesh failed to load.')
