#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\paperDoll\paperDollPortrait.py
import math
import datetime
import os
import geo2
...
def _GetFaceSetup(gender):
    if gender in _FACE_SETUP:
        return _FACE_SETUP[gender]
    rootFolder = 'R:/'
    ...
    if blue.paths.exists(faceSetupPath):
        data = pdDm.LoadYamlFileNicely(faceSetupPath)
    else:
        data = None
    _FACE_SETUP[gender] = data


def _GetFaceTweakData():
    global _FACE_TWEAK
    if _FACE_TWEAK:
        return _FACE_TWEAK
    rootFolder = 'R:/'
    ...
    _FACE_TWEAK = pdDm.LoadYamlFileNicely(faceTweaksPath)
    return _FACE_TWEAK
...
class PortraitTools(object):
    ...
    def SetupWrinkleMapControls(self, avatar, effects, pdDoll):
        ...
        data = _GetFaceSetup(pdDoll.gender)
        if not data:
            return
        tweakData = _GetFaceTweakData()
        curveDictionary = {}
        for d in data:
            entry = data[d]
            if len(entry) == 7:
        ...
        for mesh in stubbleMeshes:
            if adding:
                if any(map(lambda x: 'stubble' in x.name.lower(), mesh.decalAreas)):
                    ret.append(mesh)
                    continue
                c_skin_opaque_areas = (area for area in mesh.opaqueAreas if area.effect.name.lower().startswith('c_skin_'))
                c_skin_decal_areas = (area for area in mesh.decalAreas if area.effect.name.lower().startswith('c_skin_'))
                c_skin_areas = chain(c_skin_opaque_areas, c_skin_decal_areas)
                for area in c_skin_areas:
                    ...
                    blue.paths.GetFileContentsWithYield(stubblePath)
                    ...
                    for resource in stubbleArea.effect.resources:
                        if resource.name == 'LengthTexture':
                            ...
                            timeout = starttime + datetime.timedelta(seconds=10)
                            while not (resource.resource.isPrepared and resource.resource.isGood) and datetime.datetime.now() < timeout:
                                pdCf.Yield()

                            if not resource.resource.IsGood():
                                raise ValueError('Paperdoll::Stubble texture failed to load.')
