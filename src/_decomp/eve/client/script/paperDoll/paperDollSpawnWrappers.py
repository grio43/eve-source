#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\paperDoll\paperDollSpawnWrappers.py
import trinity
import uthread
...
class PaperDollManager(object):
    ...
    def SpawnPaperDollCharacterFromDNA(self, scene, dollName, dollDNA, position = None, rotation = None, lodEnabled = False, compressionSettings = None, gender = pdDef.GENDER.FEMALE, textureResolution = None, updateDoll = True, spawnAtLOD = 0):
        ...
        if lodEnabled:
            pdc.SpawnLOD(scene, point=position, rotation=rotation, gender=gender)
        else:
            pdc.Spawn(scene, point=position, rotation=rotation, gender=gender, updateDoll=updateDoll, lod=spawnAtLOD)
...
class PaperDollCharacter(object):
    ...
    def __PrepareAvatar(self, scene, point = None, rotation = None, name = None):
        if scene is None:
    ...
    def Spawn(self, scene, **kwargs):
        ...
        self.__PrepareAvatar(scene, point=kwargs.get('point'), rotation=kwargs.get('rotation'), name=kwargs.get('name', None))
        self.scene = scene
    ...
    def MoveToScene(self, scene, point = None):
        if scene and self.doll:
            self.Spawn(scene, point=point, updateDoll=False)
