#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\environment\spaceObject\playerOwnedStructure.py
...
POS_STATE_SOUNDS = {STATE_ONLINE: {CONTROL_TOWER: 'msg_ct_online_play',
                PLAYER_OWNED_STRUCTURE: 'msg_pos_online_play'},
 STATE_OFFLINE: {CONTROL_TOWER: 'msg_ct_offline_play',
                 PLAYER_OWNED_STRUCTURE: 'msg_pos_offline_play'},
 STATE_BUILDING: {PLAY_EVENT: 'building_effect_play',
                  STOP_EVENT: 'building_effect_stop'},
 STATE_TEARDOWN: {PLAY_EVENT: 'building_teardown_effect_play',
                  STOP_EVENT: 'building_teardown_effect_stop'}}
...
class PlayerOwnedStructure(BuildableStructure):
    ...
    def LoadModel(self, fileName = None, loadedModel = None):
        ...
        if animationState is not STATE_NONE and not self.isConstructing:
            self.SetControllerVariable('posState', animationState)
    ...
    def StartBuildingEffect(self, isBuilding, dogmaBuildingLength, elapsedTime):
        curveLength = 1.0 * dogmaBuildingLength / 1000.0
        elapsedTime = 1.0 * elapsedTime / SEC
        self.PreBuildingSteps()
        if self.model.controllers and not self.model.controllers[0].isPlaying:
            self.SetControllerVariable('IsBuilt', not isBuilding)
            self.model.StartControllers()
        self.SetControllerVariable('BuildDuration', curveLength)
        self.SetControllerVariable('BuildElapsedTime', elapsedTime)
        self.SetControllerVariable('IsBuilt', isBuilding)
        uthread.new(self.EndBuildingEffect, isBuilding, int((curveLength - elapsedTime) * 1000))
        self.PlayStateSound(isBuilding, PLAY_EVENT)

    def EndBuildingEffect(self, isBuilding, delay):
        blue.pyos.synchro.SleepSim(delay)
        if self.released or self.exploded:
            return
        requiredFreeValue = isBuilding
        while self.isFree == requiredFreeValue:
            self.logger.debug('PlayerOwnedStructure: Yielding until self.isFree is correct')
            blue.synchro.Yield()

        self.PostBuildingSteps(isBuilding)
        self.PlayStateSound(STATE_BUILDING if isBuilding else STATE_TEARDOWN, STOP_EVENT)
        self.LoadModel()
    ...
    def BeginStructureAnchoring(self, timeSinceStart):
        ...
        self.StartBuildingEffect(True, buildingLength, timeSinceStart)
    ...
    def BeginStructureUnAnchoring(self, timeSinceStart):
        ...
        self.StartBuildingEffect(False, buildingLength, timeSinceStart)
