#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\environment\spaceObject\corpse.py
...
def GetCorpseGraphicsID(gender, color, variation):
    return corpseGraphicsIDs[gender][color][variation]


def GetCorpseModel(gender, color, variation):
    gid = GetCorpseGraphicsID(gender, color, variation)
    dna = gfxutils.BuildSOFDNAFromGraphicID(gid)
    sof = GetSofService().spaceObjectFactory
    return sof.BuildFromDNA(dna)


def GetCorpseModelForCharacter(charID, variation = None):
    ...
    return GetCorpseModel(gender, color, variation)


def GetRandomCorpseModel(gender, seed = None):
    r = random.Random(seed)
    corpseVariation = r.randrange(0, CORPSE_VARIATION_COUNT)
    color = r.choice(corpseColorVariations)
    return GetCorpseModel(gender, color, corpseVariation)
...
class Corpse(SpaceObject):
    ...
    def LoadModel(self, fileName = None, loadedModel = None):
        ...
        if loadedModel is None:
            loadedModel = GetRandomCorpseModel(gender, self.id)
