#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\environment\effects\stretchEffect.py
import blue
import geo2
import trinity
import uthread
...
class StretchEffect(GenericEffect):
    ...
    def Prepare(self):
        self.DoPrepare(trinity.EveLocalPositionBehavior.nearestBounds, trinity.EveLocalPositionBehavior.nearestBounds)

    def DoPrepare(self, sourceBehavior, destBehavior):
...
class LocatorStretchEffect(StretchEffect):
    ...
    @property
    def destinationBehaviour(self):
        return trinity.EveLocalPositionBehavior.damageLocator
    ...
    def Prepare(self):
        ...
        self.gfx.source = trinity.EveLocalPositionCurve(sourceBehavior)
        self.gfx.dest = trinity.EveLocalPositionCurve(self.destinationBehaviour)
        self.gfx.source.parentPositionCurve = positionCurve
        self.gfx.source.parentRotationCurve = shipBall
...
class LocatorStretchEffectSwitching(LocatorStretchEffect):
    __guid__ = 'effects.StretchEffect'

    @LocatorStretchEffect.destinationBehaviour.getter
    def destinationBehaviour(self):
        return trinity.EveLocalPositionBehavior.damageLocator
...
class TurretStretchEffect(StretchEffect):
    ...
    def Prepare(self):
        ...
        sourceOffsetCurve = trinity.Tr2TranslationAdapter()
...
class DamageLocatorToDamageLocatorStretchEffect(StretchEffect):
    __guid__ = 'effects.DamageLocatorToDamageLocatorStretchEffect'
...
class BoundsToCenterStretchEffect(StretchEffect):
    __guid__ = 'effects.BoundsToCenterStretchEffect'

    def Prepare(self):
        self.DoPrepare(trinity.EveLocalPositionBehavior.nearestBounds, trinity.EveLocalPositionBehavior.offsetPosition)
