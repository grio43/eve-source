#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\environment\effects\cloaking.py
import random
...
class Cloak(ShipRenderEffect):
    ...
    def Prepare(self):
        ...
        self.GetEffectShipBall().CancelSkinChange()
        ball = self.GetEffectShipBall()
        self.sourceObject = ball.model
        self.AddSoundToEffect(scaler=1.0, model=self.sourceObject, guidOverride=Cloak.__guid__)
        ...
        damageLocators = self.sourceObject.locatorSets.FindByName('damage').locators
        self.sourceObject.clipSphereCenter = random.choice(damageLocators)[0]
