#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\environment\effects\MicroJumpDrive.py
...
class MicroJumpDriveEngage(ShipEffect):
    ...
    def _SetCurveTime(self, duration):
        k1 = list(self.controllerCurve.keys[-1])
        k2 = list(self.controllerCurve.keys[-2])
        timeDelta = k1[0] - k2[0]
        k1[0] = duration
        k2[0] = duration - timeDelta
        self.controllerCurve.keys[-1] = tuple(k1)
        self.controllerCurve.keys[-2] = tuple(k2)
        self.controllerCurve.OnKeysChanged()
