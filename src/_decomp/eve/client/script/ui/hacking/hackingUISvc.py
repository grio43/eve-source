#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\hacking\hackingUISvc.py
import random
import blue
import log
...
class HackingSvc(Service):
    ...
    def _OpenWindowOrReloadIfGameEnded(self):
        if self.gameEnded:
            window = HackingWindow.GetIfOpen()
            if not window:
                self._ResetGameInfo()
                HackingWindow.Open()
            else:
                self._ResetGameInfo()
                HackingWindow.Reload(window)
    ...
    def ConstructUtilityElements(self, inventory):
        for i, element in enumerate(inventory):
            elementData = UtilityElementData(index=i, **element)
