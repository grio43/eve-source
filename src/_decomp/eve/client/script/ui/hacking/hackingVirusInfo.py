#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\hacking\hackingVirusInfo.py
import random
from math import pi
...
class VirusInfo(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.strength = None
        self.coherence = None
        self.iconSprite = Sprite(name='virusIconSprite', texturePath='res:/UI/Texture/classes/hacking/hudIcon.png', parent=self, align=uiconst.CENTER, state=uiconst.UI_DISABLED, pos=(0, 0, 59, 44))
        ...
        Sprite(name='bg', parent=self, align=uiconst.TOALL, texturePath='res:/UI/Texture/classes/hacking/hudBG.png')
        ...
        self.iconBG = Sprite(name='iconBG', texturePath='res:/UI/Texture/classes/hacking/hudIconBG.png', parent=self, align=uiconst.CENTER, state=uiconst.UI_DISABLED, pos=(0, 0, 118, 118), opacity=0.0)
    ...
    def _AnimateIconBG(self):
        color = Color(*hackingUIConst.COLOR_DATACACHE).SetAlpha(0.5).GetRGBA()
    ...
    def UpdateVirusInfo(self):
        ...
        if self.coherence and coherence < self.coherence:
            ...
            if self.animateIconThread:
                self.animateIconThread.kill()
            uicore.animations.SpColorMorphTo(self.iconBG, Color.WHITE, (0, 0, 0, 0), duration=0.6, callback=self.AnimateIconBG)
...
class VirusInfoBar(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.bar = Container(name='bar', parent=self, align=uiconst.TOBOTTOM_PROP, state=uiconst.UI_DISABLED, height=0.0, clipChildren=True)
        self.barTexture = Sprite(name='barTexture', parent=self.bar, align=uiconst.CENTERBOTTOM, width=self.width, height=self.height, texturePath=fillTexturePath, textureSecondaryPath=stripeTexturePath, spriteEffect=trinity.TR2_SFX_MODULATE, color=self.color)
        self.bg = Sprite(name='bg', bgParent=self, texturePath=fillTexturePath, color=hackingUIConst.COLOR_HUD_BAR_BG, textureSecondaryPath=stripeTexturePath, spriteEffect=trinity.TR2_SFX_MODULATE)
...
class HudNoise(Transform):
    ...
    def ApplyAttributes(self, attributes):
        ...
        num = attributes.num
        self.intensity = 1.0
        self.sprite = Sprite(bgParent=self, texturePath=texturePath)
    ...
    def Animate(self, num):
        ...
        while not self.destroyed:
            ...
            color = Color(*hackingUIConst.COLOR_EXPLORED).SetAlpha(0.1 + 0.4 * random.random()).GetRGBA()
            self.sprite.SetRGBA(*color)
