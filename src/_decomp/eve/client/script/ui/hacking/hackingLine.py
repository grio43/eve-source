#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\hacking\hackingLine.py
import random
import geo2
...
class Line(object):
    ...
    def __init__(self, parent, tileFrom, tileTo):
        ...
        self.line = VectorLine(parent=parent, align=uiconst.TOPLEFT)
    ...
    def ShowBleed(self):
        if not self.bleedSprite:
            ...
            if self.lineType == hackingUIConst.LINETYPE_DECLINE:
                top += height
                height *= -1
            self.bleedSprite = Sprite(name='bleedSprite', parent=self.parent, state=uiconst.UI_DISABLED, texturePath=texturePath, align=uiconst.TOPLEFT, pos=(left,
             top,
             width,
             height), opacity=0.0, color=(0.76, 0.2, 0))
