#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\hacking\hackingTile.py
import random
from math import pi
...
class Tile(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        showCoordLabel = attributes.showCoordLabel
        ...
        if showCoordLabel:
            self.AddCoordLabel()
        self.mouseHoverSprite = Sprite(name='mouseHoverSprite', parent=self, align=uiconst.TOALL, state=uiconst.UI_DISABLED, texturePath='res:/UI/Texture/classes/hacking/tileHover.png', opacity=0.0)
        self.coherenceCont = None
        self.strengthCont = None
        self.distanceIndicatorCont = None
        self.iconSprite = Sprite(name='iconSprite', parent=self, align=uiconst.CENTER, state=uiconst.UI_DISABLED, width=hackingUIConst.TILE_ICON_SIZE, height=hackingUIConst.TILE_ICON_SIZE)
        self.tileBgTransform = Transform(name='tileBgTransform', parent=self, align=uiconst.CENTER, state=uiconst.UI_DISABLED, width=hackingUIConst.TILE_ICON_SIZE, height=hackingUIConst.TILE_ICON_SIZE, scalingCenter=(0.5, 0.5), scale=self.bgScale)
        self.tileBgSprite = Sprite(name='tileSprite', bgParent=self.tileBgTransform)
    ...
    def AddCoordLabel(self):
        if not session.role & ROLE_QA:
            return
        x, y = self.tileData.coord
        coordText = '(%s, %s)' % (x, y)
        eveLabel.Label(name='coordLabel', parent=self, align=uiconst.CENTERTOP, fontsize=fontconst.EVE_SMALL_FONTSIZE, text=coordText, idx=0)
    ...
    def UpdateTileState(self, eventID = None, tileData = None):
        ...
        self.iconColor = hackingUIConst.COLOR_TILE_ICON_BY_TYPE.get(tileType, Color.WHITE)
    ...
    def ConstructMarkerSprite(self):
        if self.utilElementMarkerSprite is None:
            self.utilElementMarkerSprite = Sprite(name='utilElementMarkerSprite', bgParent=self, texturePath='res:/UI/Texture/classes/hacking/utilTileMouseDown.png', opacity=0.0, padding=-20)
    ...
    def ShowDistanceIndicator(self):
        if self.distanceIndicatorCont is None:
            self.distanceIndicatorCont = eveLabel.Label(name='distanceIndicatorCont', parent=self, align=uiconst.CENTER, fontsize=fontconst.EVE_SMALL_FONTSIZE, text=self.tileData.distanceIndicator, idx=0)
    ...
    def OnMouseEnter(self, *args):
        ...
        color = Color(*self.bgColor).SetBrightness(1.0).GetRGBA()
        uicore.animations.SpColorMorphTo(self.tileBgSprite, self.tileBgSprite.GetRGBA(), color, duration=0.3)
        if self.HasLargeIcon():
            uicore.animations.SpColorMorphTo(self.iconSprite, self.iconSprite.GetRGBA(), Color.WHITE, duration=0.3)
        else:
    ...
    def ConstructHealingGivenSprite(self):
        if self.healingGivenSprite is None:
            self.healingGivenSprite = Sprite(name='healingGivenSprite', parent=self, align=uiconst.CENTER, state=uiconst.UI_DISABLED, color=hackingUIConst.COLOR_UNEXPLORED, texturePath='res:/UI/Texture/classes/hacking/healRing1.png', opacity=0.0)

    def ConstructHealingReceivedSprite(self):
        if self.healingReceivedSprite is None:
            self.healingReceivedSprite = Sprite(name='healingReceivedSprite', parent=self, align=uiconst.CENTER, state=uiconst.UI_DISABLED, color=hackingUIConst.COLOR_UNEXPLORED, texturePath='res:/UI/Texture/classes/hacking/healRing2.png', opacity=0.0)
