#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\structure\accessGroups\ownerGroupUtils.py
...
def GetOwnerIDsAndMembershipTypeFromNodes(nodes, useNodeMembershipType = False, membershipType = MEMBERSHIP_TYPE_MEMBER):
    if not nodes:
        return
    firstNode = nodes[0]
    guid = getattr(firstNode, '__guid__', None)
...
def GetTextLinkNode(node):
    validTypeIDs = [const.typeCharacter, const.typeCorporation, const.typeAlliance]
