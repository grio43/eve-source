#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\structure\accessGroups\dropBar.py
import math
...
class DropCont(Container):
    ...
    def ConstructUI(self, controller):
        ...
        arrowPos = (0,
         ...
         7,
         7)
        self.arrow = Sprite(parent=self, align=uiconst.TOPLEFT, pos=arrowPos, texturePath='res:/UI/Texture/classes/Neocom/arrowDown.png', rotation=math.pi)
        ...
        self.dropTextCont = Container(name='dropOverlay', parent=self, align=uiconst.TOTOP, height=32, padding=(2, 6, 2, 1), state=uiconst.UI_DISABLED, clipChildren=True)
        dropText = GetByLabel('UI/Structures/AccessGroups/DropMembers')
        self.dropLabel = EveLabelMedium(name='dropLabel', parent=self.dropTextCont, text=dropText, align=uiconst.CENTER)
        self.colorFill = FillThemeColored(name='colorFill', bgParent=self.dropTextCont, colorType=uiconst.COLORTYPE_UIHILIGHT, opacity=0.0)
        self.bgFill = FillThemeColored(name='bgFill', bgParent=self.dropTextCont, colorType=uiconst.COLORTYPE_UIBASECONTRAST, opacity=FILL_IDLE_OPACITY)
    ...
    def MoveArrow(self, membershipType):
        w = self.dropGridCont.renderObject.displayWidth
        step = float(w) / len(ACCESS_SORT_ORDER)
        membershipIdx = GetMembershipSortIdx(membershipType)
        offset = (membershipIdx + 0.5) * step - self.arrow.width / 2.0
...
class DropBar(GridContainer):
    ...
    def ApplyAttributes(self, attributes):
        ...
        for membershipType in ACCESS_SORT_ORDER:
            texturePath, hintPath, textureSize = GetMembershipTypeTextureAndLabelPaths(membershipType)
            d = DropContainer(parent=self, texturePath=texturePath, textureSize=textureSize, membershipType=membershipType, controller=self.controller, currentGroupID=self.currentGroupID, hintPath=hintPath, dropCallback=self.AddGroupMembersWithMembershipType, mouseCallback=self.MouseCallback, padding=PADDING)
            self.dropContainers[membershipType] = d

        self.dropOverlay = Container(name='dropOverlay', parent=self, align=uiconst.TOBOTTOM, height=32, state=uiconst.UI_DISABLED, clipChildren=True)
...
class DropContainer(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        textureWidth, textureHeight = attributes.textureSize
        self.membershipType = attributes.membershipType
        self.dropCallback = attributes.dropCallback
        self.isDisabled = False
        self.icon = GlowSprite(parent=self, align=uiconst.CENTER, pos=(0,
         0,
         textureWidth,
         textureHeight), texturePath=texturePath, state=uiconst.UI_DISABLED)
