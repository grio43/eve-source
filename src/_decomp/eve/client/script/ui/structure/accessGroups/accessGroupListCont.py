#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\structure\accessGroups\accessGroupListCont.py
...
class GroupListCont(Container):
    ...
    def LoadGroups(self, *args):
        ...
        for groupID, g in myGroups.iteritems():
            ...
            isDefaultCorpGroup = g.creatorID == session.corpid
            if isDefaultCorpGroup:
                myMembership = DEFAULT_CORP_GROUP
                nodeData.groupDisplayName = cfg.eveowners.Get(session.corpid).name
            else:
        ...
        for eachMemberShipType in [DEFAULT_CORP_GROUP] + ACCESS_SORT_ORDER:
            ...
            if nodes:
                ...
                scrollList.append(GetFromClass(Space, {'height': 12}))
