#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\structure\accessGroups\menuProvider.py
...
class MenuProvider(object):
    ...
    def _GetGenericMemberMenuOptions(self, nodesByMemberID):
        ...
        if typeID == const.typeAlliance:
            ...
            menuLabel = MenuLabel('UI/Common/Pilot', {'character': memberID})
        memberMenuOptions = [(menuLabel, ('isDynamic', GetMenuService().GetMenuFromItemIDTypeID, (memberID, typeID)))]
    ...
    def GetMenuForGroupID(self, node, parentController):
        ...
        m = []
        if not ownedByCorp:
            m += [(MenuLabel('UI/Structures/AccessGroups/AddMembers'), self._OpenPeopleAndPlaces, ())]
    ...
    def _OpenPeopleAndPlaces(self):
        wnd = AddressBookWindow.GetIfOpen()
        if wnd and not wnd.destroyed:
            wnd.Maximize()
        else:
            return uicore.cmd.OpenPeopleAndPlaces()
