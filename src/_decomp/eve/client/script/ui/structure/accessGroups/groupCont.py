#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\structure\accessGroups\groupCont.py
...
class GroupCont(Container):
    ...
    def LoadGroup(self, groupID):
        ...
        isDefaultCorpGroup = myGroupInfo.creatorID == session.corpid
        groupDisplayName = cfg.eveowners.Get(session.corpid).name if isDefaultCorpGroup else myGroupInfo.name
        self.nameAndDescCont.SetNameAndDescription(groupDisplayName, myGroupInfo.description)
    ...
    def GroupSelected(self, groupID):
        ...
        self.LoadGroup(groupID)
        if self.tabs is None or self.tabs.destroyed:
            return
    ...
    def OnTabSelected(self, tabIdx, oldTabID):
