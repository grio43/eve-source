#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\structure\accessGroups\accessGroupsController.py
...
class AccessGroupsController(object):
    ...
    def __init__(self):
        ...
        self.on_new_group_created = Signal(signalName='on_new_group_created')
        self.on_group_selected = Signal(signalName='on_group_selected')
        self.on_groupmembers_changed = Signal(signalName='on_groupmembers_changed')
        self.on_groupmembers_removed = Signal(signalName='on_groupmembers_removed')
        self.on_group_deleted = Signal(signalName='on_group_deleted')
        self.on_group_updated = Signal(signalName='on_group_updated')
        self.on_groups_reload = Signal(signalName='on_groups_reload')
    ...
    def GetMyGroups(self):
        if self.groupByGroupID is None:
            self.groupByGroupID = {}
            for g in self.remoteSvc.GetMyGroups():
                self.FixGroupLabelsForSystemGroups(g)
                self.groupByGroupID[g.groupID] = g

        return self.groupByGroupID

    def IsMyGroupsCached(self):
        return self.groupByGroupID is not None

    @staticmethod
    def FixGroupLabelsForSystemGroups(group):
        if group.creatorID == invConst.ownerSystem:
            if group.name.startswith('UI/AclBookmarks/'):
                group.name = GetByLabel(group.name)
            if group.description.startswith('UI/AclBookmarks/'):
                group.description = GetByLabel(group.description)
    ...
    def GetGroupInfoFromID(self, groupID, fetchToServer = False):
        if not fetchToServer and not self.IsMyGroupsCached():
            return None
        groupInfo = self.GetMyGroupInfo(groupID)
        if groupInfo:
            return groupInfo
        ...
        if not fetchToServer:
            return None
        group = self.remoteSvc.GetGroup(groupID)
        self.FixGroupLabelsForSystemGroups(group)
        return group
