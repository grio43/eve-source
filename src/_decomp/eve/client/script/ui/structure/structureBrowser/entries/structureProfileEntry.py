#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\structure\structureBrowser\entries\structureProfileEntry.py
import itertoolsext
...
class StructureProfileEntry(ProfileEntryBase):
    ...
    def ApplyAttributes(self, attributes):
        ...
        if self.profileController.IsDefault():
            self.profileNameLabel.underline = True
    ...
    def GetMenu(self, *args):
        m = [(MenuLabel('UI/StructureBrowser/EditNameAndDescription'), self.EditNameAndDescription, ()), (MenuLabel('UI/StructureBrowser/DuplicateProfile'), self.DuplicateProfile, ()), (MenuLabel('UI/StructureBrowser/DeleteProfile'), self.DeleteProfile, ())]
        if session.corprole & const.corpRoleStationManager and not self.profileController.IsDefault():
            m.append((MenuLabel('UI/StructureBrowser/SetDefaultProfile'), self.SetDefaultProfile, ()))
        if session.role & ROLEMASK_ELEVATEDPLAYER:
            profileID = self.profileController.GetProfileID()
            m.append(('GM: profileID: ' + str(profileID), blue.pyos.SetClipboardData, (str(profileID),)))
        return m

    def DuplicateProfile(self, *args):
        profileID = self.profileController.GetProfileID()
        self.allStructuresProfileController.DuplicateProfile(profileID)
    ...
    def SetDefaultProfile(self):
        self.profileController.SetDefaultProfile()
        self.structureBrowserController.on_default_profile_set()
