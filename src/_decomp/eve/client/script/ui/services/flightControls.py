#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\services\flightControls.py
import collections
import math
import geo2
import blue
...
class FlightSimulation(object):
    ...
    def Update(self):
        while True:
            ...
            if self.ship and self.controls and self.callback:
                with ExceptionEater('FlightSimulation'):
                    self.AttachBalls()
                    self.ProcessInput()
    ...
    def AttachBalls(self):
        self.ballpark.tickInterval = 100
        if getattr(self.ship, 'model', None):
