#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\services\corporation\corp_util.py
...
def HasRole(corp_role):
    return bool(corp_role & session.corprole)


def HasAnyRole(corp_roles):
    for corp_role in corp_roles:
        if HasRole(corp_role):
            return True
