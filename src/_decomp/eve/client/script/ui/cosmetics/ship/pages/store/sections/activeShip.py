#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\cosmetics\ship\pages\store\sections\activeShip.py
import evetypes
...
class ActiveShipSkinsSection(BaseSkinsSection):
    page_id = SkinrPage.STORE_SKINS
    category = StoreSection.SKINS_ACTIVE_SHIP
    name = 'UI/Personalization/ShipSkins/SKINR/Studio/ActiveShipSKINs'

    def get_type_filter(self):
        if self.apply_filtering:
            return [current_skin_design.get_default_ship_type_id()]
        else:
            return []

    def get_name(self):
        return GetByLabel(self.name, shipName=evetypes.GetName(current_skin_design.get_default_ship_type_id()))


class ActiveShipSectionContainer(BaseSkinsSectionContainer):

    def _set_section_controller(self, section_controller = None):
        self.section_controller = ActiveShipSkinsSection(num_per_page=None)


class ViewAllSkinsActiveShipSectionContainer(BaseSkinsSectionContainer):

    def _set_section_controller(self, section_controller = None):
        self.section_controller = ActiveShipSkinsSection(MAX_NUM_PER_PAGE)
