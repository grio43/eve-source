#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\skilltrading\test\test_skillExtractorController.py
...
class TestSkill(ClientTestCase):
...
@unittest.skip('Tests fail with NotImplementedError')

class TestSkillExtractorController(ClientTestCase):
    ...
    def test_restricted_skill_has_reason(self):
        skill = self.addSkill(restrictedLevel=1, restrictionReason=RESTRICTION_IMPLANT)
        self.controller.skills
        self.assertEqual(skill.restrictionReason, RESTRICTION_IMPLANT)

    def test_over_trained_skills_are_restricted(self):
        skill = self.addSkill(points=10000000, restrictedLevel=5)
        self.controller.skills
        self.assertTrue(skill.isRestricted)
    ...
    def addSkill(self, typeID = None, points = SP_R1_L1, rank = 1.0, requirements = None, restrictedLevel = None, restrictionReason = None):
        if typeID is None:
        ...
        if restrictedLevel is not None:
            self.restrictSkill(skill, restrictedLevel, restrictionReason)
        return skill

    def restrictSkill(self, skill, level, reason = None):
        if level == 0:
            del self.controller._mock_skill_restrictions[skill.typeID]
        else:
            if reason is None:
                reason = RESTRICTION_CLONE_STATE
            restriction = SkillRestriction(level, reason)
            self.controller._mock_skill_restrictions[skill.typeID] = restriction
