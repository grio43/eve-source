#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\login\charcreation\bloodlineSelector.py
import log
...
ANIM_DIR = 'res:\\animation\\%s\\charactercreation'
    ...
    def LoadRace(self, raceID, callBack = None):
        ...
        if raceID in RACE_PATHS_MAPPING:
            ...
            for bloodline in RACE_PATHS_MAPPING[raceID]:
                female = PaperDollCharacter.ImportCharacter(factory, self.scene, bloodline[0][0])
                male = PaperDollCharacter.ImportCharacter(factory, self.scene, bloodline[0][1])
                self.lineUp.append(self.Platform((female, male), bloodline[1], bloodline[2]))

        else:
            log.LogError('raceID ' + raceID + ' not valid')
            return
        remotefilecache.prefetch_folder(ANIM_DIR % 'female')
        remotefilecache.prefetch_folder(ANIM_DIR % 'male')
        for platform in self.lineUp:
            for i, each in enumerate(platform.pair):
                grannyLoader.GrannyBloodlineAnimationLoader(each.avatar, i, platform.poseID)
