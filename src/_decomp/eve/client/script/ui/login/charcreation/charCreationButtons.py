#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\login\charcreation\charCreationButtons.py
import trinity
...
class RaceButton(BaseCCButton):
    ...
    def OnMouseEnter(self, *args):
        ...
        if not info.raceID:
            uicore.layer.charactercreation.controller.UpdateBackdropLite(raceID=self.raceID, mouseEnter=True)

    def OnMouseExit(self, *args):
        BaseCCButton.OnMouseExit(self, *args)
        info = uicore.layer.charactercreation.controller.GetInfo()
        if not info.raceID:
            uicore.layer.charactercreation.controller.UpdateBackdropLite(raceID=None, mouseEnter=True)
        if self.mouseExitCallback is not None:
            self.mouseExitCallback()

    def OnClick(self, *args):
        if uicore.layer.charactercreation.controller.raceID != self.raceID:
            uicore.layer.charactercreation.controller.SelectRace(self.raceID)
...
class GenderButton(BaseCCButton):
    ...
    def ApplyAttributes(self, attributes):
        ...
        if self.genderID == ccConst.GENDERID_FEMALE:
            ...
            gender = 'Male'
        self.normalSprite.texture.resPath = 'res:/UI/Texture/classes/EmpireSelection/gender%s.png' % gender
        self.hiliteSprite.texture.resPath = 'res:/UI/Texture/classes/EmpireSelection/gender%sSelected.png' % gender

    def OnClick(self, *args):
        if uicore.layer.charactercreation.controller.genderID != self.genderID:
            uicore.layer.charactercreation.controller.SelectGender(self.genderID)

    def HoverInHighlighted(self):
        self.Deselect()
        self.normalSprite.SetAlpha(1.0)
        self.Show()

    def HoverIn(self):
        self.Deselect()
        self.normalSprite.SetAlpha(0.25)
        self.Show()

    def HoverOut(self):
        self.Deselect()
        self.normalSprite.Hide()
    ...
    def Resize(self, width, height):
        self.width = width
        self.height = height
        self.normalSprite.width = width
        self.normalSprite.height = height
        self.hiliteSprite.width = width
        self.hiliteSprite.height = height
...
class BloodlineButton(BaseCCButton):
    ...
    def OnClick(self, *args):
        if uicore.layer.charactercreation.controller.bloodlineID != self.bloodlineID:
            uicore.layer.charactercreation.controller.SelectBloodline(self.bloodlineID)
