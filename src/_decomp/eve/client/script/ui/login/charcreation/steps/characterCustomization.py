#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\login\charcreation\steps\characterCustomization.py
...
from eve.client.script.ui.login.charcreation.assetMenu import CCColorPalette, CharCreationAssetMenu
from eve.client.script.ui.login.charcreation.charCreation import BaseCharacterCreationStep, CCHeadBodyPicker, MAXMENUSIZE
from eve.client.script.ui.login.charcreation.hexes import CCGenderPicker, CCHexButtonRandomize
...
class CharacterCustomization(BaseCharacterCreationStep):
    ...
    def ApplyAttributes(self, attributes):
        ...
        clickable = uicore.layer.charactercreation.controller.CanChangeBloodline()
        if clickable and not uicore.layer.charactercreation.controller.CanChangeGender():
            disabledHex = ['gender']
        else:
            disabledHex = []
        CCGenderPicker(parent=Container(parent=customizationOptionsContainer, align=uiconst.TOTOP, height=80, left=40), align=uiconst.CENTER, genderID=info.genderID, clickable=clickable, disabledHex=disabledHex)
        EveLabelMedium(name='genderLabel', parent=customizationOptionsContainer, align=uiconst.TOTOP, text='<center>%s</center>' % localization.GetByLabel('UI/Common/Gender/Gender'))
        self.randomizeButton = CCHexButtonRandomize(name='randomizeButton', parent=Container(parent=customizationOptionsContainer, align=uiconst.TOTOP, height=64), align=uiconst.CENTER, state=uiconst.UI_NORMAL, width=64, height=64, pickRadius=32, iconNum=0, func=self.RandomizeCharacter, hexName=localization.GetByLabel('UI/CharacterCreation/Randomize'))
        ...
        uicore.layer.charactercreation.controller.camera.cameraModeChanged.connect(self._OnCameraModeChanged)
    ...
    def ToggleClothes(self, *args):
        ...
        uicore.layer.charactercreation.controller.ToggleClothes()

    def OnGenderSelected(self, genderID):
        self.GoToAssetMode(forcedMode=1)
        if self.tattoMenu:
    ...
    def RandomizeCharacter(self, *args):
        ...
        canChangeBaseAppearance = uicore.layer.charactercreation.controller.CanChangeBaseAppearance()
        ...
        if boot.region == 'optic' and info.genderID == ccConst.GENDERID_FEMALE:
        ...
        self.charSvc.RandomizeCharacterGroups(info.charID, info.raceID, categoryList, doUpdate=False, fullRandomization=True)
        if canChangeBaseAppearance:
            self.charSvc.RandomizeCharacterSculpting(info.charID, info.raceID, doUpdate=False)
    ...
    def ReloadTattooMenu(self, *args):
        ...
        self.tattoMenu = CharCreationAssetMenu(menuType='tattooMenu', parent=self.assetMenuParent, bloodlineID=info.bloodlineID, raceID=info.raceID, state=uiconst.UI_PICKCHILDREN, genderID=info.genderID, charID=info.charID, groups=groups, align=uiconst.TOALL, toggleFunc=self.GoToAssetMode, togglerIdx=0, top=26)
        return self.tattoMenu
    ...
    def ReloadAssetMenu(self, *args):
        ...
        self.assetMenu = CharCreationAssetMenu(menuType='assetMenu', parent=self.assetMenuParent, bloodlineID=info.bloodlineID, raceID=info.raceID, state=uiconst.UI_PICKCHILDREN, genderID=info.genderID, charID=info.charID, groups=groups, align=uiconst.TOALL, toggleFunc=self.GoToTattooMode, clipChildren=True, top=26)
        if self.historySlider:
            self.historySlider.Close()
        self.historySlider = CharacterCreationHistorySlider(parent=self.uiContainer, align=uiconst.CENTERBOTTOM, top=-45, width=400, opacity=0.0, bitChangeCheck=self.IsDollReady, lastLitHistoryBit=uicore.layer.charactercreation.controller.lastLitHistoryBit)
    ...
    def AddBlankDrifterDoll(self, *args):
        ...
        doll = uicore.layer.charactercreation.controller.doll = self.charSvc.GetSingleCharactersDoll(info.charID)
    ...
    def AddTypeButton(self, *args):
        ...
        if paperdollResource:
            ...
            uicore.layer.charactercreation.controller.SetItemType(itemInfo)
    ...
    def LoadFaceMode(self, *args):
        ...
        uicore.layer.charactercreation.controller.camera.ToggleMode(ccConst.CAMERA_MODE_FACE, avatar=avatar, transformTime=500.0)

    def LoadBodyMode(self, *args):
        info = self.GetInfo()
        avatar = self.charSvc.GetSingleCharactersAvatar(info.charID)
        uicore.layer.charactercreation.controller.camera.ToggleMode(ccConst.CAMERA_MODE_BODY, avatar=avatar, transformTime=500.0)
    ...
    def StoreHistorySliderPosition(self, *args):
        if self.historySlider:
            ...
            currentIndex = None
        uicore.layer.charactercreation.controller.lastLitHistoryBit = currentIndex
    ...
    def _OnCameraModeChanged(self, previousMode, newMode):
        if previousMode is not None and newMode in [ccConst.CAMERA_MODE_FACE, ccConst.CAMERA_MODE_BODY]:
            ...
            uicore.layer.charactercreation.controller.camera.cameraModeChanged.disconnect(self._OnCameraModeChanged)
