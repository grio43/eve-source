#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\login\charcreation_new\charCreationColorPicker.py
import math
...
class CharCreationColorPickerCombo(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        margin = 3
        c1 = Container(parent=self, pos=(0,
         0,
         128,
        ...
        p1 = CharCreationColorPicker(parent=c1, colors=colorsTop, OnSetValue=attributes.get('OnSetValueTop', None), modifierName=attributes.get('modifierName', ''), activeColorIndex=activeColorIndexTop, enableHalf=0)
        self.topPicker = p1
        c2 = Container(parent=self, pos=(0,
         64 + margin,
         128,
        ...
        p2 = CharCreationColorPicker(parent=c2, top=-64 - margin, colors=colorsBottom, OnSetValue=attributes.get('OnSetValueBottom', None), modifierName=attributes.get('modifierName', ''), activeColorIndex=activeColorIndexBottom, enableHalf=1)
        self.bottomPicker = p2


class CharCreationColorPicker(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.LoadColorResources(colors)
    ...
    def LoadColorResources(self, colors):
        for colorName, displayColor, colorType in colors:
            bloodlineType = colorType[1][0]
            if bloodlineType not in ccConst.SKINTYPE_TO_BLOODLINE:
                continue
            bloodlineID = ccConst.SKINTYPE_TO_BLOODLINE[bloodlineType]
            if bloodlineID not in self._colorResources:
                self._colorResources[bloodlineID] = []
            self._colorResources[bloodlineID].append((colorName, displayColor, colorType))
    ...
    def InitColors(self, colors, activeColorIndex = 0):
        ...
        activeColorResource = colors[activeColorIndex]
        info = uicore.layer.charactercreation.controller.GetInfo()
        colors = self._colorResources.get(info.bloodlineID, [])
        ...
        while totalColors not in availableColorAmount and colors:
            log.LogWarn('The amount of colors provided for colorwheel is incorrect, is:', totalColors, 'but should be one of:', availableColorAmount)
            colors.append(colors[-1])
            totalColors = len(colors)

        step = availableSpace / totalColors
        radianStep = math.radians(step)
        ...
        self.centerParent = Transform(parent=self, pos=(0,
         0,
         centerSize,
         centerSize), pickRadius=centerPickRadius, align=uiconst.CENTER, state=uiconst.UI_DISABLED, name='centerTransform', idx=0)
        self.colorSample = Sprite(parent=self.centerParent, texturePath='res:/UI/Texture/none.dds', ignoreSize=True, align=uiconst.TOALL, pos=(0, 0, 0, 0), state=uiconst.UI_DISABLED, color=(1.0, 1.0, 1.0, 0.0))
        self.colorBitsParent = Transform(parent=self, pos=(0,
         0,
         colorParentSize,
         colorParentSize), align=uiconst.TOPLEFT, name='colorBitsParent')
        activeColor = None
        for i, (colorName, displayColor, colorType) in enumerate(colors):
            if colorType[1][2] == activeColorResource[2][1][2]:
    ...
    def AddColor(self, idx, colorName, displayColor, colorType, radianAngle, colorNo, colorAmount):
        ...
        colorBit = Sprite(parent=self.colorBitsParent, texturePath=path, ignoreSize=True, align=uiconst.TOPLEFT, pos=(left,
         top,
         right - left,
    ...
    def OnMouseMove(self, *args):
        ...
        self.centerParent.SetOrder(0)
        lastActive = getattr(self, 'lastActive', None)
        if setActive:
            if lastActive is not setActive:
                sm.StartService('audio').SendUIEvent(unicode('ui_icc_color_wheel_panel_light_play'))
                self.lastActive = setActive
            setActive.SetOrder(0)

    def OnClick(self, *args):
        sm.StartService('audio').SendUIEvent(unicode('ui_icc_color_wheel_color_selected_play'))
        setActive = self.GetColorBitFromRadian(self._lastRadian)
        if setActive:
            self.SetActiveColor(setActive.colorType)

    def OnMouseExit(self, *args):
        if self.centerParent:
            self.centerParent.SetOrder(0)
    ...
    def SetActiveColor(self, colorType, initing = False):
        if self.colorBitsParent is None or self.colorSample is None:
            return
        if colorType is None:
            self.colorSample.texturePath = 'res:/UI/Texture/none.dds'
            return
        for each in self.colorBitsParent.children:
    ...
    def ShowActiveColorBit(self, colorBit):
        if colorBit:
            self.colorSample.texturePath = 'res:/UI/Texture/CharacterCreation/ColorPointer.dds'
            fromRad = self.centerParent.GetRotation()
            if fromRad < 0:
                fromRad += math.pi * 2
            ...
            if travel > 180.0:
                if toDeg > fromDeg:
                    ...
                    toDeg += 360.0
                travel = 360.0 - travel
            animations.MorphScalar(self.centerParent, 'rotation', startVal=fromRad, endVal=toRad, duration=max(0.1, travel * 0.002))
            if colorBit.displayColor:
                animations.SpColorMorphTo(self.colorSample, endColor=colorBit.displayColor, duration=0.25)

    def OnSetValue(self, control, colorType, colorName):
        pass

    def GetColorBitFromRadian(self, radian):
        if self.colorBitsParent is None:
            return
        for each in self.colorBitsParent.children:
            if not hasattr(each, 'centerRadian'):
                continue
