#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\login\charcreation_new\steps\sections\chooseNameSection.py
import uthread2
import utillib
...
RANDOMIZE_NAME_BUTTON_ANALYTIC_ID = 'RandomizeNameButton'
padding = 16
VALID = 1
...
class ChooseNameSection(BaseSidePanel):
    default_alignMode = uiconst.TOTOP
    default_mainContPadding = 20
    ...
    def _ConstructNameSelectionLabel(self):
        EveCaptionLarge(name='nameSelectionLabel', parent=self.mainCont, text=GetByLabel('UI/CharacterCreation/NameSelection'), align=uiconst.TOTOP, state=uiconst.UI_DISABLED, padTop=10)

    def _ConstructNameEdit(self):
        editContainer = Container(name='editContainer', parent=self.mainCont, align=uiconst.TOTOP, height=30, padTop=8)
        self.checkAvailabilityIcon = Icon(name='checkAvailabilityIcon', parent=Container(parent=editContainer, align=uiconst.TORIGHT, width=16), align=uiconst.CENTER, pos=(0, 0, 20, 20), state=uiconst.UI_HIDDEN)
        self.nameEdit = SingleLineEditText(name='nameEdit', setvalue=self.GetInfo().charName or '', parent=editContainer, maxLength=appConst.maximumCharacterNameLength, align=uiconst.TOALL, OnChange=self.OnTextEditChange, OnSetFocus=self.OnFirstNameFocus, color=(1.0, 1.0, 1.0, 1.0), hintText=GetByLabel('UI/CharacterCreation/FullName'), fontsize=16, width=5, height=0)
        self.nameEdit.OnReturn = self.CheckAvailability
        self.nameEdit.OnAnyChar = self.OnCharInName

    def _ConstructBottomCont(self):
        bottomCont = Container(name='bottomCont', parent=self.mainCont, align=uiconst.TOTOP, height=16, state=uiconst.UI_PICKCHILDREN, padTop=4, padRight=20)
        MoreInfoIcon(name='moreInfoIcon', parent=bottomCont, align=uiconst.TOLEFT, hint=GetByLabel('UI/CharacterCreation/NameConstraintsHint'))
        self.checkAvailabilityLabel = EveLabelMedium(name='checkAvailabilityLabel', parent=bottomCont, align=uiconst.TOLEFT, width=300, padLeft=6, state=uiconst.UI_DISABLED, color=Color.RED, opacity=0.75)
        ...
        if not AmOnChineseServer():
            buttonCont = Container(parent=self.mainCont, align=uiconst.TOTOP, height=25, padTop=12)
            Button(name='randomizeNameButton', parent=buttonCont, label=GetByLabel('UI/CharacterCreation/Randomize'), align=uiconst.TOPLEFT, func=self.RandomizeName, analyticID=RANDOMIZE_NAME_BUTTON_ANALYTIC_ID)
    ...
    def UpdateIsValidIconAndText(self, isValidCode):
        ...
        charCreationSignals.onNameInputChanged(isValidCode == VALID)
