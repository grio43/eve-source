#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\login\charcreation_new\charCreationOvalSlider.py
import math
...
class CharCreationSingleSlider(Container):
...
class CharCreationDoubleSlider(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        settings2 = {'radius': 72,
         ...
         'centerOffset': (0, 82)}
        self.slider2 = CharCreationOvalSlider(parent=self, align=uiconst.CENTERRIGHT, sliderSetting=settings2, OnSetValue=attributes.get('OnSetValue2', None), setValue=attributes.get('setValue2', 0.0), invertSlider=attributes.get('invertSlider2', True), modifierName=attributes.modifierName2, increments=attributes.get('incrementsSlider2', None))
        if attributes.label2:
            CCLabel(parent=self.slider2, fontsize=12, align=uiconst.BOTTOMLEFT, text=attributes.label2, letterspace=2, top=-20, uppercase=True, left=13)


class CharCreationTripleSlider(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        settings1 = {'radius': 72,
         ...
         'centerOffset': (82, 35)}
        self.slider1 = CharCreationOvalSlider(parent=self, align=uiconst.BOTTOMLEFT, sliderSetting=settings1, OnSetValue=attributes.get('OnSetValue1', None), setValue=attributes.get('setValue1', 0.0), invertSlider=attributes.get('invertSlider1', False), modifierName=attributes.modifierName1, increments=attributes.get('incrementsSlider1', None))
        if attributes.label1:
            CCLabel(parent=self.slider1, fontsize=12, align=uiconst.BOTTOMRIGHT, text=attributes.label1, letterspace=2, top=-10, uppercase=True, left=12)
        settings2 = {'radius': 72,
         'textureWidth': 82,
         'textureHeight': 118,
         ...
         'centerOffset': (0, 36)}
        self.slider2 = CharCreationOvalSlider(parent=self, align=uiconst.BOTTOMRIGHT, sliderSetting=settings2, OnSetValue=attributes.get('OnSetValue2', None), setValue=attributes.get('setValue2', 0.0), invertSlider=attributes.get('invertSlider2', True), modifierName=attributes.modifierName2, increments=attributes.get('incrementsSlider2', None))
        if attributes.label2:
            CCLabel(parent=self.slider2, fontsize=12, align=uiconst.BOTTOMLEFT, text=attributes.label2, letterspace=2, top=-10, uppercase=True, left=12)
        settings3 = {'radius': 72,
         'textureWidth': 164,
         'textureHeight': 48,
         ...
         'centerOffset': (82, 82)}
        self.slider3 = CharCreationOvalSlider(parent=self, align=uiconst.TOPLEFT, sliderSetting=settings3, OnSetValue=attributes.get('OnSetValue3', None), setValue=attributes.get('setValue3', 0.0), invertSlider=attributes.get('invertSlider3', False), modifierName=attributes.modifierName3, increments=attributes.get('incrementsSlider3', None))
        if attributes.label3:
            CCLabel(parent=self.slider3, fontsize=12, align=uiconst.CENTERTOP, text=attributes.label3, letterspace=2, top=-20, uppercase=True)


class CharCreationOvalSlider(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        if attributes.get('invertSlider', False):
            self.InvertSlider()
        self.marker = Sprite(parent=self, align=uiconst.TOPLEFT, pos=(0, 0, 16, 16), texturePath='res:/UI/Texture/CharacterCreation/OvalSliderHandle.dds', ignoreSize=True)
        self.marker.OnMouseDown = self.OnMarkerMouseDown
        self.marker.OnMouseEnter = self.OnMarkerMouseEnter
        self.marker.OnMouseExit = self.OnMarkerMouseExit
        self.ghostMarker = Sprite(parent=self, align=uiconst.TOPLEFT, pos=(0, 0, 16, 16), texturePath='res:/UI/Texture/CharacterCreation/OvalSliderHandle.dds', ignoreSize=True, state=uiconst.UI_DISABLED, color=(1.0, 1.0, 1.0, 0.5))
        bg = Sprite(parent=self, align=uiconst.TOPLEFT, texturePath=texturePath, ignoreSize=True, state=uiconst.UI_DISABLED, color=(1.0, 1.0, 1.0, 1.0), pos=(0,
         0,
         textureWidth,
         textureHeight))
        self.DrawIncrements()
        self.DrawFill()
        self.fill = Container(parent=self, align=uiconst.BOTTOMLEFT, clipChildren=True, pos=(0,
         0,
         textureWidth,
         textureHeight))
    ...
    def OnMarkerMouseEnter(self, *args):
        self.marker.texture.resPath = 'res:/UI/Texture/CharacterCreation/OvalSliderHandle_MO.dds'

    def OnMarkerMouseExit(self, *args):
        self.marker.texture.resPath = 'res:/UI/Texture/CharacterCreation/OvalSliderHandle.dds'

    def OnMouseDown(self, *args):
        if uicore.uilib.leftbtn:
            sm.StartService('audio').SendUIEvent(unicode('ui_icc_button_mouse_down_play'))
            self.UpdateMarkerPosition(jump=True)
            self.updateTimer = timerstuff.AutoTimer(10, self.UpdateSlider)
    ...
    def UpdateSlider(self, *args):
        if not uicore.uilib.leftbtn:
            ...
            self.ghostMarker.state = uiconst.UI_HIDDEN
            self.marker.texture.resPath = 'res:/UI/Texture/CharacterCreation/OvalSliderHandle.dds'
            sm.StartService('audio').SendUIEvent(unicode('ui_icc_button_mouse_down_play'))
            self._OnSetValue(self, self._value)
            return
        self.marker.texture.resPath = 'res:/UI/Texture/CharacterCreation/OvalSliderHandle_MD.dds'
    ...
    def DrawIncrements(self):
        ...
        for i, step in enumerate(useIncrements):
            ...
            icon = eveIcon.Icon(parent=self, pos=(left,
             top,
             8,
    ...
    def DrawFill(self):
        ...
        while startAngle < fromAngle + rotationRange + math.pi:
            ...
            fillDot = Sprite(parent=self, align=uiconst.TOPLEFT, pos=(left,
             top,
             16,
    ...
    def MoveMarkerByAngle(self, radianAngle, nonIncrementAngle = None):
        ...
        cos = math.cos(radianAngle + math.pi)
        sin = math.sin(radianAngle + math.pi)
        left = int(radius * cos) + ox - self.marker.width / 2
        top = int(radius * sin) + oy - self.marker.height / 2
        self.marker.SetPosition(left, top)
        if nonIncrementAngle is not None:
            cos = math.cos(nonIncrementAngle + math.pi)
            sin = math.sin(nonIncrementAngle + math.pi)
            left = int(radius * cos) + ox - self.ghostMarker.width / 2
            top = int(radius * sin) + oy - self.ghostMarker.height / 2
            self.ghostMarker.SetPosition(left, top)
            self.ghostMarker.state = uiconst.UI_DISABLED
        else:
            self.ghostMarker.state = uiconst.UI_HIDDEN
