#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\camera\baseCamera.py
import logging
import math
import blue
...
from eve.client.script.ui.camera.cameraUtil import GetBall, GetBallPosition, GetCameraInertiaMultiplier, GetDurationByDistance, IsInfVector3, IsNanVector3
import geo2
import trinity
import uthread
...
class Camera(object):
    ...
    def __init__(self):
        ...
        self.on_update = Signal('on_update')
        self.on_orbit = Signal('on_orbit')
        self.on_orbit_end = Signal('on_orbit_end')
        self.on_eye_position_changed = Signal('on_eye_position_changed')
    ...
    def _UpdateRotateOffset(self):
        if self._rotateOffset != (0.0, 0.0):
            offset = self._GetRotateOffset()
            self._AddToAtOffset(offset)

    def _GetRotateOffset(self):
    ...
    def UpdateProjection(self):
        ...
        if fov <= 0.0:
            ...
            logger.error('Camera: FOV Set to <= 0.0')
        offset = self.GetVerticalOffset() or self.GetCenterOffset()
        if offset:
    ...
    def _UpdateProjectionFov(self, aspectRatio, fov):
        fov *= self.GetProjectionFovConst(aspectRatio)
        self.projectionMatrix.PerspectiveFov(fov, aspectRatio, self._nearClip, self._farClip)

    def GetProjectionFovConst(self, aspectRatio):
        if aspectRatio > self.kMaxAspectRatio:
            return self.kMaxAspectRatio / aspectRatio
        else:
            return 1.0

    def _UpdateProjectionOffset(self, aspectRatio, fov):
        tanFov = math.tan(fov / 2.0)
        k = self.GetProjectionFovConst(aspectRatio)
        dX = k * aspectRatio * self._nearClip * tanFov
        dY = k * self._nearClip * tanFov
        verticalOffset = self.GetVerticalOffset()
        horizontalOffset = self.GetCenterOffset()
        left = -dX + dX * horizontalOffset
        right = dX + dX * horizontalOffset
        top = dY + dY * verticalOffset
        bottom = -dY + dY * verticalOffset
    ...
    def _SetAtOffset(self, offset):
        if offset is not None and (IsInfVector3(offset) or IsNanVector3(offset)):
            raise DestinyBallInvalid('Camera: Attempting to assign an invalid atOffset value: %s, %s' % (repr(offset), self))
        self._atOffset = offset
    ...
    def _ClampZoomPropLinear(self, ret):
        return max(0.0, min(ret, 1.0))

    def GetMinZoomProp(self):
        if FloatCloseEnough(self.minZoom, 0.0):
            return 1.0
    ...
    zoomLinear = property(GetZoomLinear, SetZoomLinear)
    ...
    def StopUpdateThreads(self, skipFov = False):
        self.zoomTarget = None
        self.panTarget = None
        self.orbitTarget = None
        self.rotateTarget = None
    ...
    def GetZoomDistance(self):
        ...
        if math.isinf(dist):
            ...
            if ballpark:
                egoBall = ballpark.ego
            wannaBeEgo = sm.GetService('gameui').wannaBeEgo
            logger.warning('Error: Infinite camera distance to %s/%s: %s, %s' % (str(egoBall),
             wannaBeEgo,
             repr(dist),
             self))
            return 1.0
    ...
    def GetZoomProportionLinear(self):
        ...
        return self._ClampZoomPropLinear(zoomProp)
    ...
    def GetZoomProportionByZoomDistanceLinear(self, zoomDist):
        if FloatCloseEnough(self.minZoom, self.maxZoom):
            return 0.0
    ...
    def SetMaxZoom(self, value):
        if value >= self.minZoom:
            logger.error('Camera: Cannot set maxZoom to a value larger than minZoom: value=%s, minZoom:%s', value, self.minZoom)
        else:
            self.maxZoom = value

    def SetMinZoom(self, value):
        if value <= self.maxZoom:
            logger.error('Camera: Cannot set minZoom to a value smaller than maxZoom: value=%s, maxZoom:%s', value, self.maxZoom)
        else:
            self.minZoom = value

    def SetMinMaxZoom(self, minZoom, maxZoom):
        if minZoom <= maxZoom:
            logger.error('Camera: Cannot set minZoom to a value smaller than maxZoom: minZoom=%s, maxZoom=%s', minZoom, maxZoom)
    ...
    def TransitTo(self, atPosition = None, eyePosition = None, duration = 1.0, smoothing = 0.1, numPoints = 1000, timeOffset = 0.0, callback = None, sleep = False):
        self.Transit(self.atPosition, self.eyePosition, atPosition, eyePosition, duration, smoothing, numPoints, timeOffset, callback, sleep)

    def Transit(self, atPos0, eyePos0, atPos1, eyePos1, duration = 1.0, smoothing = 0.1, numPoints = 1000, timeOffset = 0.0, callback = None, sleep = False):
        ...
        uicore.animations.MorphVector3(self, '_eyeTransitOffset', self._eyeTransitOffset, (0, 0, 0), duration=duration, timeOffset=timeOffset, callback=self.OnTransitEnd, sleep=sleep)
        self.SetTransitDuration(duration)
    ...
    def SetTransitDuration(self, duration):
    ...
    def SetEyePosition(self, value):
        if IsNanVector3(value) or IsInfVector3(value):
            ...
            self.on_eye_position_changed(value)
    ...
    def GetRoll(self):
        viewDir = geo2.Vec3Direction(self.atPosition, self._eyePosition)
        right = geo2.Vec3Normalize(geo2.Vec3Cross(viewDir, (0, 1, 0)))
        cosTheta = geo2.Vec3Dot(self.upDirection, geo2.Vec3Normalize(geo2.Vec3Cross(right, viewDir)))
    ...
    def EnforceMaxZoom(self):
        if self.GetZoomDistance() > self.minZoom:
            self.SetZoom(1.0)

    def __repr__(self):
        return '<%s at %s. eyePos=%s, atPos=%s, eyeOffset=%s, atOffset=%s, atTransitOffset =%s, eyeAndAtOffset=%s, minZoom=%s, maxZoom=%s, isActive=%s, fov=%s>' % (self.__class__.__name__,
         id(self),
         repr(self._eyePosition),
         repr(self._atPosition),
         repr(self._eyeOffset),
         repr(self._atOffset),
         repr(self._atTransitOffset),
