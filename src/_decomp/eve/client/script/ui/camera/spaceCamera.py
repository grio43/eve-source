#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\camera\spaceCamera.py
...
class SpaceCamera(CameraBase):
    ...
    def ResetConfiguration(self):
        ...
        self.noiseScaleCurve = trinity.Tr2CurveScalar()
        self.noiseScaleCurve.AddKey(0.0, 0.0, trinity.Tr2CurveInterpolation.LINEAR)
        self.noiseScaleCurve.AddKey(0.5, -2.2945609, trinity.Tr2CurveInterpolation.LINEAR)
        self.noiseScaleCurve.AddKey(5.0, 0.0, trinity.Tr2CurveInterpolation.LINEAR)
        self.zoomCurve = trinity.Tr2CurveScalar()
        self.zoomCurve.AddKey(0.0, evecamera.FOV_MAX, trinity.Tr2CurveInterpolation.HERMITE, 0.0, 1.0, trinity.Tr2CurveTangentType.FREE_SPLIT)
        self.zoomCurve.AddKey(0.224, evecamera.FOV_MAX, trinity.Tr2CurveInterpolation.HERMITE, 0.0, -7.0, trinity.Tr2CurveTangentType.FREE_SPLIT)
        self.zoomCurve.AddKey(0.45, evecamera.FOV_MAX, trinity.Tr2CurveInterpolation.HERMITE, 0.0, -9.0, trinity.Tr2CurveTangentType.FREE_SPLIT)
        self.zoomCurve.AddKey(0.6, evecamera.FOV_MAX, trinity.Tr2CurveInterpolation.HERMITE, 0.0, 20.0, trinity.Tr2CurveTangentType.FREE_SPLIT)
        self.zoomCurve.AddKey(0.0, evecamera.FOV_MAX, trinity.Tr2CurveInterpolation.HERMITE, 0.0, 1.0, trinity.Tr2CurveTangentType.FREE_SPLIT)
    ...
    def DoBallRemove(self, ball, slimItem, terminal):
        if session.shipid is not None:
            ...
            if lookingAtID is not None and ball.id == lookingAtID:
                if ball.destructionEffectId != destructionEffectType.NONE and ball.id != session.shipid:
                    uthread.new(self._HandleTargetKilled, ball)
                else:
                    uthread.new(self._AdjustLookAtTarget, ball)
            elif self.cameraInterestID == ball.id and ball.destructionEffectId is not destructionEffectType.NONE:
                uthread.new(self._HandleTargetKilled, ball)
    ...
    def _LookingAtSelf(self):
        ...
        if scene and scene.cameraAttachments.FindByName('dustfield'):
            scene.cameraAttachments.FindByName('dustfield').display = True
        self.checkDistToEgoThread = None
    ...
    def _IsLookingAtOtherOK(self, item):
        ...
        if not obs and item.surfaceDist > GetCameraMaxLookAtRange():
            ShowQuickMessage(localization.GetByLabel('UI/Camera/OutsideLookingRange'))
            return False
        return True
    ...
    def _LookingAtOther(self):
        ...
        if scene is not None:
            if scene.cameraAttachments.FindByName('dustfield'):
                scene.cameraAttachments.FindByName('dustfield').display = False
    ...
    def LookAt(self, itemID, setZ = None, resetCamera = False, smooth = True, **kwargs):
        item = sm.StartService('michelle').GetBallAndWaitForModel(itemID)
        ...
        sm.GetService('stateSvc').SetState(itemID, state.lookingAt, 1)
