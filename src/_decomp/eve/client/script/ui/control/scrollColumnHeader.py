#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\control\scrollColumnHeader.py
import eveicon
        ...
        for columnID in columns:
            ...
            if not isStretchColumn:
                ...
                if self.fixedColumns and columnID not in self.fixedColumns:
                    ...
                    scaler.cursor = uiconst.UICURSOR_LEFT_RIGHT_DRAG
            label = EveLabelSmall(parent=header, text=columnID, align=uiconst.CENTERLEFT, left=uiconst.LABELTABMARGIN + 8, state=uiconst.UI_DISABLED, maxLines=1)
            header.label = label
            if isStretchColumn:
    ...
    def UpdateActiveState(self):
        ...
        for each in self.headers:
            if each.columnID == currentActive:
                ...
                texturePath = eveicon.caret_down if currentDirection else eveicon.caret_up
                each.sortTriangle.texturePath = texturePath
