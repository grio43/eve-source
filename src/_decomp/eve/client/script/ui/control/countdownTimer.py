#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\control\countdownTimer.py
from math import pi
...
class CountdownTimer(Transform):
    default_width = 32
    default_height = 32
    ...
    def SetTimerColor(self, color):
        r, g, b, a = color
        self.circleSprite.SetRGBA(r, g, b, 0.5)
        self.halfCircleSprite.SetRGBA(r, g, b, a)
        self.cycleSprite.SetRGBA(r, g, b, a)
    ...
    def Rewind_Thread(self, resetTo, timerDuration, doAlert):
        ...
        while not self.destroyed and cycleSpeed > 0:
            if resetTo is not None:
                resetRatio = self.GetRatio(resetTo - self.GetCurrentTime(), timerDuration)
                if self.ratio >= resetRatio:
                    break
            elapsedTime = blue.os.TimeDiffInMs(startTime, self.GetCurrentTime())
