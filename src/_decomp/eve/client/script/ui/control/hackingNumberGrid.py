#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\control\hackingNumberGrid.py
import math
import random
import blue
import uthread
...
class HackingNumberGrid(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        Frame(parent=self, color=(0.7, 0.7, 0.7, 0.5), idx=0)
        for x in xrange(0, self.numCellRows):
            newRow = []
            newRowContainer = Container(parent=self, name='hackingCellRow', align=uiconst.TOTOP, state=uiconst.UI_DISABLED, pos=(0,
             0,
             self.width,
             self.cellHeight))
    ...
    def CreateCell(self, parent, height, width, backgroundColorTuple, foregroundColorTuple):
        newCell = Container(parent=parent, name='hackingCell', align=uiconst.TOLEFT, state=uiconst.UI_DISABLED, pos=(0,
         0,
         width,
         ...
         self.cellPaddingVertical))
        newCell.sr.background = Fill(parent=newCell, color=backgroundColorTuple)
        newCellFill = Container(parent=newCell, name='hackFill', align=uiconst.TOPLEFT, pos=(0,
         0,
         width,
         height), padding=(self.cellPaddingHorizontal,
         self.cellPaddingVertical,
         self.cellPaddingHorizontal,
         self.cellPaddingVertical))
        newCellFill.sr.background = Fill(parent=newCellFill, color=foregroundColorTuple)
        newCellFill.opacity = 0.0
        newCell.sr.fill = newCellFill
        newCell.sr.text = EveHeaderMedium(text=random.choice(self.cellGlyphs), parent=newCell, align=uiconst.CENTER, fontsize=self.cellTextSize, height=height, state=uiconst.UI_DISABLED)
        return newCell
