#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\control\panContainer.py
...
class PanContainer(Container):
    ...
    default_isPanRelativeToContentSize = True

    def GetCursor(self):
        if not (self.IsClampedHorizontally() and self.IsClampedVertically()):
            return UICURSOR_DRAGGABLE
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.isPanRelativeToContentSize = attributes.Get('isPanRelativeToContentSize', self.default_isPanRelativeToContentSize)
        self._scale = 1.0
    ...
    def OnMouseMove(self, *args):
        if uicore.uilib.leftbtn and not uicore.uilib.rightbtn and not uicore.IsDragging():
    ...
    def _PanUpdateThread(self):
        while True:
            ...
            dx, dy = toMove
            if self.isPanRelativeToContentSize:
                totalPanWidth = self.mainCont.width
                totalPanHeight = self.mainCont.height
            else:
                minLeft, maxLeft, minTop, maxTop = self._GetPanningConstraints()
                totalPanWidth = maxLeft - minLeft
                totalPanHeight = maxTop - minTop
            if totalPanWidth > 0:
                self.panLeft -= dx / totalPanWidth
            if totalPanHeight > 0:
                self.panTop -= dy / totalPanHeight
            blue.synchro.Yield()
