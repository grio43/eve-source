#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\control\pointerPanel.py
...
class PointerPanel(LayoutGrid):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.pointerDirection = attributes.get('pointerDirection', self.default_pointerDirection) or self.default_pointerDirection
...
def RefreshPanelPosition(pointerPanel):
    ...
    if isBlocked:
        pointer = pointerPanel.pointerDirection
    else:
...
def _TryPanelPosition(panel, interestRect, menuPointFlag = None):
    ...
    if panel.left < 0:
        xClip = -panel.left
    elif panel.left + panel.width > uicore.desktop.width:
        xClip = panel.left + panel.width - uicore.desktop.width
    if panel.top < 0:
        yClip = -panel.top
    elif panel.top + panel.height > uicore.desktop.height:
        yClip = panel.top + panel.height - uicore.desktop.height
    return (xClip, yClip)
...
def UpdatePanelPosition(panel, interestRect, menuPointFlag = None, fallbackPointFlags = None):
    ...
    xClip, yClip = _TryPanelPosition(panel, interestRect, menuPointFlag)
    if not (xClip or yClip):
        return
    cutDepth = SortTwo(xClip, yClip)
    cutDepthToFlag = {cutDepth: menuPointFlag}
    while cutDepth and fallbackPointFlags:
        tryFlag = fallbackPointFlags.pop(0)
        xClip, yClip = _TryPanelPosition(panel, interestRect, menuPointFlag=tryFlag)
        cutDepth = SortTwo(xClip, yClip)
        if cutDepth not in cutDepthToFlag:
            cutDepthToFlag[cutDepth] = tryFlag

    if cutDepth:
        shallowestCut = min(cutDepthToFlag)
        bestFlag = cutDepthToFlag[shallowestCut]
        _TryPanelPosition(panel, interestRect, menuPointFlag=bestFlag)
...
def FadeOutPanelAndClose(panel, duration = 0.2):
    ...
    if getattr(panel, 'beingDestroyedCallback', None):
        panel.beingDestroyedCallback(panel)
