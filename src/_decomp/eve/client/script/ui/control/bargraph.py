#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\control\bargraph.py
import math
import blue
import localization
import uthread
...
class BarGraph(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        XAXISHEIGHT = 15
        YAXISWIDTH = 40
        self.yAxisCont = Container(parent=self, name='yAxisCont', align=uiconst.TOLEFT, width=YAXISWIDTH, padBottom=XAXISHEIGHT + 1)
        self.xAxisCont = Container(parent=self, name='xAxisCont', align=uiconst.TOBOTTOM, height=XAXISHEIGHT)
        self.graphCont = Container(parent=self, name='graphCont', align=uiconst.TOALL)
        self.timeIndicator = Line(parent=self.graphCont, name='timeIndicator', state=uiconst.UI_HIDDEN, align=uiconst.TOPLEFT, color=timeIndicatorColor, padding=(0, 1, 0, 1), width=1)
        self.backgroundCont = Container(parent=self, name='backgroundCont', align=uiconst.TOALL, padding=(1, 2, 1, 1), clipChildren=True)
        Frame(parent=self, color=COLOR_FRAME)
        Sprite(parent=self.backgroundCont, name='backgroundStripes', align=uiconst.TOTOP, height=500, texturePath='res:/UI/Texture/Bargraph/background_stripes.dds', ignoreSize=True, color=backgroundColor)
        self.barCont = Container(parent=self.graphCont, name='barCont', align=uiconst.TOALL, padding=(2, 2, 1, 1), clipChildren=True)
    ...
    def _CreateYAxis(self):
        ...
        for i in xrange(NUMYAXISLABELS):
            ...
            YLabel(parent=self.yAxisCont, width=self.yAxisCont.width, value=value, text=FmtAmt(value * self.maxValue))

        yAxisFill = Fill(parent=self.yAxisCont, color=Color.WHITE)
        animations.FadeTo(yAxisFill, startVal=0.2, endVal=0.0, duration=0.3)
...
class Bar(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.backgroundFill = Fill(parent=self, color=Color.GetGrayRGBA(0.0, 0.5), state=uiconst.UI_HIDDEN)
        self.bar = Sprite(parent=self, align=uiconst.BOTTOMLEFT, color=self.color, texturePath='res:/UI/Texture/Bargraph/%s.dds' % self.BARHEIGHT, pos=(0,
         -self.BARHEIGHT,
         self.width,
    ...
    def OnValueChanged(self, bar, value, *args):
        ...
        color = Color().SetHSB(hue, 0.7, 0.8, 1.0).GetRGBA()
        self.bar.SetRGBA(*color)
...
class LabelText(eveLabel.EveLabelSmall):
...
class YLabel(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        Line(parent=self, color=COLOR_FRAME, align=uiconst.BOTTOMRIGHT, width=4, height=1, top=-1)


class XLabel(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        Line(parent=self, color=COLOR_FRAME, align=self.align, width=1, height=4)
