#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\control\eveImagePicker.py
import math
import blue
...
from carbon.common.script.util import logUtil as log, timerstuff
...
class ImagePicker(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        if session.role & serviceConst.ROLE_CONTENT:
        ...
        self.sr.counterParent = Container(parent=self.parent, align=uiconst.TOPLEFT, name='counterParent', state=uiconst.UI_NORMAL, hint=localization.GetByLabel('UI/CharacterCustomization/ShowActive'), pos=(7, 6, 13, 9), opacity=0.5, idx=0)
        ...
        self.sr.counterParent.sr.icon = Frame(frameConst=(ccConst.ICON_FOCUSFRAME, 15, -11), parent=self.sr.counterParent, color=ccConst.COLOR50)
        self.sr.counterParent.sr.label = CCLabel(text='', parent=self.sr.counterParent, color=ccConst.COLOR50, align=uiconst.CENTER, left=1)
        if self.DebugRoleCheck():
            self.sr.debugLabel = CCLabel(text='', parent=self, align=uiconst.CENTER, top=self.imageHeight / 2 + 8, letterspace=0, fontsize=9, color=(1.0, 1.0, 1.0, 0.5), state=uiconst.UI_NORMAL, idx=0)
    ...
    def BuildStrip(self, images, *args):
        if self.sr.slots:
            self.sr.slots.Close()
        self.sr.slots = Container(parent=self, align=uiconst.CENTERLEFT)
        self.images = images
        self.slots = []
    ...
    def AddSlot(self, idx, imageData):
        ...
        slotpar = Container(name='slotPar_%s' % idx, parent=self.sr.slots, align=uiconst.TOPLEFT, state=uiconst.UI_PICKCHILDREN, pos=(-self.imageWidth / 2 + (idx + 1) * step + specialItemPadding,
         0,
         self.imageWidth,
         self.imageHeight))
        slotpar.idx = idx
        slotpar.imageData = imageData
        slot = Container(parent=slotpar, align=uiconst.CENTER, state=uiconst.UI_NORMAL, pos=(0,
         0,
         self.imageWidth / 2,
         self.imageHeight / 2))
        slot.OnMouseDown = (self.OnSlotMouseDown, slot)
        slot.OnMouseEnter = self.OnSlotMouseEnter
        slot.OnMouseUp = (self.OnSlotMouseUp, slot)
        slot.imageData = imageData
        slot.sr.hilite = Frame(parent=slot, align=uiconst.TOALL, frameConst=HILITE, state=uiconst.UI_HIDDEN, color=(1.0, 1.0, 1.0, 1.0))
        slot.sr.hilite.padding = -6
        slot.sr.thumbnail = Sprite(parent=slot, texturePath=None, align=uiconst.TOALL, padding=(2, 2, 2, 2), state=uiconst.UI_DISABLED)
        Fill(parent=slot, padding=(2, 2, 2, 2), color=(0.0, 0.0, 0.0, 0.75))
        return slot

    def OnSlotMouseEnter(self):
    ...
    def CheckIfSlotLoaded(self, slotParent):
        ...
        if len(data) > 2 and data[2] is not None and session.role & serviceConst.ROLE_CONTENT:
            flag = Container(parent=slot, align=uiconst.TOPLEFT, padding=(2, 2, 2, 2), pos=(0, 0, 20, 20), state=uiconst.UI_DISABLED, idx=0)
            fill = Fill(parent=flag, color=(0, 1.0, 0, 0.5))
            if data[2] not in self._availableTypeIDs:
                fill.SetRGB(1.0, 0, 0)
                slot.GetMenu = (self.GetRightClickMenu, data[2])
            if getattr(uicore.layer.charactercreation.controller, 'mode', -1) == ccConst.MODE_FULLINITIAL_CUSTOMIZATION:
                Fill(parent=flag, color=(0, 0.0, 0, 1.0), padding=5, idx=0)
        if self._activeData == slot.imageData:
            slot.sr.hilite.state = uiconst.UI_DISABLED
            slot.sr.hilite.SetAlpha(1.0)
        elif slot.sr.hilite:
            slot.sr.hilite.state = uiconst.UI_HIDDEN
        slot.sr.mouseOverTimer = timerstuff.AutoTimer(33, self.CheckMouseOver, slot)
    ...
    def OnMouseDown(self, mouseBtn = 0, *args):
        ...
        self.sr.scrollTimer = timerstuff.AutoTimer(10, self.UpdateScroll)
    ...
    def GetRightClickMenu(self, typeID, *args):
        if not session.role & serviceConst.ROLE_WORLDMOD:
            return []
        return [('WM: /create this type', self.GMCreateItem, (typeID,))]

    def GMCreateItem(self, typeID, *args):
        try:
            sm.RemoteSvc('slash').SlashCmd('/create %s %s %s' % (typeID, 1, session.stationid))
            uicore.layer.charactercreation.controller.sr.step.GoToAssetMode(forcedMode=1)
