#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\control\eveSpinWheelPicker.py
import math
import uthread
import blue
...
class SpinWheelPicker(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        transform = Transform(parent=self, pos=(0,
         0,
         self.TEXTURESIZE,
         self.TEXTURESIZE), align=uiconst.CENTER, state=uiconst.UI_DISABLED, name='mainTransform')
        self.sr.mainTransform = transform
        sprite = Sprite(parent=transform, align=uiconst.TOALL, texturePath='res:/UI/Texture/spinWheelPicker_main.dds')
        self.sr.mainTransform = transform
        self.sr.marker = Fill(parent=self, align=uiconst.CENTER, pos=(0, -64, 2, 16), color=(1.0, 1.0, 1.0, 1.0), idx=0)
    ...
    def LoadOptions(self, options, activeOptionIndex = 0):
        ...
        for i, each in enumerate(self._options):
            isZero, args = each
            markerTransform = Transform(parent=self.sr.mainTransform, pos=(0,
             0,
             self.MARKERTFSIZE,
             self.MARKERTFSIZE), align=uiconst.CENTER, state=uiconst.UI_DISABLED, name='tick', rotation=angle + math.pi * 0.25, idx=0)
            sprite = Sprite(parent=markerTransform, align=uiconst.TOPRIGHT, pos=(0, 0, 16, 16), color=(0.0, 0.0, 0.0, 1.0), texturePath='res:/UI/Texture/spinWheelPicker_marker2.dds')
    ...
    def SettleOnTick(self):
        sm.StartService('audio').SendUIEvent(unicode('ui_icc_circular_slider_item_selected_play'))
        currentAngle = self.sr.mainTransform.GetRotation()
    ...
    def OnMouseDown(self, button, *args):
        if button == uiconst.MOUSELEFT:
            sm.StartService('audio').SendUIEvent(unicode('ui_icc_inventory_item_mouse_over_loop_play'))
            self._lastAngle = self.GetRotationAngleFromCenter()
            self._spinning = True
            self._didSpin = False
            self.sr._rotationTimer = timerstuff.AutoTimer(10, self.RotationTick)

    def OnMouseUp(self, button, *args):
        if button == uiconst.MOUSELEFT:
            sm.StartService('audio').SendUIEvent(unicode('ui_icc_inventory_item_mouse_over_loop_stop'))
            self._spinning = False
            if self._didSpin:
