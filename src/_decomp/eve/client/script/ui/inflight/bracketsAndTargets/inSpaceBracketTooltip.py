#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\inflight\bracketsAndTargets\inSpaceBracketTooltip.py
...
from eve.client.script.ui.inflight.spaceObjectIcon import SpaceObjectIcon
from eve.client.script.ui.shared.fleet import fleetBroadcastConst
from eveservices.menu import GetMenuService
from sensorsuite.overlay.bookmarks import BookmarkSiteData
...
class PersistentInSpaceBracketTooltip(TooltipPersistentPanel):
    ...
    def LoadTooltip(self, bracket, overlaps, boundingBox, overlapSites, *args, **kwds):
        ...
        scroll._scrollbar_vertical.padLeft = 0
        self.scroll = scroll
    ...
    def LoadEntries(self):
        ...
        self.scroll.width = self.subGrid.width + (8 if len(rowOrder) > MAXVISIBLE else 0)
...
class BracketTooltipRow(BracketTooltipRowBase):
    ...
    def ApplyAttributes(self, attributes):
        ...
        if self.bracket.slimItem:
            selected, hilited = sm.GetService('stateSvc').GetStates(self.bracket.slimItem, [state.selected, state.mouseOver])
    ...
    def UpdateIcon(self):
        ...
        if slimItem:
            ...
            self.iconObj.UpdateSpaceObjectIconColor(slimItem)
    ...
    def UpdateDynamicValues(self):
        ...
        if fleetBroadcastType:
            if fleetBroadcastType != self.fleetBroadcastType:
                ...
                icon = fleetBroadcastConst.iconsByBroadcastType[fleetBroadcastType]
...
class SensorOverlaySiteTooltipRow(BracketTooltipRowBase):
    ...
    def OnMouseDown(self, *args):
        bracket = self.GetBracket()
        if bracket:
            itemID = bracket.GetItemID()
            bookmarkInfo = bracket.data.bookmark if isinstance(bracket.data, BookmarkSiteData) else None
            GetMenuService().TryExpandActionMenu(itemID, self, siteData=bracket.data, bookmarkInfo=bookmarkInfo)
