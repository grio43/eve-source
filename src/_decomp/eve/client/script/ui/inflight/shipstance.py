#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\inflight\shipstance.py
import blue
...
class ShipStanceButtonController(object):
    ...
    def set_stance(self, stance_id, ship_id, type_id):
        if shipmode.data.ship_has_stances(type_id):
...
def set_stance(stance_id, ship_id, type_id):
    return ShipStanceButtonController().set_stance(stance_id, ship_id, type_id)


class ShipStanceButton(LeftSideButton):
    __notifyevents__ = ['OnStanceActive']
    ...
    def OnClick(self, *args):
        ...
        stanceBtnControllerClass().set_stance(self.stanceID, self.shipID, self.typeID)
