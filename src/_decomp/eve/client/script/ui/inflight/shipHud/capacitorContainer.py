#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\inflight\shipHud\capacitorContainer.py
import math
import random
...
class CapacitorContainer(Container):
    ...
    uniqueUiName = pConst.UNIQUE_NAME_CAPACITOR
    ...
    def InitCapacitor(self):
        ...
        for i in range(numcol):
            powerColumn = Transform(parent=self, name='powerColumn', pos=(0,
             0,
             colWidth,
             56), align=uiconst.CENTER, state=uiconst.UI_DISABLED, rotation=math.radians(i * -rotstep), idx=0)
    ...
    def Update(self):
        ...
        if maxCap is not None and self.initialized and self.powerCells:
            ...
            for ci, each in enumerate(self.powerCells):
                if ci >= visible:
                    each.SetRGBA(0.5, 0.5, 0.5, 0.5)
                    each.glowColor = (0, 0, 0, 1)
                    each.glowFactor = 0.0
                    each.glowExpand = 0.1
                    each.shadowOffset = (0, 0)
                else:
                    each.SetRGBA(0.125, 0.125, 0.125, 1)
