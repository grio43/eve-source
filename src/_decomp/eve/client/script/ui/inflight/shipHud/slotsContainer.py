#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\inflight\shipHud\slotsContainer.py
import random
import telemetry
...
class SlotsContainer(UiHiderMixin, Container):
    default_name = 'slotsContainer'
    uniqueUiName = pConst.UNIQUE_NAME_MODULE_SLOTS
    ...
    def ApplyAttributes(self, attributes):
        super(SlotsContainer, self).ApplyAttributes(attributes)
    ...
    def Hide(self, *args):
        super(SlotsContainer, self).Hide(*args)
        self.opacity = 0

    def Show(self, *args, **kwargs):
        super(SlotsContainer, self).Show(*args, **kwargs)
        self.opacity = 1.0
    ...
    def OnInvChange(self, item, change):
        if const.ixFlag in change or const.ixLocationID in change and item[const.ixItemID] in self.modulesByID:
    ...
    def FitSlot(self, slot, moduleInfo, charge = None, grey = 0, slotUIID = 'slot'):
        pos = (slot.width - 48) / 2
        module = ModuleButton(name='ModuleButton_%s' % moduleInfo.typeID, parent=slot, align=uiconst.TOPLEFT, width=48, height=48, top=pos, left=pos, idx=0, state=uiconst.UI_NORMAL)
