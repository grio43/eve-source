#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\inflight\squadrons\squadronManagementCont.py
...
class FighterLaunchControlCont(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        inSpaceCont = Container(name='inSpaceCont', parent=self, height=86, align=uiconst.TOTOP)
        ...
        self.stateCont = Container(name='stateCont', parent=self, height=24, align=uiconst.TOTOP, state=uiconst.UI_NORMAL)
        ...
        self.loadingGauge = Gauge(parent=self.stateCont, align=uiconst.CENTERBOTTOM, backgroundColor=(0, 0, 0, 1), color=(0.2, 0.2, 0.2, 0.5), padLeft=1, padRight=1, top=1, width=75, value=0.0, useRealTime=False)
        self.loadingGauge.display = False
        spaceColor = Color(*COLOR_INSPACE)
        spaceColor.a = 1.0
        self.launchIcon = ButtonIcon(name='launchIcon', parent=inSpaceCont, align=uiconst.CENTER, width=86, height=86, iconSize=44, texturePath='res:/UI/Texture/classes/CarrierBay/tubeLaunch_Up.png', downTexture='res:/UI/Texture/classes/CarrierBay/tubeLaunch_Down.png', hoverTexture='res:/UI/Texture/classes/CarrierBay/tubeLaunch_Over.png', iconColor=spaceColor.GetRGBA(), showGlow=False, hint=GetByLabel('UI/Inventory/Fighters/LaunchFighterHint'))
        ...
        returnColor.a = 1.0
        self.scoopIcon = ButtonIcon(parent=deckCont, name='scoopIcon', align=uiconst.CENTER, width=86, height=86, iconSize=44, texturePath='res:/UI/Texture/classes/CarrierBay/tubeRecall_Up.png', downTexture='res:/UI/Texture/classes/CarrierBay/tubeRecall_Down.png', hoverTexture='res:/UI/Texture/classes/CarrierBay/tubeRecall_Over.png', iconColor=returnColor.GetRGBA(), showGlow=False, hint=GetByLabel('UI/Inventory/Fighters/RecallFighterHint'))
        ...
        self.emptyTubeIcon = ButtonIcon(parent=deckCont, name='emptyTubeIcon', align=uiconst.CENTER, width=86, height=86, iconSize=44, texturePath='res:/UI/Texture/classes/CarrierBay/tubeOpen_Up.png', downTexture='res:/UI/Texture/classes/CarrierBay/tubeOpen_Down.png', hoverTexture='res:/UI/Texture/classes/CarrierBay/tubeOpen_Over.png', showGlow=False, hint=GetByLabel('UI/Inventory/Fighters/AddFighterHint'))
        self.emptyTubeIcon.OnDropData = self.OnDropToTube
        self.emptyTubeIcon.GetMenu = self.GetMenu
        ...
        self.shipFighterState.signalOnShipLocationChange.connect(self.OnShipLocationChange)
    ...
    def Close(self):
        ...
        self.shipFighterState.signalOnShipLocationChange.disconnect(self.OnShipLocationChange)
    ...
    def OnDropToSpace(self, dragSource, dragData):
        data = dragData[0]
        if getattr(data, 'tubeFlagID', None) == self.tubeFlagID and getattr(data, 'squadronState', None) == TUBE_STATE_READY:
            self.LaunchFightersFromTube()

    def OnDropToTube(self, dragSource, dragData):
        data = dragData[0]
        guid = getattr(data, '__guid__', None)
        if guid == 'uicls.FightersHealthGauge':
            if data.tubeFlagID == self.tubeFlagID and data.squadronState == TUBE_STATE_INSPACE:
                self.RecallFighterToTube()
        elif guid in ('xtriui.InvItem', 'listentry.InvItem'):
    ...
    def UpdateStatusUI(self, tubeStatus):
        ...
        self.loadingGauge.SetColor(stateColor.GetRGBA())
    ...
    def AbandonFighter(self):
        fighterInSpace = self.shipFighterState.GetFightersInSpace(self.tubeFlagID)
        sm.GetService('fighters').AbandonFighter(fighterInSpace.itemID)
...
class SquadronNumber(Container):
    ...
    def SetColor(self, isEmpty = False):
        if isEmpty or ShouldShowDockedUI():
