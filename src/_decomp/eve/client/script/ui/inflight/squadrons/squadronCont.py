#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\inflight\squadrons\squadronCont.py
from math import pi
...
from eve.client.script.ui.inflight.overview.overviewConst import KILOMETERS10, KILOMETERS10000000
...
from eve.client.script.ui.shared.fittingScreen.fighterTooltip import TooltipFighterWrapper
...
class SquadronCont(Container):
    ...
    default_doDamageUpdates = True

    def ApplyAttributes(self, attributes):
        Container.ApplyAttributes(self, attributes)
        self.tooltipPanelClassInfo = TooltipFighterWrapper()
        self.controller = attributes.controller
        self.tubeFlagID = attributes.tubeFlagID
        self.doDamageUpdates = attributes.get('doDamageUpdates', self.default_doDamageUpdates)
        textCont = Container(name='textCont', parent=self, align=uiconst.TOBOTTOM, top=10, height=30)
        ...
        self.fighterHealthCont = FightersHealthGauge(parent=self, align=uiconst.TOTOP, height=86, tubeFlagID=self.tubeFlagID)
        ...
        self.actionLabel = SquadronActionLabel(parent=textCont, align=uiconst.CENTERTOP, top=2, controller=self.controller)
        self.speedLabel = SquadronInfoLabel(parent=textCont, align=uiconst.CENTERBOTTOM, top=2)
        if self.controller.GetIsInSpace(self.tubeFlagID):
            self.SetSquadronInfo(self.tubeFlagID)
        self.SetSquadronAction(self.tubeFlagID)
        if self.doDamageUpdates:
...
class SquadronContEmpty(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.actionLabel = SquadronActionLabel(parent=textCont, align=uiconst.CENTERTOP, top=2, controller=self.controller)
        self.fighterCont = Container(name='fighterCont', parent=self, align=uiconst.TOTOP, height=86, state=uiconst.UI_NORMAL, pickRadius=32)
        self.fighterCont.OnMouseEnter = self.OnFighterContMouseEnter
        self.fighterCont.OnMouseExit = self.OnFighterContMouseExit
        self.fighterCont.hint = GetByLabel('UI/Inventory/Fighters/EmptyLaunchTube')
        self.fighterCont.GetTooltipPosition = self.GetTooltipPosition
        Sprite(parent=self.fighterCont, texturePath='res:/UI/Texture/classes/ShipUI/Fighters/fighterItemEmpty_Up.png', align=uiconst.CENTER, height=86, width=86, state=uiconst.UI_DISABLED)
        self.hilite = Sprite(parent=self.fighterCont, name='hilite', texturePath='res:/UI/Texture/classes/ShipUI/Fighters/fighterItemEmpty_Over.png', align=uiconst.CENTER, height=86, width=86, state=uiconst.UI_DISABLED)
    ...
    def OnFighterContMouseEnter(self, *args):
        self.hilite.display = True

    def OnFighterContMouseExit(self, *args):
        self.hilite.display = False

    def GetTooltipPosition(self):
        retLeft, retTop, retWidth, retHeight = self.fighterCont.GetAbsolute()
        return (retLeft,
         retTop + 15,
         retWidth,
         retHeight)
