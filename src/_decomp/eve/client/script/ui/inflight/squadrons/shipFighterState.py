#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\inflight\squadrons\shipFighterState.py
...
import signals
import logging
from eve.common.script.mgt.fighterConst import TUBE_STATE_READY, TUBE_STATE_EMPTY, TUBE_STATE_LANDING, TUBE_STATE_REFUELLING, TUBE_STATE_INSPACE
...
class ShipFighterState(object):
    ...
    signalOnShipLocationChange = None
    ...
    def __init__(self, fightersSvc):
        ...
        self.signalOnFighterTubeStateUpdate = signals.Signal(signalName='signalOnFighterTubeStateUpdate')
        self.signalOnFighterTubeContentUpdate = signals.Signal(signalName='signalOnFighterTubeContentUpdate')
        self.signalOnFighterInSpaceUpdate = signals.Signal(signalName='signalOnFighterInSpaceUpdate')
        self.signalOnAbilityActivationStatusUpdate = signals.Signal(signalName='signalOnAbilityActivationStatusUpdate')
        self.signalOnIncomingEwarStarted = signals.Signal(signalName='signalOnIncomingEwarStarted')
        self.signalOnIncomingEwarEnded = signals.Signal(signalName='signalOnIncomingEwarEnded')
        self.signalOnShipLocationChange = signals.Signal(signalName='signalOnShipLocationChange')

    def OnSessionChanged(self, isRemote, sess, change):
        if 'shipid' in change:
            self._UpdateStateForShip()
        if 'locationid' in change or 'structureid' in change:
            ...
            self.signalOnShipLocationChange()
    ...
    def OnFighterRemovedFromController(self, fighterID, tubeFlagID):
        ...
        sm.ScatterEvent('OnFighterRemovedFromControllerClient', fighterID)
    ...
    def _UpdateStateForShip(self):
        ...
        self.abilityTargetTracker.FlushAllTargets()
        fightersInTubes, fightersInSpace, abilityStates = self.fightersSvc.GetFightersForShip()
        ...
        for tubeFlagID, fighterItemID, fighterTypeID, squadronSize in fightersInSpace:
            ...
            self.fightersInSpaceByTube[tubeFlagID] = fighterInSpace
            self._SetTubeStatus(tubeFlagID, TUBE_STATE_INSPACE, None, None)
            self.signalOnFighterInSpaceUpdate(fighterItemID, tubeFlagID)
            self.signalOnFighterTubeStateUpdate(tubeFlagID)

        for fighterID, abilitySlotStates in abilityStates.iteritems():
            self._UpdateAbilitySlotStates(fighterID, abilitySlotStates)
    ...
    def GetAllFightersIDs(self):
        return [ f.itemID for f in self.GetAllFighters() ]

    def GetAllFighters(self):
        return self.fightersInLaunchTubes.values() + self.fightersInSpaceByID.values()

    def GetFightersForTubeID(self, tubeFlagID):
        return self.GetFightersInTube(tubeFlagID) or self.GetFightersInSpace(tubeFlagID)
    ...
    def KillEffectsFromBall(self, ballID):
        for eachTargetID, eachAttackerDict in self.incomingEwarByFighterID.iteritems():
            effectsForBall = eachAttackerDict.get(ballID, None)
            if effectsForBall:
                sourceBallID, jammingType = effectsForBall
                self.OnEwarEnd(sourceBallID, ballID, eachTargetID, jammingType)
...
class AbilityTargetTracker(object):
    ...
    def __init__(self):
        ...
        self.signalOnAbilityTargetsUpdate = signals.Signal(signalName='signalOnAbilityTargetsUpdate')
