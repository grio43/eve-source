#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\inflight\squadrons\fightersHealthGaugeCont.py
import math
...
from eve.common.script.mgt.fighterConst import DAMAGED_FIGHTER_HIGH, DAMAGED_FIGHTER_LOW, DAMAGED_FIGHTER_MEDIUM, DEAD_FIGHTER, HEALTHY_FIGHTER_SPACE, HEALTHY_FIGHTER_TUBE, HINTTEXT_BY_ROLE, TUBE_STATES_INSPACE
import evetypes
from fighters import GetMaxSquadronSize, GetSquadronRole
from fighters.client import GetSquadronSizeFromSlimItem, GetSquadronRoleResPath
        ...
        for i in xrange(1, self.squadronMaxSize + 1):
            if i <= self.squadronSize:
                ...
                if damage and damage < 0.99:
                    if i == self.squadronSize:
                        if damage < 0.33:
                            dashColor = DAMAGED_FIGHTER_HIGH
                        elif damage < 0.67:
                            dashColor = DAMAGED_FIGHTER_MEDIUM
                        else:
                            dashColor = DAMAGED_FIGHTER_LOW
                        self.noHealthyFighters -= 1
            else:
                dashColor = DEAD_FIGHTER
            dashColor = colorblind.CheckReplaceColor(dashColor)
...
class FighterHealthGaugeLine(VectorLineTrace):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.startAngle = attributes.startAngle or math.radians(138)
        self.range = attributes.range or math.radians(265)
