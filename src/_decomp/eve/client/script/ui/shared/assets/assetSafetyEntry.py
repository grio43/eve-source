#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\assets\assetSafetyEntry.py
...
class AssetSafetyEntry(BaseListEntryCustomColumns):
    ...
    def GetMenu(self):
        ...
        if session.role & ROLE_GML:
            subMenu = [(str(self.assetWrapID), blue.pyos.SetClipboardData, (str(self.assetWrapID),))]
    ...
    def MoveEjectTimeGM(self, assetWrapID, days):
        ...
        sm.GetService('objectCaching').InvalidateCachedMethodCall('structureAssetSafety', 'GetItemsInSafetyForCharacter')
        sm.GetService('objectCaching').InvalidateCachedMethodCall('structureAssetSafety', 'GetItemsInSafetyForCorp')
    ...
    def AddColumnAssetSafetyTimers(self):
        ...
        nearestItemID = self.nearestNPCStationInfo['itemID']
        stationName = cfg.evelocations.Get(nearestItemID).name
        stationText = GetShowInfoLink(self.nearestNPCStationInfo['typeID'], stationName, nearestItemID)
        self.nearestNPCStationLabel = EveLabelMedium(parent=colContainer, align=uiconst.CENTERLEFT, text=stationText, left=150, state=uiconst.UI_NORMAL)
...
class DeliverButton(Button):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.progressFill = FillThemeColored(parent=self, name='progressFill', state=uiconst.UI_DISABLED, align=uiconst.TOLEFT_PROP, colorType=uiconst.COLORTYPE_UIHILIGHT, opacity=1.0)
