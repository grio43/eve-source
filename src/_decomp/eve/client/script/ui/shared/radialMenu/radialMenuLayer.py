#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\radialMenu\radialMenuLayer.py
import math
...
class RadialMenuLayer(Transform):
    ...
    def AnimateButtonsIn(self, startingDegree):
        ...
        for eachButton in self.buttonDict.itervalues():
            ...
            radians = math.radians(degreeToUse)
            startRad = math.radians(startingDegree)
    ...
    def AddActionButton(self, counter, degree, isEmpty = False, *args, **kw):
        ...
        if actionCont is None or actionCont.destroyed:
            actionCont = ActionButtonParent(parent=self, pos=(0,
             0,
             self.buttonWidth,
             self.actionDistance * 2), buttonBackgroundOpacity=self.buttonBackgroundOpacity, degree=degree, sizeInfo=self.sizeInfo, isEmpty=isEmpty, actionKeywords=kw, menuType=self.menuType)
