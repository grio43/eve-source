#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\radialMenu\rangeCircle.py
import math
...
class RangeCircle(Transform):
    ...
    def SetRangeCircle(self, degree, percOfAllRange):
        ...
        self.rangeMeterRightSide.display = True
        self.display = True
        self.rotation = math.radians(-degree)
        halfCircle = 180
        ...
        if percOfAllRange > 0.5:
            ...
            self.rangeMeterLeftSide.rotationSecondary = math.radians(changingDegree)
        else:
            self.rangeMeterLeftSide.display = False
            self.rangeMeterRightSide.rotationSecondary = math.radians(changingDegree)
