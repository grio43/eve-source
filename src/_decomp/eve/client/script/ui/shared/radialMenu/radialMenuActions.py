#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\radialMenu\radialMenuActions.py
import math
import types
...
from eve.client.script.ui.shared.radialMenu import BROADCAST_RADIAL_MENU
from eve.client.script.ui.shared.radialMenu.broadcastRadialMenuFunctions import GetBroadcastMenuColor

class RadialMenuActionBase(Transform):
    default_left = 0
    default_top = 0
    ...
    def ApplyAttributes(self, attributes):
        ...
        iconPar = Transform(parent=self, name='iconPar', pos=(0, 3, 32, 32), state=uiconst.UI_DISABLED, align=uiconst.CENTER)
        iconPar.rotation = math.radians(self.degree)
        self.icon = GlowSprite(parent=iconPar, name='icon', pos=(0, 0, 32, 32), state=uiconst.UI_DISABLED, align=uiconst.CENTER)
        if attributes.menuType == BROADCAST_RADIAL_MENU:
            color = GetBroadcastMenuColor()
        else:
            color = SpriteThemeColored.default_fixedColor
        selectionSlice = SpriteThemeColored(parent=self, name='selectionSlice', state=uiconst.UI_DISABLED, texturePath=self.selelectedBasePath % self.degreeWidth, align=uiconst.TOALL, opacity=0.9, color=color)
        ...
        self.availableSlice = SpriteThemeColored(parent=self, name='availableSlice', state=uiconst.UI_DISABLED, texturePath=sliceTexturePath, align=uiconst.TOALL, opacity=attributes.get('buttonBackgroundOpacity', 0.8), color=color)
    ...
    def SelectButtonSlice(self, duration = 0.5):
        ...
        uicore.animations.MorphScalar(self, 'top', duration=duration, curveType=curvePoints, sleep=False)
        curvePoints = ([0.0, self.selectionSlice.opacity],
         ...
         [0.8, 1.5],
         [1, 0])
        uicore.animations.MorphScalar(self, 'opacity', duration=duration, curveType=curvePoints, sleep=True)
...
class RadialMenuRangeAction(RadialMenuActionBase):
    ...
    def ApplyAttributes(self, attributes):
        RadialMenuActionBase.ApplyAttributes(self, attributes)
        ...
        self.rangeArrow = Sprite(parent=self, name='rangeArrow', pos=(0,
    ...
    def SetEnabled(self):
        RadialMenuActionBase.SetEnabled(self)
        self.rangeArrow.opacity = 1.0

    def SetDisabled(self):
        RadialMenuActionBase.SetDisabled(self)
...
class ActionButtonParent(ActionParent):
    ...
    def ApplyAttributes(self, attributes):
        ...
        actionButton = RadialMenuAction(parent=self, name='actionButton', sizeInfo=attributes.sizeInfo, degree=attributes.degree, isEmpty=attributes.isEmpty, buttonBackgroundOpacity=attributes.buttonBackgroundOpacity, menuType=attributes.menuType, **attributes.actionKeywords)
