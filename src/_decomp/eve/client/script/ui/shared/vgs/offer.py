#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\vgs\offer.py
...
from eve.client.script.ui.shared.vgs.label import VgsLabelSmall, VgsHeaderLarge, VgsHeaderMedium, PlexPriceTag
from eve.client.script.ui.view.aurumstore.shared.offerpricing import get_offer_price_and_base_price_in_currency
...
class Offer(Container):
    ...
    def Layout(self):
        ...
        if self.price:
            offer_price, base_price = get_offer_price_and_base_price_in_currency(self.offer, OFFER_CURRENCY_PLEX)
            PlexPriceTag(parent=descriptionBox, align=uiconst.TOBOTTOM, amount=offer_price, baseAmount=base_price, padding=(self.PADDING_BIG,
             0,
             self.PADDING_BIG,
             10))
        if self.description:
...
class OfferProductList(LayoutGrid):
    ...
    def AddProduct(self, product):
        self.AddRow(rowClass=OfferProductRow, typeID=product.typeId, quantity=product.quantity, name=product.name, imageUrl=product.imageUrl, iconSize=self.iconSize)
...
class OfferProductRow(LayoutGridRow):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.imageUrl = attributes.get('imageUrl')
    ...
    def _PrepareProductLabel(self):
        ...
        VgsLabelSmall(parent=labelCont, padding=(8, 0, 4, 0), text=localization.GetByLabel('UI/VirtualGoodsStore/ItemNameAndQuantity', itemName=self.name, quantity=self.quantity))
