...
class CorpRolesNew(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.mainCont = Container(name='mainCont', parent=self, padding=4)
        self.topCont = Container(name='topCont', parent=self.mainCont, height=70, align=uiconst.TOTOP, padBottom=4)
        self.browseCont = Container(name='browseCont', parent=self.mainCont, align=uiconst.TOBOTTOM, padding=(0, 4, 0, 0), state=uiconst.UI_NORMAL)

    def Load(self, populateView = 1, *args):
        if not self.sr.Get('inited', 0):
            self.sr.inited = 1
            height = 60 * fontconst.fontSizeFactor
            ...
            self.rolesSortHeaders = VerticalScrollHeader(parent=self.roleScroll.sr.maincontainer, settingsID='memberRoles', idx=0, scroll=self.roleScroll, height=height)
            ...
            self.accessSortHeaders = VerticalScrollHeader(parent=self.accessScroll.sr.maincontainer, settingsID='memberAccess', idx=0, scroll=self.accessScroll, height=height)
            ...
            self.titlesSortHeaders = VerticalScrollHeader(parent=self.titlesScroll.sr.maincontainer, settingsID='membertitles', idx=0, scroll=self.titlesScroll, height=height)
            ...
            self.roleGroupsBtns = ToggleButtonGroup(name='roleGroupsBtns', parent=self.topCont, align=uiconst.TOBOTTOM, callback=self.ChangeState, unisize=False)
            ...
            self.searchInput = SearchInput(name='search', parent=self.topCont, maxLength=100, width=120, align=uiconst.TOPRIGHT, GetSearchEntries=self.Search, OnSearchEntrySelected=self.OnSearchEntrySelected, hintText=GetByLabel('UI/EVEMail/MailingLists/SearchByName'))
            self.searchInput.displayHistory = False
            self.prevBtn = Button(parent=self.browseCont, align=uiconst.CENTERLEFT, label=GetByLabel('UI/Common/Prev'), func=lambda : self._BrowseRoles(-1), args=())
            self.nextBtn = Button(parent=self.browseCont, align=uiconst.CENTERRIGHT, label=GetByLabel('UI/Common/Next'), func=lambda : self._BrowseRoles(1), args=())
            self.browseCont.height = self.nextBtn.height
    ...
    def _BrowseRoles(self, increment):
        self.isBrowsed = True
        self.viewFrom = self.viewFrom + increment * MEMBERS_PER_PAGE
        if self.viewFrom < 0:
            self.viewFrom = 0
    ...
    def LoadScroll(self, scroll, sortHeaders, headers, fixedHeaders, scrollList, hints = None):
        sortHeaders.CreateColumns(headers, fixedHeaders, hints)
    ...
    def LoadRolesScroll(self):
        headers, hints = self.GetRoleHeaders()
        self.LoadScroll(self.roleScroll, self.rolesSortHeaders, headers, self.GetFixedColumns(headers), self.GetRolesScrollList(), hints)
        ...
        for column in self.roleGroupings[self.GetSelectedState()].columns:
            ...
            for subColumn in subColumns:
                ...
                if role.shortDescription == columnName:
                    hints[columnName] = role.description
                    break

        return (header, hints)

    def ChangeState(self, selectedState, *args):
