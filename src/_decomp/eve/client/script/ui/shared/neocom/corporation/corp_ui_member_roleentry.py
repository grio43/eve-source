...
from eve.client.script.ui.services.corporation.corp_util import ACCESS_COL_WIDTH, BASE_COL_WIDTH, CB_OFFSET, CHECKBOX_COL_WIDTH, DIV_HANGAR, GRANTABLE_ROLES, GRANTABLE_ROLES_AT_BASE, GRANTABLE_ROLES_AT_HQ, GRANTABLE_ROLES_AT_OTHER, GetCheckStateForRole, NAME_COL_WIDTH, ROLES, ROLES_AT_BASE, ROLES_AT_HQ, ROLES_AT_OTHER
...
class BaseRoleEntry(BaseListEntryCustomColumns):
    ...
    def AddColumnName(self):
        ...
        nameText = GetByLabel('UI/Contracts/ContractsWindow/ShowInfoLink', showInfoName=self.charLabel, info=('showinfo', const.typeCharacter, self.charID))
    ...
    def AddColBase(self):
        ...
        self.baseCombo = Combo(name='myCombo', parent=col, options=self.bases, select=defaultSelectedValue, align=uiconst.CENTERLEFT, callback=self.OnBaseChange, left=4, density=Density.COMPACT)
    ...
    def GetMenu(self, *args):
        self.OnClick()
        return GetMenuService().CharacterMenu(self.node.charID)
...
class TitleEntry(BaseRoleEntry):
    ...
    def SetCbAttributes(self, checkbox, title):
        ...
        checkbox.SetCallback(self.OnCheckboxClick)
        ...
        for roleGroupID in xrange(4, 10):
            ...
            for columnName, subColumns in roleGroup.columns:
                ...
                for cbNo, (subColumnName, role) in enumerate(subColumns):
                    cbPlacement = CB_OFFSET.get(roleGroupID)
                    if roleGroupID in DIV_HANGAR:
                        cbPlacement += cbNo
...
class VerticalScrollHeader(Container):
    ...
    def CreateColumns(self, columns, fixedColumns = None, hints = None):
        hints = hints or {}
        ...
        if columns:
            for columnID in columns:
                ...
                if columnID in hints:
                    header.hint += '<br>%s' % hints[columnID]
