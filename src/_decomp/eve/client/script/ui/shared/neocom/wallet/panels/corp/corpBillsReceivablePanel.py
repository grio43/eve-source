#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\neocom\wallet\panels\corp\corpBillsReceivablePanel.py
import blue
...
class CorpBillsReceivablePanel(Container):
    default_name = 'CorpBillsReceivablePanel'
    __notifyevents__ = ['OnCorpWalletBillsChanged']

    def ApplyAttributes(self, attributes):
        super(CorpBillsReceivablePanel, self).ApplyAttributes(attributes)
        sm.RegisterNotify(self)
        self.scroll = Scroll(parent=self, id='CorpBillsReceivablePanel')
    ...
    def GetBillsReceivable(self, ownerID):
        if IsCorporation(ownerID):
            return sm.RemoteSvc('billMgr').GetCorporationBillsReceivable()
        if IsAlliance(ownerID):
            return sm.GetService('alliance').GetBillsReceivable()
