#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\market\sellItemEntry.py
from carbon.common.script.util.timerstuff import AutoTimer
from carbonui import const as uiconst, Density
...
class SellItemContainer(BuySellItemContainerBase):
    ...
    def ApplyAttributes(self, attributes):
        self.isReadOnly = attributes.isReadOnly
        ...
        self.brokersFee = 0.0
        self.sccSurcharge = 0.0
        self.rawBrokerFeePercentage = 0.0
        self.salesTax = 0.0
        self.totalSum = 0.0
        ...
        self.rightCont = Container(name='rightCont', parent=self, align=uiconst.TORIGHT_PROP, width=0.3)
        self.priceCont = Container(name='priceCont', parent=self, align=uiconst.TORIGHT_PROP, width=0.22)
        self.qtyCont = Container(name='qtyCont', parent=self, align=uiconst.TORIGHT_PROP, width=0.15)
        self.itemCont = Container(name='itemCont', parent=self, align=uiconst.TORIGHT_PROP, width=0.33)
        self.deleteCont = Container(name='deleteCont', parent=self.itemCont, align=uiconst.TORIGHT, width=24)
        self.totalCont = ContainerAutoSize(name='totalCont', parent=self.rightCont, align=uiconst.CENTERRIGHT, padRight=4)
    ...
    def OnDurationChanged(self, duration):
        if self.IsImmediateOrder():
            self.priceEdit.DeactivateTickAlignment()
            self.priceEdit.SetValue(self.bestPrice)
        else:
            self.priceEdit.ActivateTickAlignment()
        self.OnChange()
    ...
    def DrawQty(self):
        ...
        self.qtyEdit = SingleLineEditInteger(name='qtyEdit', parent=self.qtyCont, align=uiconst.TOTOP, top=8, padLeft=4, minValue=1, maxValue=qty, density=Density.COMPACT)
        self.qtyEdit.SetValue(qty)
        self.qtyEdit.OnChange = self.OnChange
        self.qtyEdit.hint = GetByLabel('UI/Common/Quantity')

    def DrawPrice(self):
        self.priceEdit = SingleLineEditMarketPrice(name='priceEdit', parent=self.priceCont, align=uiconst.TOTOP, top=8, padLeft=8, isBuy=False, isTickActivated=not self.IsImmediateOrder(), readonly=self.isReadOnly, density=Density.COMPACT)
        if self.IsImmediateOrder():
            self.priceEdit.SetValue(self.bestPrice)
        else:
            self.priceEdit.SetValue(ticks.increment_to_next_tick(self.bestPrice))
        self.priceEdit.OnChange = self.OnChange
        self.priceEdit.hint = GetByLabel('UI/Market/MarketQuote/AskPrice')

    def DrawDelta(self):
        self.deltaContainer = SellDeltaContainer(parent=self.totalCont, delta=self.GetDelta(), func=self.OpenMarket, top=16, align=uiconst.TOPRIGHT)
    ...
    def GetBestMatchText(self):
        ...
        if 1 < self.bestBid.minVolume <= self.bestBid.volRemaining:
    ...
    def UpdateBrokersFee(self):
        feeInfo = self.quoteSvc.GetBrokersFeeInfo(self.stationID, None, self.totalSum, self.brokersFeePerc, None)
        surchargeFeeInfo = self.quoteSvc.GetSccSurchargeFeeInfo(self.stationID, None, self.totalSum)
        feeAmount = feeInfo.amt
        self.brokersFee = feeAmount
        self.rawBrokerFeePercentage = feeInfo.rawPercentage
        self.sccSurcharge = surchargeFeeInfo.sccSurchargeAmt
    ...
    def UpdateTotalSumLabel(self, totalSum, sumToDisplay):
        ...
        if sumToDisplay != totalSum:
            ...
            self.ChangeTotalStrikethroughDisplay(display=True)
        else:
    ...
    def ConstructPriceAmountWarning(self):
        ...
        self.priceAmountWarning.SetRGBA(1.0, 0.3, 0.3, 0.8)
    ...
    def ConstructTotalStrikethroughLine(self):
        if self.totalStrikethroughLine:
            return
        self.totalStrikethroughLine = Line(parent=self.totalCont, align=uiconst.TOPRIGHT, pos=(2, 9, 0, 1), idx=0, color=(1, 1, 1, 0.8))
