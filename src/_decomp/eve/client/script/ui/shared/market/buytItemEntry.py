#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\market\buytItemEntry.py
...
class BuyItemContainer(BuySellItemContainerBase):
    ...
    def DrawQty(self):
        ...
        qty = self.qty
        self.qtyEdit = SingleLineEditInteger(name='qtyEdit', parent=self.qtySubCont, align=uiconst.TOTOP, padLeft=4, minValue=1, setvalue=qty, OnChange=self.OnChange, hint=GetByLabel('UI/Common/Quantity'))
    ...
    def OnMouseWheelForQtyEdit(self, *args):
        if uicore.registry.GetFocus() != self.qtyEdit:
            return
        SingleLineEditInteger.OnMouseWheel(self.qtyEdit, *args)
    ...
    def DrawDelta(self):
        self.deltaContainer = BuyDeltaContainer(parent=self.totalCont, delta=self.GetDelta(), func=self.OpenMarket, align=uiconst.TOPRIGHT, top=16)
    ...
    def DrawItem(self):
        ...
        self.icon = ItemIcon(parent=iconInnerCont, typeID=self.typeID, state=uiconst.UI_DISABLED, align=uiconst.CENTER, pos=(0, 0, 32, 32))
    ...
    def OnMouseEnter(self, *args):
        BuySellItemContainerBase.OnMouseEnter(self, *args)
        self.icon.OnMouseEnter()

    def OnMouseExit(self, *args):
        BuySellItemContainerBase.OnMouseExit(self, *args)
