#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\maps\browserwindow.py
import blue
import uthread
import trinity
...
from eveuniverse.solar_systems import is_map_browser_suppressed
...
class MapBrowserWnd(Window):
    ...
    def ApplyAttributes(self, attributes):
        ...
        closeX = eveIcon.Icon(parent=self.sr.main, icon='ui_38_16_220', align=uiconst.TOPRIGHT, top=-1, idx=0)
    ...
    def ProcessSessionChange(self, isRemote, session, change):
        if is_map_browser_suppressed(session.locationid):
    ...
    def GetMenu(self):
        ...
        if self.GetAlign() == uiconst.TOLEFT:
            m.append((MenuLabel('UI/Map/MapBrowser/AlignRight'), self.ChangeAlign, ('right',)))
        else:
            m.append((MenuLabel('UI/Map/MapBrowser/AlignLeft'), self.ChangeAlign, ('left',)))
        return m
    ...
    def OnDirectionalScannerAngleChanged(self, angle):
        self.SetTempAngle(angle)
    ...
    def MapScaler(self, where):
        parent = Container(parent=where, align=uiconst.TOBOTTOM, height=14)
        eveLabel.EveLabelSmall(text=localization.GetByLabel('UI/Map/MapBrowser/ZoomLevel'), parent=parent, left=0, top=-12, width=100, color=(1.0, 1.0, 1.0, 0.5), state=uiconst.UI_NORMAL)
        for level in (1, 2, 4):
            sub = Container(parent=parent, align=uiconst.TOLEFT, width=24, state=uiconst.UI_NORMAL)
            sub.OnClick = (self.ChangeZoomLevel, sub, level)
            parent.width += sub.width
            Frame(parent=sub, color=(1.0, 1.0, 1.0, 0.5))
            txt = eveLabel.EveLabelSmall(text='%sx' % level, parent=sub, align=uiconst.TOALL, left=6, top=2, state=uiconst.UI_DISABLED)
            if settings.user.ui.Get('mapbrowserzoomlevel', 1) == level:
                Fill(parent=sub, padding=(1, 1, 1, 1))

    def ChangeZoomLevel(self, btn, level, *args):
        for each in btn.parent.children:
            FlushList(each.children[5:])

        Fill(parent=btn, padding=(1, 1, 1, 1))
