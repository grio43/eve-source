#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\maps\map2D.py
import sys
import math
from math import pi
import evetypes
...
from carbonui.primitives import vectorline, vectorarc
from carbonui.primitives.container import Container
from carbonui.primitives.polygon import Polygon
from carbonui.primitives.sprite import Sprite
from carbonui.primitives.transform import Transform
from carbonui.uicore import uicore
from carbonui.util.various_unsorted import FlushList, SortListOfTuples
...
class Map2D(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.sr.marks = None
        self.Reset()
        self.overlays = Container(name='overlays', parent=self, clipChildren=1, pos=(0, 0, 0, 0))
        self.sr.areas = Container(name='areas', parent=self, clipChildren=1, pos=(0, 0, 0, 0))
        self.hilite = Sprite(parent=self.overlays, pos=(0, 0, 16, 16), color=(1.0, 1.0, 1.0, 0.4), name='hilite', state=uiconst.UI_HIDDEN, texturePath='res:/UI/Texture/Shared/circleThin16.png', align=uiconst.RELATIVE)
        self.imhere = Container(name='imhere', parent=self.overlays, state=uiconst.UI_HIDDEN, align=uiconst.TOPLEFT, width=16, height=16)
        circle = Sprite(parent=self.imhere, idx=0, pos=(0, 0, 16, 16), color=(1.0, 0.0, 0.0, 1.0), name='imhere_sprite', texturePath='res:/UI/Texture/Shared/circleThin16.png', align=uiconst.RELATIVE)
        self.destination = Sprite(parent=self.overlays, pos=(0, 0, 16, 16), color=(1.0, 1.0, 0.0, 1.0), state=uiconst.UI_HIDDEN, name='destination', texturePath='res:/UI/Texture/Shared/circleThin16.png', align=uiconst.RELATIVE)
        self.sprite = Container(name='mapsprite', parent=self, align=uiconst.TOALL, clipChildren=1)
        self.bgSprite = None
        self.dragging = 0
        self.ditherIn = 1
        self.dragAllowed = 0
    ...
    def SetInfoMode(self):
        ...
        FlushList(self.imhere.children[1:])
    ...
    def _MarkAreas(self, areas):
        ...
        for area in areas:
            ...
            mark = Sprite(parent=self.sr.areas, name='area', left=int(int(x * size) - mark.width / 2), top=int(int(y * size) - mark.height / 2 + 1), width=int(rad * size * 2), height=int(rad * size * 2), state=uiconst.UI_DISABLED, texturePath='res:/UI/Texture/circle_full.png', color=(1.0, 0.2, 0.2, 1.0))
            mark.sr.x = x
            mark.sr.y = y
            mark.sr.rad = rad
    ...
    def _SetMarks(self, marks):
        if not self.IsUnder(uicore.desktop):
            return
        for i in xrange(0, len(marks), 4):
            ...
            if self.sr.marks is None:
                self.sr.marks = Container(name='marks', parent=self, align=uiconst.TOALL, pos=(0, 0, 0, 0), idx=0, state=uiconst.UI_DISABLED)
            mark = Sprite(parent=self.sr.marks, name='area', left=x - mark.width / 2, top=y - mark.height / 2 + 1, width=128, height=128, state=uiconst.UI_PICKCHILDREN, texturePath='res:/UI/Texture/circle_full.png', color=(1.0, 1.0, 1.0, 0.21))
            if hint:
                EveLabelMedium(text=hint, parent=self.sr.marks, left=mark.left + mark.width, top=mark.top + 2, width=min(128, max(64, size - mark.left - mark.width)), state=uiconst.UI_NORMAL)
    ...
    def Draw(self, ids, idlevel, drawlevel, needsize, sprite = None, solarsystem = None, lineColors = None, dotColor = None, dotSizeMultiplier = None):
        _settings = (ids,
         idlevel,
         drawlevel,
         needsize)
        ...
        for x, y, dotrad, cordtype, col in cords.itervalues():
            if cordtype == 0:
                dotrad = dotrad / 2
            if dotSizeMultiplier is not None:
                dotrad = dotrad * dotSizeMultiplier
            Sprite(parent=sprite, align=uiconst.TOPLEFT, left=x - dotrad, top=y - dotrad, width=2 * dotrad, height=2 * dotrad, texturePath='res:/UI/Texture/classes/graph/point.png', state=uiconst.UI_DISABLED, color=dotColor)

        done = []
        i = 0
        lineWidth = 1.0
        if lineColors is None:
            lineColors = [(0, 0.7, 0, 1), (0, 0, 1, 1), (1, 0, 0, 1)]
        for jumptype in connections:
            for pair in jumptype:
                ...
                if fr in cords and to in cords:
                    color = lineColors[i]
                    vectorline.VectorLine(parent=sprite, translationFrom=cords[fr], widthFrom=lineWidth, translationTo=cords[to], widthTo=lineWidth, colorFrom=color, colorTo=color, align=uiconst.TOPLEFT)
    ...
    def CheckMyLocation(self, solarsystem = None):
        ...
        if self.drawlevel < DRAW_SOLARSYSTEM_INTERIOR or eve.session.stationid:
            ...
            self.updatemylocationtimer = timerstuff.AutoTimer(100, self.UpdateMyLocation)
    ...
    def SetSelected(self, ids):
        ...
        for id in ids:
            ...
            if x is not None and y is not None:
                newsel = Container(parent=self.overlays, name='selected', align=uiconst.TOPLEFT, width=16, height=16, state=uiconst.UI_DISABLED)
                pointer = Sprite(parent=newsel, pos=(0, 0, 16, 32), state=uiconst.UI_PICKCHILDREN, texturePath='res:/UI/Texture/Shared/circlePointerDown.png', color=(1.0, 1.0, 1.0, 0.4))
                newsel.sr.x = x
                newsel.sr.y = y
    ...
    def GetMenu(self):
        ...
        if len(pick) == 1:
            ...
            m += GetMenuService().CelestialMenu(pick[0], item)
        else:
            for itemID in pick:
                item, insider = self.GetItemRecord(itemID)
                if item:
                    submenu = GetMenuService().CelestialMenu(itemID, item)
                    if len(submenu):
                        ...
                        if locationName.lower().find(groupname.lower()) >= 0:
                            groupname = ''
                        locationName = MenuLabel('UI/Map/Map2D/menuLocationEntry', {'locationName': locationName,
                         'groupName': groupname})
        ...
        if self.drawlevel == DRAW_SOLARSYSTEM_INTERIOR:
            if self.allowAbstract:
                ...
                if isAbstract:
                    lbl = MenuLabel('UI/Map/Map2D/menuShowNonAbstract')
                else:
                    lbl = MenuLabel('UI/Map/Map2D/menuShowAbstract')
    ...
    def DrawSolarsystem(self, sprite, ids, imageid, mapitems, SIZE):
        ...
        for item in mapitems:
            ...
            if item.groupID == const.groupSun:
                Sprite(parent=sprite, align=uiconst.TOPLEFT, left=SIZE / 2 - 4, top=SIZE / 2 - 4, width=8, height=8, texturePath='res:/UI/Texture/classes/graph/point.png', state=uiconst.UI_DISABLED)
        ...
        if self.allowAbstract and settings.user.ui.Get('solarsystemmapabstract', 0):
            ...
            _planets = SortListOfTuples(_planets)
            planet = _planets
        i = 1
        for item in planets:
            pos = (item.x, 0.0, item.z)
            ...
            Sprite(parent=sprite, align=uiconst.TOPLEFT, left=x - 4, top=SIZE - y - 4, width=8, height=8, texturePath='res:/UI/Texture/classes/graph/point.png', state=uiconst.UI_DISABLED, color=self.GetColorByGroupID(const.groupPlanet))
            self.AddChilds(x, y, radius, item.itemID, SIZE, sprite, cords, item)
            i += 1
        ...
        for orbit in planets:
            if orbit.itemID in cords:
                ...
                vectorarc.VectorArc(parent=sprite, align=uiconst.TOPLEFT, radius=rad, fill=False, left=center, lineWidth=1, top=center, width=SIZE, height=SIZE, color=self.GetColorByGroupID(const.groupPlanet), spriteEffect=trinity.TR2_SFX_FILL_AA)
        ...
        self.orbs = SortListOfTuples(self.orbs)
        for item in stargates:
            ...
            x += 6
            radius = 1
            Sprite(parent=sprite, align=uiconst.TOPLEFT, left=x - 4, top=SIZE - y - 4, width=8, height=8, texturePath='res:/UI/Texture/classes/graph/point.png', state=uiconst.UI_DISABLED, color=self.GetColorByGroupID(const.groupStargate))
            cords[item.itemID] = (x, SIZE - y, radius)
    ...
    def AddChilds(self, parentX, parentY, parentRad, parentID, SIZE, sprite, cords, parent, _x = None, _y = None):
        ...
        sorted = SortListOfTuples(sorted)
        if self.allowAbstract and settings.user.ui.Get('solarsystemmapabstract', 0):
            done = []
            i = 1
            xi = 0
            for child in sorted:
                ...
                Sprite(parent=sprite, align=uiconst.TOPLEFT, left=x - 2, top=SIZE - y - 2, width=4, height=4, texturePath='res:/UI/Texture/classes/graph/point.png', state=uiconst.UI_DISABLED, color=self.GetColorByGroupID(child.groupID))
    ...
    def GetColorByGroupID(self, groupID):
        col = {const.groupAsteroidBelt: (0, 0, 1),
         const.groupPlanet: (0.53, 0.53, 0.53),
         const.groupStargate: (0, 0.53, 0),
         const.groupStation: (1, 0.2, 0.2)}.get(groupID, (0.6, 0.6, 0.6))
        return col
    ...
    def PlaceBackground(self, imagepath):
        ...
        if self.bgSprite is None:
            self.bgSprite = Sprite(name='bgSprite', parent=self, align=uiconst.TOALL, state=uiconst.UI_DISABLED, color=(1.0, 1.0, 1.0, 1.0), texturePath=imagepath, filter=True)
...
class Fov(Transform):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.polygon = Polygon(parent=self, align=uiconst.TOALL, blendMode=trinity.TR2_SBM_ADD)
        self.RenderGradient()
