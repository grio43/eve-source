#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\maps\label.py
import evecamera
import telemetry
import uthread
import trinity
...
class MapLabel(Bracket):
    ...
    def Startup(self, name, itemID, typeID, tracker, cloud, mylocation = None):
        ...
        if itemID > 0:
            if typeID == const.typeConstellation:
                label = ParentLabel(text='<b>' + name + '</b>', parent=self, left=154, top=4, fontsize=12, uppercase=1, state=uiconst.UI_NORMAL)
                frame = Frame(parent=self, align=uiconst.RELATIVE, color=(1.0, 1.0, 1.0, 0.5))
            else:
                label = ParentLabel(text=name, parent=self, left=154, letterspace=1, fontsize=9, uppercase=1, align=uiconst.CENTERLEFT, state=uiconst.UI_NORMAL)
                label.top = -label.textheight / 2
        else:
            label = ParentLabel(text=name, parent=self, left=154, top=7, letterspace=1, fontsize=9, uppercase=1, state=uiconst.UI_NORMAL)
        if not mylocation:
            if self.sr.id < 0:
                label.SetRGBA(1.0, 0.96, 0.78, 1.0)
    ...
    def ShowFullAlpha(self):
        if self.preFullAlpha is None:
            self.preFullAlpha = self.children[0].opacity
        self.children[0].opacity = 1.0
    ...
    def ResetFullAlpha(self):
        if self.preFullAlpha is not None:
            self.children[0].opacity = self.preFullAlpha
    ...
    def GetMenu(self):
        ...
        if self.sr.id >= 0:
            ...
            m.append((MenuLabel('UI/Map/StarMap/CenterOnScreen'), starmap.SetInterest, (self.sr.id, 1)))
        return m
...
class ParentLabel(eveLabel.Label):
...
class TransformableLabel(object):
    ...
    def __init__(self, text, parent, size = 72, shadow = 0, hspace = 8):
        ...
        fontFamily = uicore.font.GetFontFamilyBasedOnClientLanguageID()[STYLE_DEFAULT]
