#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\industry\views\containersMETE.py
...
class BaseContainerMETE(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.previewEdit = SingleLineEditInteger(name='previewEdit', parent=mainCont, align=uiconst.CENTERRIGHT, state=uiconst.UI_HIDDEN, maxValue=self.MAXVAL, OnChange=self.OnPreviewEdit, pos=(12, 0, 43, 20))
    ...
    def OnNewJobData(self, jobData):
        ...
        self.display = True
        if jobData:
            if self.jobData.activityID == industry.REACTION:
                self.display = False
            elif self.jobData and self.jobData.blueprint.IsAncientRelic():
                self.display = False
        self.ExitPreviewMode()
...
class ContainerTE(BaseContainerMETE):
    ...
    def ExitPreviewMode(self):
        BaseContainerMETE.ExitPreviewMode(self)
        if self.jobData:
