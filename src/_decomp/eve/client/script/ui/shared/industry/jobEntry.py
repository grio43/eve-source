#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\industry\jobEntry.py
...
class JobEntry(BaseListEntryCustomColumns):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.AddColumnText(self.jobData.GetOutputLocationName())
    ...
    def GetSecurityLabel(self):
        return eveformat.solar_system_security_status(self.jobData.solarSystemID)

    @staticmethod
    def GetColumnSortValues(jobData, jumps):
        if jobData.facilityID == session.stationid or jobData.solarSystemID == session.solarsystemid:
            jumps = -1
    ...
    def GetHeaders(isPersonalJob = True):
        if isPersonalJob:
            return ('',
             ...
             localization.GetByLabel('UI/Industry/OutputLocation'),
    ...
    def GetMenu(self):
        m = GetMenuService().GetMenuFromItemIDTypeID(self.jobData.blueprintID, self.jobData.blueprintTypeID, includeMarketDetails=True)
        label = MenuLabel('UI/Industry/Facility')
        m.append((label, GetMenuService().CelestialMenu(itemID=self.jobData.facilityID)))
        if bool(session.role & ROLE_GML):
            m.append(None)
            m.append(('GM: Complete Job', sm.GetService('industrySvc').CompleteJob, (self.jobData.jobID, self.jobData.solarSystemID)))
            m.append(('GM: Cancel Job', sm.GetService('industrySvc').CancelJob, (self.jobData.jobID, self.jobData.solarSystemID)))
            m.append(None)
    ...
    def GetCopyData(cls, node):
        ...
        labelList = [StripTags(jobData.GetJobStateLabel()),
         unicode(jobData.runs),
         localization.GetByLabel(industryUIConst.ACTIVITY_NAMES.get(jobData.activityID)),
         evetypes.GetName(jobData.blueprint.blueprintTypeID),
         unicode(node.jumps),
         unicode(FmtSystemSecStatus(sm.GetService('map').GetSecurityStatus(jobData.solarSystemID), 1)[0]),
         jobData.GetFacilityName()]
        if not IsPersonalJob(jobData.ownerID):
            labelList += [jobData.GetInstallerName()]
        labelList += [jobData.GetStartDateLabel(), jobData.GetEndDateLabel()]
...
class JobStateContainer(Container):
    ...
    def ConstructLayout(self):
        ...
        if self.jobData.status == industry.STATUS_INSTALLED:
            if self.jobData.activityID == industry.MANUFACTURING:
                color = industryUIConst.COLOR_MANUFACTURING
            elif self.jobData.activityID == industry.REACTION:
                color = industryUIConst.COLOR_REACTION
            else:
                color = industryUIConst.COLOR_SCIENCE
