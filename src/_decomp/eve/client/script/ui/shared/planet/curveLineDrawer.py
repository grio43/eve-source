#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\planet\curveLineDrawer.py
import math
import trinity
import geo2
import utillib
...
class CurveLineDrawer:
    ...
    def RemoveLineSet(self, lsName):
        self.lineSets.pop(lsName, None)
    ...
    def DrawArc(self, lsName, surfacePoint1, surfacePoint2, lineWidth = 1.0, color1 = Color.WHITE, color2 = Color.WHITE):
        ls = self.GetLineSet(lsName)
    ...
    def DrawLine(self, lsName, surfacePoint1, surfacePoint2, lineWidth = 1.0, color1 = Color.WHITE, color2 = Color.WHITE):
        ls = self.GetLineSet(lsName)
        p1 = surfacePoint1.GetAsXYZTuple()
        p2 = surfacePoint2.GetAsXYZTuple()
    ...
    def DrawMesh(self, zoom):
        layers = [utillib.KeyVal(numTheta=40, numPhi=20, showAtZoom=1.0, phiSkip=3, linesetName='planetExternalGrid', lineSet=None)]
    ...
    def DrawCartesianAxis(self):
        ...
        self.DrawLine(lsName, sp(), sp(len, 0, 0), 3.0, Color.RED, Color.RED)
        self.DrawLine(lsName, sp(), sp(0, len, 0), 3.0, Color.GREEN, Color.GREEN)
        self.DrawLine(lsName, sp(), sp(0, 0, len), 3.0, Color.BLUE, Color.BLUE)
        self.SubmitLineset(lsName)
