#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\progressWnd.py
import types
import blue
import localization
import utillib
...
class ProgressWnd(ContainerAutoSize):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.sr.readprogress = utillib.KeyVal(text='', prev=0)
    ...
    def SetAbortFunc(self, func):
        ...
        if self.abortbtnpar is None:
            if func is None:
                return
            self.abortbtnpar = ButtonGroup(parent=self, align=uiconst.TOTOP, btns=[[localization.GetByLabel('UI/Commands/Abort'),
              func,
              args,
              66]])
            self.abortbtn = self.abortbtnpar.buttons[0]
        if func is None:
    ...
    def SetAbortConfirmFunc(self, abortFunc, confirmFunc):
        ...
        if self.abortconfirmbtnpar is None:
            btns = ((localization.GetByLabel('UI/Common/Yes'),
              ...
              0,
              1))
            self.abortconfirmbtnpar = ButtonGroup(parent=self, btns=btns, align=uiconst.TOTOP)
            self.confirmbtn = self.abortconfirmbtnpar.buttons[0]
            self.abortbtn = self.abortconfirmbtnpar.buttons[1]
    ...
    def CountDown(self, startTime, duration):
        while True and not self.destroyed:
            ...
            if dt > duration:
                break
            self.progressText.text = FmtDate(long((duration - dt) * 10000) + const.SEC)
            blue.pyos.synchro.SleepWallclock(10)

        if not self.destroyed:
            self.progressText.text = FmtDate(0L)
            self.Abort()
    ...
    def SetStatus(self, title, strng, portion = None, total = None, abortFunc = None, animate = 1):
        ...
        self.sr.readprogress = utillib.KeyVal(text=strng, prev=0)
        if self.progressText.text != strng:
            self.progressText.text = strng
        return 0
