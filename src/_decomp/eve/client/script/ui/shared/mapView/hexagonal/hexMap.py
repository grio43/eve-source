#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\mapView\hexagonal\hexMap.py
import math
import logging
...
from evePathfinder.stateinterface import StandardPathfinderInterface
import trinity
import pyEvePathfinder
from eve.common.script.sys.idCheckers import IsConstellation, IsRegion
from eve.client.script.ui.shared.mapView.hexagonal.hexMapUtil import GetHexMapSizeForObject, GetPlotDataForObject, PrepareCombinedMap
...
class HexMap(Container):
    ...
    def PreparePathFinder(self):
        ...
        pathMap = EveMapWrapper()
        pathMap.add_region(-1)
        pathMap.add_constellation(-2, -1)
        self.pathMap = pathMap
        pathfinderCore = pathMap.create_core()
        ...
        self.pathfinderHandler = ClientPathfinder(pathfinderCore, pathfinderState, self.avoidPathFinderState, ConvertPathFinderIDMock, ConvertPathFinderIDMock)
        ...
        if getattr(self, 'pathFindingPairs', None):
            for i in xrange(yRange * xRange):
                pathMap.add_solar_system(i, -2, -1)
    ...
    def AddToPathFinding(self, pathFindingPairs, showDebug = False, groupID = 0, debugColor = None):
        ...
        for _p1, _p2 in pathFindingPairs:
            self.pathMap.add_jump(_p1, _p2, groupID)
            self.pathMap.add_jump(_p2, _p1, groupID)
