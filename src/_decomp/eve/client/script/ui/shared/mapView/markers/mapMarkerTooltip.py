#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\client\script\ui\shared\mapView\markers\mapMarkerTooltip.py
...
class MarkerTooltipRowBase(LayoutGridRow):
    ...
    def LoadIconFromMarkerObject(self):
        if self.markerObject.IsDscanHiliteShown():
            color = COLOR_DSCAN
        else:
            color = self.markerObject.iconColor
        self.iconObj = Sprite(state=uiconst.UI_DISABLED, pos=(0, 0, 16, 16), align=uiconst.CENTER, parent=self.iconParent, texturePath=self.markerObject.texturePath, color=color)
        return self.iconObj
...
class MarkerTooltipBase(TooltipPersistentPanel):
    ...
    def LoadEntries(self):
        ...
        self.scroll.width = self.subGrid.width + (8 if len(rowOrder) > SCROLL_THRESHOLD else 0)
        totalHeight = 0
