#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\devtools\script\svc_simulator.py
...
class SimulatorService(Service):
    ...
    def Show(self):
        if self.wnd:
            self.wnd.Maximize()
            return
        self.wnd = wnd = Window.Open(windowID='Cap Simulator')
        wnd._OnClose = self.Hide
        wnd.SetCaption('Capacitor Simulator')
        wnd.SetMinSize([256, 384])
        main = Container(name='main', parent=wnd.GetChild('main'), pos=(const.defaultPadding,
         const.defaultPadding,
         const.defaultPadding,
         const.defaultPadding))
        bottom = Container(name='bottom', parent=main, align=uiconst.TOBOTTOM, height=60)
        wnd.sr.text = eveLabel.Label(text='<br><br><br><br>', parent=bottom, align=uiconst.TOALL, left=const.defaultPadding, state=uiconst.UI_NORMAL)
        wnd.sr.scroll = eveScroll.Scroll(name='attributescroll', parent=main)
        wnd.sr.scroll.sr.id = 'capsim_modulescroll'
        wnd.sr.scroll.hiliteSorted = 0
        btns = ButtonGroup(btns=[['Run Simulation',
          self.Simulate,
          (),
          None]], parent=main, idx=0, button_size_mode=ButtonSizeMode.DYNAMIC)
        self.Load()
        wnd.Maximize(1)
    ...
    def Load(self):
        ...
        scrollList = []
        for a in mods:
            e = GetFromClass(AttributeCheckbox, {'line': 1,
             'info': a,
    ...
    def Simulate(self):
        ...
        if sim.duration == const.DAY:
            ...
            t.text = 'Sustainable: %s%s</color><br>' % (colorTag, FmtTimeInterval(sim.duration, breakAt='sec'))
        i = 1
        if sim.duration < const.DAY:
