#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\devtools\script\colorThemeEditor.py
...
class ColorThemeEditor(Window):
    ...
    default_width = 600
    default_height = 510
    default_minSize = (600, 510)

    def ApplyAttributes(self, attributes):
        Window.ApplyAttributes(self, attributes)
        self.baseColorPicker = ColorPicker(parent=self.content, align=uiconst.TOTOP, label='Base Color', colorCallback=self.OnBaseColor, padding=(8, 4, 4, 20), color=sm.GetService('uiColor').GetUIColor(uiconst.COLORTYPE_UIBASE), maxValue=0.3)
        LineThemeColored(parent=self.content, align=uiconst.TOTOP)
        self.hiliteColorPicker = ColorPicker(parent=self.content, align=uiconst.TOTOP, label='Hilite Color', colorCallback=self.OnHiliteColor, padding=(8, 20, 4, 4), color=sm.GetService('uiColor').GetUIColor(uiconst.COLORTYPE_UIHILIGHT))
...
class ColorPicker(ContainerAutoSize):
    ...
    def ApplyAttributes(self, attributes):
        ...
        Label(parent=Container(parent=self, align=uiconst.TOTOP, height=Button.default_height + 8), align=uiconst.TOTOP, text=label, fontsize=20, left=14)
...
class ValueSelector(Container):
    ...
    def ApplyAttributes(self, attributes):
        ...
        self.slider = Slider(parent=self, align=uiconst.TOLEFT_PROP, width=0.95, padLeft=3, minValue=0.0, maxValue=maxValue, value=value, on_dragging=callback)
