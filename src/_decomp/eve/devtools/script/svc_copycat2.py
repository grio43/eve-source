#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\devtools\script\svc_copycat2.py
import cPickle
import os
import random
import sys
...
from eve.client.script.ui.control import eveLabel, eveScroll, eveWindowDropDownMenu
...
from eve.devtools.script import dna, spiffy, param
from eveprefs import Handler
from eveprefs.iniformat import IniIniFile
from carbon.common.script.sys.service import Service, ROLE_WORLDMOD, ROLEMASK_ELEVATEDPLAYER, ROLE_GML, ROLE_PLAYER, ROLE_HEALSELF, ROLE_SPAWN, SERVICE_RUNNING, SERVICE_START_PENDING
CFG_USEONLINE = 0
myversion = '2.4'
SERVICENAME = 'copycat'
...
def AskFile(title = 'Title goes here', body = 'Enter name of file:', setvalue = '', mustexist = True):
    while True:
        ret = utilWindows.NamePopup(caption=title, label=body, setvalue=setvalue, maxLength=256)
        if not ret:
            return None
        filename = ret
        if mustexist:
...
class CopycatService(Service):
    ...
    def igb(self, action = '', key = '', name = None):
        if not eve.session.role & ROLEMASK_ELEVATEDPLAYER:
            return
        action = action.lower()
        if action == 'open':
            dna.Popup(key, name)
    ...
    def Run(self, memStream = None):
        ...
        try:
            ...
            self.prefs = Handler(IniIniFile('copycat', INSIDERDIR))
    ...
    def SetAutoSave(self, mode):
        ...
        if mode == AUTOSAVE_TIMED:
            self.dbautosavetimer = timerstuff.AutoTimer(300000, self.SaveDatabaseIfChanged)
        else:
    ...
    def Show(self):
        ...
        wnd.sr.main = main = wnd.GetChild('main')
        top = Container(name='push', align=uiconst.TOTOP, height=14, parent=main)
        Line(parent=top, align=uiconst.TOBOTTOM)
        push = Container(name='push', parent=top, align=uiconst.TOLEFT, width=const.defaultPadding, state=uiconst.UI_DISABLED)
        Line(parent=push, align=uiconst.TORIGHT)
        for label, func in [('File', self.GetMenu_File), ('My Ship', self.GetMenu_Ship)]:
            menu = eveWindowDropDownMenu.WindowDropDownMenu(name='menu', parent=top, align=uiconst.TOLEFT, state=uiconst.UI_NORMAL)
            menu.Setup(label, func)

        Container(name='bottompush', parent=main, state=uiconst.UI_DISABLED, align=uiconst.TOBOTTOM, height=const.defaultPadding)
        c = ContainerAutoSize(name='', parent=main, state=uiconst.UI_PICKCHILDREN, align=uiconst.TOBOTTOM)
        wnd.sr.filenamelabel = eveLabel.Label(text='', parent=c, align=uiconst.TOPLEFT, padLeft=const.defaultPadding, color=None, state=uiconst.UI_NORMAL)
        self.UpdateStatus()
        wnd.sr.tabs = tabGroup.TabGroup(name='tabsparent', parent=main)
        body = Container(name='scroll', parent=main, pos=(const.defaultPadding,
        ...
        self.scaling = 0
        wnd.sr.div = Container(name='meow', parent=body, align=uiconst.TORIGHT)
        Fill(parent=body, color=(1.0, 1.0, 1.0, 0.5), width=2, align=uiconst.TORIGHT)
        wnd.sr.scroll = eveScroll.Scroll(parent=body)
        ...
        icon = Sprite(parent=main, width=16, height=16, align=uiconst.TOPRIGHT, top=14, left=const.defaultPadding, texturePath='res:/UI/Texture/classes/Browser/backIdle.png')
    ...
    def UpdateInfoPanelStuff(self, *args):
        ...
        if info.state == uiconst.UI_HIDDEN:
            ...
            b.texturePath = 'res:/UI/Texture/classes/Browser/nextIdle.png'
            self.wnd.SetMinSize((256, 220))
            self.smallwidth = self.wnd.width
            settings.user.ui.Set(UI_WSMALL, self.smallwidth)
        else:
            b.flag = b.hint = b.name = 'Hide Info Panel'
            b.texturePath = 'res:/UI/Texture/classes/Browser/backIdle.png'
            self.wnd.SetMinSize((max(400, info.width + 100), 220))
        ...
        for this in folder.content:
            ...
            if isinstance(this, CCFolder):
                ...
                node = {'raceID': raceID,
                 ...
                 'state': 0,
                 'allowCopy': 0,
                 'allowGuids': ['listentry.DNAGroup', 'listentry.DNAEntry'],
                ...
                entry = GetFromClass(DNAGroup, node)
            else:
                ...
                entry = GetFromClass(DNAEntry, node)
    ...
    def GroupAdd(self, name = None, parent = None):
        if not name:
            ret = utilWindows.NamePopup(caption='New folder', label='Enter name for new folder:', setvalue='New Folder', maxLength=64)
            if ret:
                name = ret
        if not name:
            return None
    ...
    def DoImport(self, mode):
        if mode == 'clipboard':
            self.DoStore(dnaKey=blue.clipboard.GetClipboardString())
            return
        verb = 'Import'
    ...
    def DoSave(self, mode = 'default'):
        if mode == 'default':
            ...
            ret = utilWindows.NamePopup(caption='Save database as...', label='Enter filename', setvalue=self.dbfilename, maxLength=256)
            if ret:
    ...
    def GetMenu_Ship(self):
        HaveShip = eveCfg.GetActiveShip() is not None
        ...
        s = dna.Ship(shipID=eveCfg.GetActiveShip())
        dogmaLocation = sm.GetService('clientDogmaIM').GetDogmaLocation()
        ship = dogmaLocation.GetDogmaItem(eveCfg.GetActiveShip())
        pod = False
        if HaveShip:
            if ship.groupID == 29:
                if not CanSpawn:
    ...
    def DoStore(self, dnaKey = None, name = None, shipID = None):
        if dnaKey:
            ...
            action = 'Store'
            name = template.name
        ret = utilWindows.NamePopup(caption='%s %s DNA' % (action, GetTypeName(template.typeID)), label='Enter name for this entry', setvalue=name, maxLength=64)
        if ret:
            item = CCItem()
            item.name = ret
            item.dna = dnaKey
    ...
    def DoClonePerfect(self):
        if eve.session.stationid:
            if eve.session.role & ROLE_WORLDMOD:
                ship = sm.GetService('clientDogmaIM').GetDogmaLocation().GetDogmaItem(eveCfg.GetActiveShip())
                if ship is None:
                    return
                tname = GetTypeName(ship.typeID)
                Progress('Clone %s' % tname, 'Executing copyship command...', 0, 1)
                Slash('/copyship %s 1' % eveCfg.GetActiveShip())
...
class DNAEntry(Generic):
    ...
    def GetDragData(self, *args):
        ...
        if len(nodes):
            return filter(lambda n: not isinstance(n.panel, ListGroup), nodes)
        else:
            return nodes
    ...
    def Rename(self):
        ...
        if self.sr.node.locked:
            item = item.fromItem
        ret = utilWindows.NamePopup(caption='Rename %s DNA' % self.sr.node.typename, label='Enter new name', setvalue=item.name, maxLength=32)
        if ret:
            newname = ret
