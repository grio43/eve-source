#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\devtools\script\menu_implant.py
import os
import sys
import blue
import evetypes
...
from carbon.common.script.sys.serviceConst import ROLE_WORLDMOD, SERVICE_RUNNING, SERVICE_START_PENDING
...
class ImplantService(Service):
    ...
    def ImplantMenu(self, *args):
        ...
        if len(self.implants) == 0:
            self.implants = {'armor tank': ('Armor Tank', 'IMP:20499:20501:20507:20503:20505:20509:19685:21606'),
             ...
             'low-grade amulet': ('Low-grade Amulet', 'IMP:22119:22120:22123:22121:22122:22124'),
             ...
             'high-grade amulet': ('High-grade Amulet', 'IMP:20499:20501:20507:20503:20505:20509'),
        ...
        for implant in dgm.GetItem(eve.session.charid).implants:
            if implant.implantness in head:
                if implant.typeID == head[implant.implantness]:
                    ...
                    dogmaLm.DestroyImplant(implant.itemID)
    ...
    def FullFrontalLobotomy(*args):
        ...
        dlm = dgm.GetDogmaLM()
        for implant in dgm.GetItem(eve.session.charid).implants:
            dlm.DestroyImplant(implant.itemID)
            blue.synchro.SleepWallclock(1000)
    ...
    def StoreImplants(self, *args):
        ...
        if not mygodma:
            return
        imps = SortListOfTuples([ (getattr(godma.GetType(implant.typeID), 'implantness', None), implant) for implant in mygodma.implants ])
        ret = utilWindows.NamePopup('Save Implant Set', 'Enter name for set', setvalue='New', maxLength=32)
        if not ret:
            return
        name = ret['name']
