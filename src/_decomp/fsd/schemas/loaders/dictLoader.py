#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\fsd\schemas\loaders\dictLoader.py
import ctypes
import collections
...
try:
    ...
    log.warn('Could not import pyFSD, falling back to slower python implementation!')
    hasPyFSD = False
...
class DictLoader(object):
    ...
    def values(self):
        return [ self.__GetItemFromOffset__(key, offset) for key, (offset, size) in self.footer.iteritems() ]

    def items(self):
        return [ (key, self.__GetItemFromOffset__(key, offset)) for key, (offset, size) in self.footer.iteritems() ]
