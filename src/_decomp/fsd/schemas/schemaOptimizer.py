#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\fsd\schemas\schemaOptimizer.py
import copy
import collections
import ctypes
import yaml
...
def OptionalAndVariableSizedAttributeIterator(schema, target, pathRoot):
    attributes = GetAttributesForTarget(schema, target)
    for attr in attributes:
        if IsAttributeOptional(schema, attr):
            yield attr
        elif not IsFixedSize(schema['attributes'][attr], target, pathRoot):
            yield attr


def FixedSizeAttributeIterator(schema, target, pathRoot):
    attributes = GetAttributesForTarget(schema, target)
    for attr in attributes:
        if IsAttributeOptional(schema, attr):
            continue
...
def IsFixedSize(schema, target, pathRoot):
    if 'schemaReference' in schema:
        schema = LoadReferencedSchema(schema['schemaReference'], pathRoot)
        return IsFixedSize(schema, target, pathRoot)
    ...
    if schemaType == 'object':
        ...
        for attr in attributes:
            if not IsUsedInTarget(attributes[attr], target):
                continue
            if attributes[attr].get('isOptional', False) or not IsFixedSize(attributes[attr], target, pathRoot):
                return False
...
def OptimizeObjectSchema(schema, target, pathRoot):
    ...
    for attributeName in FixedSizeAttributeIterator(schema, target, pathRoot):
    ...
    for attributeName in OptionalAndVariableSizedAttributeIterator(schema, target, pathRoot):
