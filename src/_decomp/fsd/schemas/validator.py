#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\fsd\schemas\validator.py
import re
import collections
import os
import yaml
...
def ValidateGenericId(schemaNode, o, path = 'root', state = {}):
    errors = ValidateInt(schemaNode, o, path, state)
    if not ListContainsType(errors, SchemaTypeError) and o < 0:
        errors.append(SchemaComparisonError(path, '%s: Range Mismatch - Integer Identifier should be > 0, this is %i' % (path, o)))
    return errors
...
def ValidateDogmaAttributeID(schemaNode, o, path = 'root', state = {}):
    errors = ValidateInt(schemaNode, o, path, state)
    if not ListContainsType(errors, SchemaTypeError) and o < 0:
        errors.append(SchemaComparisonError(path, '%s: Range Mismatch - dogmaAttributeIDs should be > 0, this is %i' % (path, o)))
    return errors
...
def ValidateObject(schemaNode, o, path = 'root', state = {}):
    ...
    for attrName, attrValue in schemaNode.get('attributes').iteritems():
        if _IsAttributeValueMissing(o, attrName, attrValue):
...
def _IsAttributeValueMissing(obj, attrName, attrValue):
    if attrName in obj:
        return False
    if attrValue.get('isOptional', False):
        return False
    if attrValue.get('generatedData', False):
        return False
    return True
...
builtInValdationFunctions = {'int': ValidateInt,
 ...
 'bool': ValidateBool,
 'union': ValidateUnion,
 'color': ValidateVector4,
 'dogmaAttributeID': ValidateDogmaAttributeID,
 'npcTag': ValidateGenericId,
 'deploymentType': ValidateGenericId,
 'npcEnemyFleetTypeID': ValidateGenericId,
 'groupBehaviorTreeID': ValidateGenericId,
 'npcCorporationID': ValidateGenericId,
 'spawnTableID': ValidateGenericId,
 'npcFleetCounterTableID': ValidateGenericId,
 'dungeonID': ValidateGenericId,
 'typeListID': ValidateGenericId,
 'npcFleetTypeID': ValidateGenericId,
 'metaGroupID': ValidateGenericId,
 'fsdReference': ValidateGenericId,
 'raceID': ValidateGenericId,
 'marketGroupID': ValidateGenericId,
 'ShipGroupID': ValidateGenericId,
 'certificateTemplateID': ValidateGenericId,
 'factionID': ValidateGenericId}
...
def ValidateReference(schemaNode, o, path, state):
    ...
    if not state.get('validateReferences', False):
        return []
    if 'schema' in referenceInfo and 'branchRoot' in state:
