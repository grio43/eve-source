#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\beeline\trace.py
import base64
import copy
import datetime
import functools
...
class Tracer(object):
    ...
    def __call__(self, name, trace_id = None, parent_id = None):
        try:
            ...
            if self.get_active_trace_id() and trace_id is None:
                ...
                if span:
                    log('tracer context manager started new trace, id = %s', span.trace_id)
            yield span
        except Exception as e:
            if span:
                span.add_context({'app.exception_type': str(type(e)),
    ...
    def start_trace(self, context = None, trace_id = None, parent_span_id = None, dataset = None):
        if trace_id:
            if self._trace:
                log('warning: start_trace got explicit trace_id but we are already in a trace. starting new trace with id = %s', trace_id)
    ...
    def start_span(self, context = None, parent_id = None):
        ...
        if context:
            ev.add(data=context)
        ev.add(data={'trace.trace_id': self._trace.id,
         'trace.parent_id': parent_span_id,
         'trace.span_id': span_id})
        is_root = len(self._trace.stack) == 0
        span = Span(trace_id=self._trace.id, parent_id=parent_span_id, id=span_id, event=ev, is_root=is_root)
    ...
    def finish_span(self, span):
        ...
        if span.trace_id != self._trace.id:
            log('warning: finished span called for span in inactive trace. current trace_id = %s, span trace_id = %s', self._trace.id, span.trace_id)
            return
    ...
    def propagate_and_start_trace(self, context, request):
        ...
        try:
            ...
            err = sys.exc_info()[0]
            log('error: http_trace_parser_hook returned exception: %s', sys.exc_info()[0])

        if propagation_context:
            return self.start_trace(context=context, trace_id=propagation_context.trace_id, parent_span_id=propagation_context.parent_id, dataset=propagation_context.dataset)
    ...
    def get_propagation_context(self):
        if not self._trace:
            return None
        return beeline.propagation.PropagationContext(self.get_active_trace_id(), self.get_active_span().id, self._trace.fields)
    ...
    def marshal_trace_context(self):
        if not self._trace:
            ...
            return
        return marshal_trace_context(self._trace.id, self._trace.stack[-1].id, self._trace.fields)
...
def marshal_trace_context(trace_id, parent_id, context):
    ...
    trace_context = '{};trace_id={},parent_id={},context={}'.format(version, trace_id, parent_id, trace_fields)
