#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\battleground_capture_point_ui\client\ui_container.py
import math
import weakref
import eveui
import geo2
...
class TimerRingUIContainer(InSceneContainer):
    ...
    def __init__(self, ball, scene, camera):
        ...
        super(TimerRingUIContainer, self).__init__(name='CapturePointTimerRingCont{}'.format(ball.id), state=uiconst.UI_DISABLED, scene=scene, sceneParent=scene.uiObjects, parentTransform=self.transform, trackType=self.TRACKTYPE_TRANSFORM, width=1000, height=1000, clearBackground=True, backgroundColor=(0.0, 0.0, 0.0, 0.0), renderJob=uicore.uilib.GetRenderJob(), isFullscreen=False)
