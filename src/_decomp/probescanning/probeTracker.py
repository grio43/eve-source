#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\probescanning\probeTracker.py
import logging
import geo2
import math
...
class ProbeTracker(object):
    ...
    def __init__(self, scanSvc, ScatterEvent):
        ...
        self.formationTracker.CreateFormation(formations.PINPOINT_FORMATION)
    ...
    def GetProbeTypeID(self):
        activeProbes = self.GetActiveProbes()
        if activeProbes:
            probeID = activeProbes[0]
            return self.probeData[probeID].typeID
    ...
    def MoveProbesToFormation(self, formationID, LaunchProbes, availableProbes = 0, initialPosition = None):
        ...
        excessProbeIDs = self.formationTracker.CreateFormation(formationID, self.probeData.keys(), initialPosition=initialPosition)
        if excessProbeIDs:
    ...
    def ScaleAllProbes(self, scale, scaleScanRange = True):
        ...
        self.PersistProbeFormation()
        PlaySound('msg_newscan_probe_scale_play')
        self.ScatterEvent('OnProbePositionsUpdated')

    def StartMoveMode(self):
        if self.backupProbeData is not None:
            self.logger.warning('StartMoveMode - already a backed up data for this, ignoring this call')
            return
