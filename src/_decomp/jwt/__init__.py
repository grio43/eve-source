#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\eve\common\lib\jwt\__init__.py
...
def base64_urlsafe_decode_int(data):
    if isinstance(data, six.text_type):
        data = data.encode(u'ascii')
    _d = base64.urlsafe_b64decode(bytes(data) + '==')
    arr = struct.unpack(str(u'%sB' % len(_d)), _d)
    return int(u''.join([ u'%02x' % byte for byte in arr ]), 16)
...
try:
    ...
    def prepare_key_rs256(key):
        return key


    def verify_rs256(signing_input, key, signature):
        key = rsa.PublicKey.load_pkcs1(key, format=u'PEM')
        try:
            rsa.verify(signing_input, signature, key)
    ...
    def sign_rs256(msg, key):
        return rsa.sign(msg, key, u'SHA-256')


    if u'RS256' not in verify_methods:
        prepare_key_methods.update({u'RS256': prepare_key_rs256})
        verify_methods.update({u'RS256': verify_rs256})
    if u'RS256' not in signing_methods:
        signing_methods.update({u'RS256': sign_rs256})
except ImportError:
    pass
...
def is_jwt(token):
    try:
        return token.count(u'.') == 2
    except AttributeError:
        return False


def fabricate_local_dev_user_token(tenant, user_id):
    subject = u'USER:EVE:{}'.format(user_id)
    payload = {u'sub': subject,
     u'tenant': tenant,
     u'tier': u'dev',
     u'region': u'local'}
    return encode(payload, u'dev')
