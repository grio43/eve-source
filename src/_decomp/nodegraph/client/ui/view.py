...
class BaseNodeGraphView(eveui.Container):
    node_graph_color = (0, 0, 0)
    default_align = eveui.Align.to_all

    def __init__(self, controller, **kwargs):
        super(BaseNodeGraphView, self).__init__(**kwargs)
        self._nodes = {}
    ...
    def Close(self):
        ...
        super(BaseNodeGraphView, self).Close()
    ...
    def _construct_left_top_content(self):
        eveui.Button(parent=self._left_top_container, align=eveui.Align.to_top, label='Start', func=self._controller.start_node_graph, padTop=8)
    ...
    def _construct_top(self):
        ...
        NodeSearch(parent=parent, align=eveui.Align.bottom_left, controller=self._controller)
    ...
    def _construct_nodes(self):
        ...
        for node_id, node_data in self._controller.nodes_data.iteritems():
            ...
            node = node_controller.node_component_class(parent=self._node_container, align=eveui.Align.top_left, graph_controller=self._controller, node_controller=node_controller, edit_node=self._edit_node, top=y, left=x)
        ...
        for node_id, node_data in self._controller.nodes_data.iteritems():
            for connection_id, to_node_ids in node_data.connections.iteritems():
                for to_node_id in to_node_ids:
                    self._construct_connection(from_node_id=node_id, from_connection_type='output', from_connection_id=connection_id, to_node_id=to_node_id, to_connection_type='input', to_connection_id=InPort.input, connection_color=get_connection_color(connection_id))

            for parameter_id, parameter in node_data.nodeParameters.iteritems():
                if parameter.connection:
                    self._construct_connection(from_node_id=parameter.connection.node, from_connection_type='output_variables', from_connection_id=parameter.connection.parameter, to_node_id=node_id, to_connection_type='input_variables', to_connection_id=parameter_id, connection_color=get_connection_color('variables'))
    ...
    def _construct_connection(self, from_node_id, from_connection_type, from_connection_id, to_node_id, to_connection_type, to_connection_id, connection_color):
...
class AuthoredNodeGraphView(BaseNodeGraphView):
    ...
    def Close(self):
        ...
        super(AuthoredNodeGraphView, self).Close()
    ...
    def _construct_left_top_content(self):
        ...
        if hasattr(self._controller, 'has_custom_graph'):
...
class NodeSearch(eveui.ContainerAutoSize):
    ...
    def _update(self):
        ...
        self._update_buttons()
        if text:
            for node_id in self._controller.nodes_data:
                node_controller = self._controller.get_node_controller(node_id)
                if text in node_id.lower() or text in node_controller.title.lower():
