#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\crimewatch\corp_aggression\settings.py
import logging
...
class AggressionSettings:
    ...
    def CreateFromDBRows(rows):
        if len(rows) == 1:
            return AggressionSettings.CreateFromDBRow(rows[0])

    @staticmethod
    def CreateFromDBRow(row):
        return AggressionSettings(row.enableAfter, row.disableAfter)
