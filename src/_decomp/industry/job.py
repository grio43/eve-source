#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\industry\job.py
import math
import copy
import random
import signals
...
class JobBase(industry.Base):
    ...
    def __new__(cls, blueprint, activityID, *args, **kwargs):
        ...
        obj.on_validate = signals.Signal(signalName='on_validate')
        obj.on_updated = signals.Signal(signalName='on_updated')
        obj.on_errors = signals.Signal(signalName='on_errors')
        obj.on_delete = signals.Signal(signalName='on_delete')
        obj.on_facility = signals.Signal(signalName='on_facility')
        obj.on_input_location = signals.Signal(signalName='on_input_location')
        obj.on_output_location = signals.Signal(signalName='on_output_location')
        obj.on_dirty = signals.Signal(signalName='on_dirty')
    ...
    def _get_successful_runs(self):
        if self.completed:
            if self._successfulRuns is None:
                rand = random.Random()
                rand.seed(self.random)
                probabilities = [ rand.random() for i in range(self.runs) ]
                self._successfulRuns = len([ p for p in probabilities if p <= self.probability ])
    ...
    def _get_base_cost(self):
        return int(round(self.activity.job_cost(self) or 0))

    base_cost = property(_get_base_cost)

    def _get_total_cost(self):
        return (self.cost or 0) + (self.tax or 0) + (self.scc_surcharge or 0)

    total_cost = property(_get_total_cost)

    def _get_scc_surcharge_ratio(self):
        if self.facility:
            sccTaxModifier = self.facility.sccTaxModifier
        else:
            sccTaxModifier = 1.0
        scc_rate = industry.SCC_SURCHARGE_RATE * sccTaxModifier
        return max(0, scc_rate)

    scc_surcharge_ratio = property(_get_scc_surcharge_ratio)

    def _get_scc_surcharge(self):
        return int(round(self.base_cost * self.scc_surcharge_ratio))

    scc_surcharge = property(_get_scc_surcharge)
    ...
    def _get_cost_for_modifiers(self, modifiers):
        cost = base_cost = self.activity.job_cost(self) or 0
        additive_cost = 0
        for modifier in modifiers:
            if isinstance(modifier, industry.CostModifier) and modifier.activity in (None, self.activityID):
                if modifier.additive:
                    additive_cost += base_cost * (modifier.amount - 1.0)
                else:
                    cost *= modifier.amount

        return int(round(cost + additive_cost))
...
class Job(JobBase):
    ...
    def _get_tax(self):
        ...
        return int(round(self.base_cost * tax))
    ...
    def _get_structure_service_id(self):
        if self.productTypeID or self.activityID not in (industry.MANUFACTURING, industry.REACTION):
            return structures.GetServiceID(self.activityID, self.productTypeID)
        else:
            return structures.GetServiceID(self.activityID, self.products[0].typeID if self.products else self.productTypeID)

    structure_service_id = property(_get_structure_service_id)
    ...
    def _get_errors(self):
        ...
        if self.facility is None:
            ...
            if self.facility.tax is None:
                serviceAccess = False
                if evetypes.GetCategoryID(self.facility.typeID) == inventoryconst.categoryStructure:
                    serviceID = self.structure_service_id
                    if serviceID and self.facility.serviceAccess.get(serviceID) is not None:
                        serviceAccess = True
                if not serviceAccess:
        ...
        if not self.blueprint_location:
            self.add_error(industry.Error.INVALID_BLUEPRINT_LOCATION, self.blueprint.location.itemID)
    ...
    def create(cls, request, blueprint):
        ...
        job.installerID = request['characterID']
