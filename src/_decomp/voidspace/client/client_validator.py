#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\voidspace\client\client_validator.py
import gametime
import blue
import inventorycommon.const as invconst
from abyss import get_all_abyss_system_ids
from eve.common.lib import appConst
from voidspace.common import get_all_void_system_ids
from voidspace.common.error import VoidSpaceJumpError

class ClientVoidSpaceJumpValidator(object):

    def __init__(self, key = None):
        super(ClientVoidSpaceJumpValidator, self).__init__()
    ...
    def add_error(self, error, *args):
        if isinstance(error, VoidSpaceJumpError):
    ...
    def _is_in_abyss_system(self):
        return self.get_session().solarsystemid2 in get_all_abyss_system_ids()

    def _is_in_void_space_system(self):
        return self.get_session().solarsystemid2 in get_all_void_system_ids()

    def _is_session_change_in_progress(self):
        return self.get_session().nextSessionChange > gametime.GetSimTime()
    ...
    def _validate_key(self):
        ...
        if key.groupID != invconst.groupVoidSpaceKeys:
            self.add_error(VoidSpaceJumpError.INVALID_KEY_TYPE, self.get_char_id())
        if key.ownerID != self.get_char_id():
            self.add_error(VoidSpaceJumpError.INVALID_OWNER, self.get_char_id())
        if key.stacksize < 1:
            self.add_error(VoidSpaceJumpError.INVALID_KEY_AMOUNT)

    def is_docked(self):
        return self.get_session().structureid or session.stationid
    ...
    def raise_errors(self):
        if self._errors:
            raise VoidSpaceJumpError(self._errors)
    ...
    def get_ship_item(self):
        ...
        try:
            ...
            raise VoidSpaceJumpError([(VoidSpaceJumpError.SHIP_NOT_FOUND, ())])
    ...
    def perform_base_validation_checks(self):
        self._validate_ship()
        if self.is_docked():
            self.add_error(VoidSpaceJumpError.SHIP_IS_DOCKED, self.get_char_id())
        if self.is_cloaked():
            self.add_error(VoidSpaceJumpError.SHIP_IS_CLOAKED, self.get_char_id())
        if self.is_invulnerable():
            self.add_error(VoidSpaceJumpError.SHIP_IS_INVULNERABLE, self.get_char_id())
        if self.is_self_destructing():
            self.add_error(VoidSpaceJumpError.SHIP_IS_SELF_DESTRUCTING, self.get_char_id())
        if self.is_tethered():
            self.add_error(VoidSpaceJumpError.SHIP_IS_TETHERED, self.get_char_id())
        if self.is_warping():
            self.add_error(VoidSpaceJumpError.SHIP_IN_WARP, self.get_char_id())
        if self._is_in_abyss_system():
            self.add_error(VoidSpaceJumpError.IN_ABYSS_SYSTEM, self.get_char_id())
        if self._is_in_void_space_system():
            self.add_error(VoidSpaceJumpError.VOID_SPACE_DISALLOWED)
        if self._is_session_change_in_progress():
            self.add_error(VoidSpaceJumpError.SESSION_CHANGE_IN_PROGRESS, self.get_char_id())
        if self.is_pvp_timer_active():
            self.add_error(VoidSpaceJumpError.PVP_TIMER_ACTIVE, self.get_char_id())
