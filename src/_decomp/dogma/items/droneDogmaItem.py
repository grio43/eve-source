#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\dogma\items\droneDogmaItem.py
import weakref
...
class DroneDogmaItem(BaseDogmaItem):
    ...
    def IsOwnerModifiable(self, locationID = None):
        ...
        if not IsSolarSystem(locationID) and not boot.role == 'client':
            return False
        if not pilotShip.IsItemIDInDrones(self.itemID):
            return False
        return True
...
class GhostDroneDogmaItem(DroneDogmaItem):
    FAKE_LOCATION_ID = -1
    ...
    def SetLocation(self, locationID, locationDogmaItem, flagID):
        ...
        if flagID == const.flagDroneBay:
            locationDogmaItem.RegisterDrone(self)
        self.HandleLocationChange(self.FAKE_LOCATION_ID)

    def IsOwnerModifiable(self, locationID = None):
        if locationID == self.FAKE_LOCATION_ID:
            return False
        return DroneDogmaItem.IsOwnerModifiable(self, locationID)
