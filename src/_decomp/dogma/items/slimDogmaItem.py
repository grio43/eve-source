#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\dogma\items\slimDogmaItem.py
import itertools
import weakref
...
from dogma.attributes.itemAttributeFactory import NewItemAttributeFactory
...
class SlimDogmaItem(object):
    ...
    def __init__(self, dogmaLocation, invItem, clientIDFunc, attributes = None):
        ...
        attributeFactory = NewItemAttributeFactory(self, self.dogmaLocation, attributes)
    ...
    def HandleLocationChange(self, oldLocationID):
        if self.location is None:
            ...
            try:
                oldLocationItem = self.dogmaLocation.dogmaItems[oldLocationID]
            except KeyError:
                pass
            else:
                self.RemoveModifierSet(oldLocationItem.locationMods)
                try:
                    locationGroupMods = oldLocationItem.locationGroupMods[self.groupID]
                except KeyError:
                    pass
                else:
                ...
                for skillID in self.reqSkills:
                    try:
                        locationReqSkillMods = oldLocationItem.locationReqSkillMods[skillID]
                    except KeyError:
                        pass
                    else:
                        self.RemoveModifierSet(locationReqSkillMods)

                if self in oldLocationItem.subItems:
                    oldLocationItem.subItems.remove(self)
    ...
    def HandleOwnerChange(self, oldOwnerID):
        ...
        if oldOwner is not None:
            oldOwner.ownedItems.discard(self)
            for skillID in self.reqSkills:
                try:
