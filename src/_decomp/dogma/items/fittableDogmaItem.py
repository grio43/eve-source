#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\dogma\items\fittableDogmaItem.py
...
class FittableDogmaItem(BaseDogmaItem):
    ...
    def SerializeForPropagation(self):
        ...
        if self.lastStopTime:
            retVal.lastStopTime = self.lastStopTime - blue.os.GetSimTime() + blue.os.GetWallclockTime()
        else:
            retVal.lastStopTime = None
        return retVal

    def UnpackPropagationData(self, propData, charID, shipID):
        BaseDogmaItem.UnpackPropagationData(self, propData, charID, shipID)
        if propData.lastStopTime:
            lastStopTime = propData.lastStopTime - blue.os.GetWallclockTime() + blue.os.GetSimTime()
        else:
            lastStopTime = None
        self.SetLastStopTime(lastStopTime)
