#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\randomJump\client\clientRandomJumpValidator.py
import gametime
import blue
import inventorycommon.const as invconst
from abyss import get_all_abyss_system_ids
from eveuniverse.solar_systems import is_random_jump_filament_restricted
from randomJump.error import RandomJumpError
from eve.common.lib import appConst

class ClientRandomJumpValidator(object):

    def __init__(self, key = None):
        super(ClientRandomJumpValidator, self).__init__()
    ...
    def add_error(self, error, *args):
        if isinstance(error, RandomJumpError):
    ...
    def _validate_key(self):
        ...
        if key.groupID not in (invconst.groupRandomJumpKeys, invconst.groupTriglavianJumpKeys):
            self.add_error(RandomJumpError.INVALID_KEY_TYPE, self.get_char_id())
        if key.ownerID != self.get_char_id():
            self.add_error(RandomJumpError.INVALID_OWNER, self.get_char_id())
        if key.stacksize < 1:
            self.add_error(RandomJumpError.INVALID_KEY_AMOUNT)
    ...
    def raise_errors(self):
        if self._errors:
            raise RandomJumpError(self._errors)
    ...
    def get_ship_item(self):
        ...
        try:
            ...
            raise RandomJumpError([(RandomJumpError.SHIP_NOT_FOUND, ())])
    ...
    def is_in_banned_system(self):
        return is_random_jump_filament_restricted(session.solarsystemid2)
    ...
    def validate_activation(self):
        self._errors = []
        self.perform_base_validation_checks()
        self.perform_activation_validation_checks()
        if self._errors:
            self.raise_errors()
    ...
    def perform_base_validation_checks(self):
        self._validate_ship()
        if self.is_docked():
            self.add_error(RandomJumpError.SHIP_IS_DOCKED, self.get_char_id())
        if self.is_cloaked():
            self.add_error(RandomJumpError.SHIP_IS_CLOAKED, self.get_char_id())
        if self.is_invulnerable():
            self.add_error(RandomJumpError.SHIP_IS_INVULNERABLE, self.get_char_id())
        if self.is_self_destructing():
            self.add_error(RandomJumpError.SHIP_IS_SELF_DESTRUCTING, self.get_char_id())
        if self.is_tethered():
            self.add_error(RandomJumpError.SHIP_IS_TETHERED, self.get_char_id())
        if self.is_warping():
            self.add_error(RandomJumpError.SHIP_IN_WARP, self.get_char_id())
        if self._is_in_abyss_system():
            self.add_error(RandomJumpError.IN_ABYSS_SYSTEM, self.get_char_id())
        if self.is_in_banned_system():
            self.add_error(RandomJumpError.IN_BANNED_SYSTEM)
        if self.is_blocked_by_safety():
            self.add_error(RandomJumpError.SAFETY_ENGAGED, self.get_char_id())

    def perform_jump_validation_checks(self):
        if self._is_session_change_in_progress():
            self.add_error(RandomJumpError.SESSION_CHANGE_IN_PROGRESS, self.get_char_id())

    def perform_activation_validation_checks(self):
        if self.is_pvp_timer_active():
            self.add_error(RandomJumpError.PVP_TIMER_ACTIVE, self.get_char_id())
