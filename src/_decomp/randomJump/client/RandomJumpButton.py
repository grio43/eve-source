#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\randomJump\client\RandomJumpButton.py
import itertools
import randomJump
...
class RandomJumpButton(PrimaryButton):

    def ApplyAttributes(self, attributes):
        super(RandomJumpButton, self).ApplyAttributes(attributes)
    ...
    def UpdateState(self):
        super(RandomJumpButton, self).UpdateState()
        super(RandomJumpButton, self).UpdateBlinkByState()
...
class ActivateButtonTooltipPanel(object):
    ...
    def Reconstruct(self, errors):
        ...
        for error, errorArgs in set(itertools.islice(errors, 5)):
            self.AddErrorRow(error, errorArgs)

        self.panel.AddSpacer(width=0, height=4, colSpan=2)

    def AddErrorRow(self, error, args):
        text = randomJump.get_error_label(localization.GetByLabel, error, *args)
