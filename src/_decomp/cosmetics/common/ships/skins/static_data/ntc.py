#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\cosmetics\common\ships\skins\static_data\ntc.py
        ...
        for i in range(len(ntc.names)):
            if color == '#' + ntc.names[i][0]:
                return ['#' + ntc.names[i][0],
                 ...
                 ntc.names[i][2],
                 True]
            ndf1 = (r - ntc.names[i][3]) ** 2 + (g - ntc.names[i][4]) ** 2 + (b - ntc.names[i][5]) ** 2
            ndf2 = abs((h - ntc.names[i][6]) ** 2) + (s - ntc.names[i][7]) ** 2 + abs((l - ntc.names[i][8]) ** 2)
            ndf = ndf1 + ndf2 * 2
            if df < 0 or df > ndf:
                df = ndf
                cl = i
