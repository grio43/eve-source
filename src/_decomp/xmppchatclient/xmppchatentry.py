#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\xmppchatclient\xmppchatentry.py
import logging
import re
import sys
import types
...
from xmppchatclient.const import MESSAGEMODE_TEXTONLY, MESSAGEMODE_SMALLPORTRAIT, MESSAGEMODE_BIGPORTRAIT
from xmppchatclient.xmppchannelmenu import XmppChannelMenu
from chat.client.const import SLASH_EMOTE_STRING
from chat.common.util import get_color_for_role
logger = logging.getLogger(__name__)
PICTURE_SIZE = {MESSAGEMODE_TEXTONLY: 0,
 MESSAGEMODE_SMALLPORTRAIT: 32,
 MESSAGEMODE_BIGPORTRAIT: 64}

class XmppChatEntry(SE_BaseClassCore):
    ...
    def ApplyAttributes(self, attributes):
        ...
        super(XmppChatEntry, self).ApplyAttributes(attributes)
    ...
    def Load(self, node):
        ...
        charname, text = XmppChatEntry.GetCharacterAndTextForNode(node)
        self.text.text = text
    ...
    def GetCharacterAndTextForNode(node):
        ...
        if nodetext.startswith('/slash'):
            ...
            color = get_color_for_role(node.colorkey)
            showCharname = True
        isEmote = False
    ...
    def GetDynamicHeight(node, width):
        props = XmppChatEntry.GetTextProperties(node)
        width = width - props['padLeft'] - props['padRight']
        maxLines = getattr(node, 'maxLines', None)
        _, text = XmppChatEntry.GetCharacterAndTextForNode(node)
        ...
        if node.mode == MESSAGEMODE_TEXTONLY:
            return textHeight
        elif node.mode == MESSAGEMODE_SMALLPORTRAIT:
            ...
            try:
                ...
                if parsed.item_id:
                    ...
                    if evetypes.GetGroupID(parsed.type_id) == const.groupCharacter:
                        ...
                        m += [None]
                        m += XmppChannelMenu(self.sr.node.channelid, parsed.item_id)
                else:
                    m = Link().GetLinkMenu(self.text, mouseOverUrl.replace('&amp;', '&'))
    ...
    def _OnSizeChange_NoBlock(self, width, height):
        super(XmppChatEntry, self)._OnSizeChange_NoBlock(width, height)
    ...
    def GetTextProperties(cls, node):
        ...
        if node.mode == MESSAGEMODE_TEXTONLY:
            ...
            if node.mode == MESSAGEMODE_SMALLPORTRAIT:
