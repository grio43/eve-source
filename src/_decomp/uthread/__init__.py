#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\uthread\__init__.py
__version__ = '0.1'
...
def LockCheck():
    while 1:
        blue.pyos.synchro.SleepSim(60660)
...
def PoolWithoutTheStars(ctx, func, args, kw, notls = False):
    ...
    if ctx[0] != '^':
        ...
        ctx = prefix + '^' + ctx
    stackless_tracing_enabled, kw = bluepy.tracing_enabled(**kw)
    tasklet = bluepy.TaskletExt(ctx, func, stackless_tracing_enabled=stackless_tracing_enabled)
    if notls:
