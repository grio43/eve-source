#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\spacecomponents\client\ui\underConstruction\bracketPanel.py
import math
...
class UnderConstructionBracketPanel(BracketPanel):

    def __init__(self, parent, camera, under_construction_root_transform, on_focus = None, itemID = None, typeID = None):
        self._camera = camera
        self._under_construction_root_transform = under_construction_root_transform
        self._state_machine = StateMachine()
        self._item_id = itemID
        self._type_id = typeID
        self.playerRequestedCollapseToken = None
        self._under_construction_transform = PanelTransformController(root=under_construction_root_transform, camera=camera, offset_y=400.0)
        super(UnderConstructionBracketPanel, self).__init__(name='UnderConstructionBracketPanel', transform=self._under_construction_transform.transform, layer=parent, camera=camera, collapse_at_camera_distance=75000.0, clipping_dead_zone=50, on_focus=on_focus, collapsed=True)
        self._base = ContainerAutoSize(name='underConstructionUI_base', alignMode=uiconst.CENTERBOTTOM)
        ...
        self._content_tracker_token = self.tracker.add(self._base, anchor=(0.0, 1.0), offset=(10, -20))
        tether_cont = Transform(name='underConstruction_tether', rotation=math.radians(270.0), width=100, height=100)
        ...
        self._tether = BracketTether(parent=tether_inner_cont, align=uiconst.TOPLEFT, collapsed=True, on_click=self._on_tether_click, on_close_point_click=self._on_close_point_click, width=112)
    ...
    @property
    def under_construction_transform(self):
        return self._under_construction_root_transform
    ...
    def close(self):
        ...
        self._under_construction_transform.close()
        super(UnderConstructionBracketPanel, self).close()

    def _on_state_update(self, link = None):
        from spacecomponents.client.ui.underConstruction.underConstructionStates import Idle
