#Embedded file name: C:\BuildAgent\work\ba3dced9a47cf95a\eve\release\V22.02\packages\spacecomponents\client\components\microJumpDriver.py
import logging
...
class MicroJumpDriver(Component):
    ...
    def _StartVisualEffectThread(self):
        ...
        try:
            ...
            self.lastEndTime = None
            self.TriggerEffect(False)

    def TriggerEffect(self, isTriggered):
        logger.debug('TriggerEffect %s', isTriggered)
        ball = sm.GetService('michelle').GetBall(self.itemID)
        if ball is not None:
            ball.SetControllerVariable('IsTriggered', isTriggered)
    ...
    def GetAttributeInfo(typeID, attributes, instance, localization):
        ...
        attributeEntries = [EntryData(Header, localization.GetByLabel('UI/Inflight/SpaceComponents/MicroJumpDriver/InfoAttributesHeader')),
         ...
         DogmaEntryData(LabelTextSides, GetDogmaAttributeAndValue(typeID, attributeSignatureRadiusBonusPercent)),
